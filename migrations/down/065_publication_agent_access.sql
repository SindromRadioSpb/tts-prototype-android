-- Fail closed: the down migration is only for an empty rehearsal. It must not
-- silently discard owner assertions or active grants for the three new scopes.
CREATE TEMP TABLE publication_agent_access_down_guard(value INTEGER NOT NULL CHECK(value=0));
INSERT INTO publication_agent_access_down_guard
  SELECT COUNT(*) FROM published_corpus_agent_rights_facts;
INSERT INTO publication_agent_access_down_guard
  SELECT COUNT(*) FROM agent_connection_grants
   WHERE scope IN ('reading.publication.catalog.read','reading.publication.item.read','reading.publication.resource.read');
DROP TABLE publication_agent_access_down_guard;

DROP TRIGGER IF EXISTS trg_publication_agent_rights_events_no_delete;
DROP TRIGGER IF EXISTS trg_publication_agent_rights_events_no_update;
DROP TRIGGER IF EXISTS trg_published_agent_rights_no_delete;
DROP TRIGGER IF EXISTS trg_published_agent_rights_no_update;
DROP TRIGGER IF EXISTS trg_published_agent_rights_target_valid;
DROP TABLE publication_agent_rights_idempotency;
DROP TABLE publication_agent_rights_events;
DROP INDEX IF EXISTS ix_published_agent_rights_latest;
DROP TABLE published_corpus_agent_rights_facts;
