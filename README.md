...
| [`docs/PREMIUM_RELEASE_PLAN_v3_1.md`](docs/PREMIUM_RELEASE_PLAN_v3_1.md) | План v3.1.0 — 8 directions, audit checklist (reconciled 2026-05-10). |
| [`docs/PREMIUM_RELEASE_PLAN_v3_2.md`](docs/PREMIUM_RELEASE_PLAN_v3_2.md) | План v3.2.0 — premium notes + text-card system + analytics foundation + research mode (master plan, links to 4 sub-plans). |
...
### v3.2.0 — в разработке
Mega-release объединяющий **четыре directions** в одну cohesive поставку:

- **[Direction 9 — Premium Notes Redesign](docs/PREMIUM_NOTES_PLAN_v3_2.md)** — polymorphic targets (sentence / word / root / binyan / text / note / free) + 4 templates (word_study / grammar_rule / translation_discrepancy / pronunciation_note) + audio-anchored notes + bidirectional links + version history + note → SRS micro-card. **Категория-defining** для Hebrew-learning notes.
- **[Direction 10 — Text-card System](docs/TEXT_CARD_PLAN_v3_2.md)** — three-mode artifact lifecycle: Mode A (bulk self-serve builder), Mode B (peer-share via lightweight JSON, content-addressed audio cache restoration), Mode C (curator request с Standard/Curated split). Full lifecycle: создание → распространение → опциональное курирование.
- **[Direction 11A — Analytics Foundation](docs/ULPAN_RESEARCH_PLAN_v3_2.md)** *(mini-direction, ships independently)* — event emission gap closure (12 event types) + heartbeat-based time-spent v2. Closes existing CONTRACTS_ANALYTICS drift; improves Activity Heatmap accuracy для всех users.
- **[Direction 11B — Research Mode](docs/ULPAN_RESEARCH_PLAN_v3_2.md)** — opt-in privacy-preserving research infrastructure: anonymous student_id + cohort code + daily aggregate uploads + IRB-style consent + teacher dashboard `/teacher.html`. Enables Hebrew ulpan diploma project (academic methodology contribution).

Companion docs: [`RESEARCH_METRICS_SCHEMA.md`](docs/RESEARCH_METRICS_SCHEMA.md), [`RESEARCH_ETHICS_CONSENT_TEMPLATE.md`](docs/RESEARCH_ETHICS_CONSENT_TEMPLATE.md).

Master plan: [`PREMIUM_RELEASE_PLAN_v3_2.md`](docs/PREMIUM_RELEASE_PLAN_v3_2.md).

### Deferred → v3.3
- **Functional code-split** (Dashboard / SRS / IDE → отдельные dynamic-import ES-модули) — требует extraction inline `<script>` из 30k-line монолита.
...
- **Calibrated in-app diagnostic quiz** для outcome capture v2 (Direction 11).
...
- **Hebrew local TTS commercial** — license-blocked.
