...
### Changed — единая заметка о слове в Зале и Студии (v3.11.241)

- Подтверждён и сделан видимым единый контракт: `＋ Сохранить слово` в Читальном зале создаёт
  каноническую `word_study` в `notes_v2`, а «Заметки к строке» показывает ту же запись через
  `note_occurrences`; второй параллельной сущности нет.
- Сохранённое состояние стало действием `✎ Дополнить заметку`: прямо в карточке слова доступны
  личная формулировка значения, мнемоника и собственный пример. Сохранение обновляет ту же запись,
  ставит `user_touched=1` и не пишет в FSRS/`review_log`.
- При первой личной переформулировке исходная машинная подсказка сохраняется отдельно как
  `reference_meaning`, остаётся видимой и восстанавливается при очистке личной формулировки.
- Карточка Студии показывает личный блок либо честное пустое состояние, а `Открыть` переименовано
  в `Редактировать заметку`. Справка объясняет разницу между компактным редактором Зала и полным
  редактором Студии.
- `smoke:reader-notes` теперь детерминированно создаёт локальный текст и проверяет реальный
  OPFS-roundtrip трёх личных полей; `smoke:notes:wordcard` проверяет их отображение на 390 px.
...
- **Пре-флайт** при клике: точные счётчики (тексты · аудио), ≈размер, ≈время →
  выбор `[Экспорт с аудио]` / `[Только метаданные (быстро)]` / `[Отмена]`. Soft-warn
  (amber) при >3000 аудио или >200 МБ.
- **«Только метаданные»** — быстрый ZIP без папки `audio/` (аудио восстановится на
  приёмнике из content-addressed кэша при первом воспроизведении); `export_mode` в манифесте.
- **Прогресс-карточка** вместо строки: бар (вес аудио 85% / упаковка 15%) + степпер
  ①Чтение→②Аудио→③Упаковка→④Готово + «N/Total · МБ» + ETA/темп + «⚠N пропусков».
- **Отмена** — единый `AbortController`, прерывает фетчи/упаковку, без частичного файла.
- **Глобальный чип** (bottom-right; в RTL — bottom-left) переживает закрытие модалки:
  клик → возврат на карточку, ✕ → отмена.
- A11y: `role="progressbar"` + `aria-live`. i18n namespace `exp.*` (ru/en/he).

### Fixed — Library «Экспорт ZIP (с аудио)» прогресс (SW v3.10.80-zip-export-progress)

- **Персистентный прогресс экспорта.** Тост «Подготовка ZIP с аудио…»
  гас через 2.5 с, а сборка большой библиотеки идёт минутами — пользователь
  не отличал «работает» от «оборвалось». Теперь строка статуса библиотеки
  ведёт живой счётчик: `Сборка ZIP: N текстов, M аудио…` → `Загрузка аудио
  {done}/{total}…` → `Упаковка ZIP… {percent}%` → итог (и на ошибке).
- **STORE вместо DEFLATE для аудио.** MP3 уже сжат — DEFLATE level 6 давал
  ~0% выигрыша, но жёг минуты CPU в главном потоке. Аудио теперь кладётся
  без сжатия (упаковка из минут → секунды); DEFLATE остаётся на JSON-манифестах.
  Формат ZIP совместим с импортом.
- **Таймаут 8 с на каждый аудио-запрос** (AbortController) — один зависший
  ассет больше не стопорит весь экспорт (помечается `timeout`).
- **Прогресс упаковки** через `generateAsync(onUpdate → meta.percent)`.
