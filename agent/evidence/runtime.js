...
const audioRepo=require("../../db/audioRepo");
...
async function audio(ctx,id){const a=await requireUse(ctx.userId);if(!a.ok)return a;const req=await repo.loadRequest(ctx.userId,String(id));if(!req)return {ok:false,error:"F2_NOT_FOUND"};if(req.construct_id!==C.CONSTRUCTS.B1||!["OFFERED","ACCEPTED"].includes(req.state))return {ok:false,error:"F2_AUDIO_UNAVAILABLE"};const vocalized=req.expected&&req.expected.vocalized;if(!vocalized)return {ok:false,error:"F2_AUDIO_UNAVAILABLE"};const assetKey=computeDictateAssetKey(vocalized);if(!await audioRepo.hasAsset(assetKey))return {ok:false,error:"F2_AUDIO_UNAVAILABLE"};return {ok:true,assetKey};}
