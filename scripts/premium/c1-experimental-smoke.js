...
ok(server.includes('microphone=(self)') && server.includes('loopback-network=(self)') && server.includes('local-network-access=(self)') && server.includes('PRONUNCIATION_CSP'), "pronunciation shell must allow microphone and permission-gated loopback under strict CSP");
ok(server.includes("http://127.0.0.1:8766") && !server.includes("/api/pronunciation"), "Node server must allow loopback UI but expose no pronunciation API");
...
ok(client.includes('http://127.0.0.1:8766') && client.includes('X-C1-Token'), "client must use token-gated loopback only");
ok(client.includes('targetAddressSpace: "loopback"') && client.includes('local-network-access'), "client must trigger and diagnose the browser loopback permission");
...
ok(sw.includes('v3.11.240') && sw.includes('/pronunciation.html') && sw.includes('/js/pronunciation-lab.js'), "new shell assets must be versioned and precached");
...
const required = [
  ...
  "privacyAudio","license","errorNetworkDenied","errorConnect","errorMic","errorScore",
];
