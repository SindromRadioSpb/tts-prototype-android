...
(async()=>{
  ...
  try{
    ...
    await plain.locator('dialog').getByRole('button',{name:'Сохранить привязку',exact:true}).click();await plain.waitForFunction(()=>document.querySelector('dialog [role=status]').textContent==='Привязка сохранена');
    ...
    await guest.goto(ORIGIN+'/library.html?canon=skip&public_corpus=youtube-fixture&public_work=youtube-fixture',{waitUntil:'load'});await guest.locator('#readerStudyVideo').waitFor({state:'visible'});
    await guest.locator('#readerStudyVideo').click();await guest.waitForURL('**/study-video.html#*');await guest.waitForSelector('#proTable tbody tr');assert.equal(await guest.locator('#proTable tbody tr').count(),3);
    const publicView=await guest.evaluate(async()=>StudyVideoTransfer.get(location.hash.slice(1)));assert.equal(publicView.video_id,'djzKaEoqka8');assert.deepEqual(publicView.entries,source.view.entries);
