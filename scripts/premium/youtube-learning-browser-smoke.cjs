...
(async()=>{
  ...
  try{
    ...
    await guest.goto(ORIGIN+'/library.html?canon=skip&public_corpus=youtube-fixture&public_work=youtube-fixture',{waitUntil:'load'});await guest.locator('#roomMediaPlayBtn').waitFor({state:'visible'});
    await guest.locator('#roomMediaPlayBtn').click();await guest.waitForSelector('#proTable tbody tr');assert.equal(await guest.locator('#proTable tbody tr').count(),3);assert.ok(guest.url().includes('/library.html'));
    const publicView=await guest.evaluate(async()=>{const link=document.querySelector('#roomMediaBar .playback-source-actions a');const texts=await __localDB.dbQuery("SELECT id FROM texts WHERE source_meta_json LIKE '%playback_source%'");const ctx=await StudyVideoSourceUI.context(texts[0].id);const view=await PlaybackSource.youtubeView(ctx.audio,ctx.rows,ctx.record);return {video_id:link?PlaybackSource.parseVideoId(link.href):null,rows:ctx.rows,entries:view.entries,reason:document.querySelector('#roomMediaBarNote').textContent};});assert.equal(publicView.video_id,'djzKaEoqka8');assert.deepEqual(publicView.entries,source.view.entries);
