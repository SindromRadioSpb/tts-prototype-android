...
const must=["mentor-memory-wrap","mentor-memory-card","mentor_memory_store","mentor_memory_unfinished","mentor_memory_candidates","/api/agent/memory/proposals","DELETE MEMORY","room.mentor.memory.title"];
...
for(const loc of ["ru","en","he"]){const s=fs.readFileSync(path.join(root,"public/i18n/locales",loc+".js"),"utf8");if(!s.includes("memory: {")||!s.includes("deleteAll:")||!s.includes("why:"))throw new Error("locale incomplete "+loc);}
