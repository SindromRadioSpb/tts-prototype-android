...
const SCOPES = [
  ...
  ["reading.publication.derivative.read", "READ_OWNER_REVIEWED_LEARNING_DERIVATIVE", "PHYSICS_CONDITION_BEGINNER_BRIDGE_EXAM_SOLUTION_ANSWER_AND_PROVENANCE", "NO_LEARNER_STATE_NO_GRADES_NO_NOTES_NO_PRIVATE_OR_GROUP_TEXT_EXACT_EDITION_ITEM_AND_DERIVATIVE_RIGHT_REQUIRED", "CONTENT"],
];
...
http.createServer((req, res) => {
  ...
  if (url.pathname === "/api/agent-access/consent/fixture-request") return json(res, { ok: true, preview: { schema_version: "aa.consent.preview.1.0.0", request_id: "fixture-request", client_display_name: "Hermes · новый профиль", connection_label: "Personal learning agent", requested_scopes: SCOPES.map(([scope, purpose, data_class, excludes, retention_tier]) => ({ scope, purpose, data_class, excludes, retention_tier, downstream_retention: retention_tier === "CONTENT" ? "EXTERNAL_STORAGE_OUTSIDE_LINGUISTPRO_CONTENT_IRRECOVERABLE" : "EXTERNAL_STORAGE_OUTSIDE_LINGUISTPRO" })), retention_tier: "CONTENT", consent_version: "agent-access-consent-v4", capability_version: "aa-v0.1", retention_notice_version: "downstream-retention-v2", expires_at: "2099-01-01T00:00:00.000Z", canonical_authority: "LINGUISTPRO_ONLY", external_memory_is_learner_truth: false } });
