...
(async () => {
  ...
  await page.goto(ORIGIN + '/?v=513', { waitUntil: 'domcontentloaded' });
  await page.waitForFunction(() => window.StudioImport && window.YoutubeAsr && window.LearningMaterialTaskUI);

  check('the shell serves the release under test', await page.evaluate(() => window.APP_VERSION) === '3.11.513');
  ...
  async function providerFor(storage) {
    ...
    await p2.goto(ORIGIN + '/?v=513', { waitUntil: 'load' });
