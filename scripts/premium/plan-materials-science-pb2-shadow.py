...
def write_json(path: Path, value: Any) -> None:
    # Historical packet hashes were frozen from Windows CRLF bytes. Keep that
    # serialization explicit on every platform (see the scoped .gitattributes).
...
def build(args: argparse.Namespace) -> None:
    ...
    (out / "shadow-audit-prompt.md").write_text(prompt, encoding="utf-8", newline="\n")
...
"""
    (out / "SHADOW_PLAN.md").write_text(plan, encoding="utf-8", newline="\n")
