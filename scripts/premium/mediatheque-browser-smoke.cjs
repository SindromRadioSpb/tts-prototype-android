...
const OUT=path.join(ROOT,'docs/research/room-mediatheque/2026-09-12/local');
...
async function shot(page,name){await page.screenshot({path:path.join(OUT,name+'.png')});const overflow=await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+1);check(name+' no horizontal overflow',!overflow);evidence.screenshots.push(name+'.png');}
...
async function personal(page){
 ...
 await page.locator('[data-action=edit-category][data-id="'+science+'"]').click();await act(page,'merge-category');await page.locator('[name=targetId]').selectOption(history);await submit(page);
 ...
 await act(page,'import');await page.locator('[name=file]').setInputFiles({name:'structure.json',mimeType:'application/json',buffer:backup});await submit(page);check('structure restore returns exact prior categories',(await structure(page)).structure.categories.length===3);
...
async function editorial(page,guest){
 ...
 await act(page,'organize');await act(page,'new-category');await page.locator('[name=title]').fill('Публичная наука');await submit(page);
 ...
 await act(page,'preview');check('preview hides editorial category controls',await page.locator('[data-action=new-category]').count()===0);await act(page,'publish');await submit(page);
...
async function main(){
 ...
 page=await context.newPage();const guest=await guestContext.newPage();await personal(page);await editorial(page,guest);await offline(browser);check('zero browser page errors',evidence.errors.length===0);evidence.status='PASS';
