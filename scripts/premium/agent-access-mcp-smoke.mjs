...
const allScopes = [...new Set(Object.values(capabilities.CAPABILITIES).map((entry) => entry.scope))];
...
const args = Object.freeze({
  ...
  get_personal_text_content: { text_key: 'text-1783830247939-hpbn', rows: 5 },
  get_word_morphology: { word: 'לכתוב' },
  get_text_coverage: { target: { work_id: '42' }, top_unknown_limit: 10 },
  search_group_reading_catalog: { corpus_id: 'study-songs-pilot', query: 'אושר', audio: 'ANY', sort: 'RELEVANCE', limit: 10 },
  get_group_reading_content: { corpus_id: 'study-songs-pilot', work_id: 'song-pos-001', rows: 5 },
  get_group_text_coverage: { corpus_id: 'study-songs-pilot', work_id: 'song-pos-001', top_unknown_limit: 10 },
  propose_import_text: { source:{title:'טקסט',origin:'OWNER_SUPPLIED'},body_preview:'שלום',language:'he',niqqud_status:'NONE',reason:'ללמוד' },
  ...
  read_published_learning_support: { corpus_slug: 'physics-year1-problems', edition_id: 'ed-physics-2', edition_item_id: 'ei-physics-1' },
});
...
try {
  ...
  for (const [scope, deniedTool] of [
    ...
    ['reading.publication.derivative.read', 'list_published_public_corpora'],
  ]) {
