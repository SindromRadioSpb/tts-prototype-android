...
(function (root) {
  ...
  function exactImpact(cluster, selected, selectAll) {
    ...
    // `batch_review_eligible` governs the one-click selection of the entire
    // cluster. It must not block an explicit subset that the owner reviewed
    // occurrence by occurrence: exactImpact still enumerates every selected
    // id and the confirmation screen remains mandatory before writing.
    if(selectAll&&!cluster.batch_review_eligible)throw new Error('LEXICAL_BATCH_NOT_ELIGIBLE');
  ...
  function open(options) {
    ...
    function clusterCard(cluster,index) {
      ...
      if(cluster.batch_review_eligible){const allLabel=$('label','lexres-select-all');selectAll=$('input');selectAll.type='checkbox';const allCopy=$('span','',t('room.resolution.selectAll','Выбрать все примеры'));allLabel.append(selectAll,allCopy);selectionBar.append(explain(allLabel,t('room.resolution.selectAllHelp','Отметить все оставшиеся примеры этой группы. Можно снять отметки с отдельных предложений.')).wrap);selectAll.addEventListener('change',()=>{selectedIds=selectAll.checked?new Set(cluster.occurrence_ids):new Set();updateSelectionState();});}
      else{const manual=$('span','lexres-manual-selection',t('room.resolution.manualSelection','Выберите примеры вручную'));manual.tabIndex=0;manual.setAttribute('role','note');selectionBar.append(explain(manual,t('room.resolution.manualSelectionHelp','Автовыбор всей группы отключён: примеры могут требовать разных разборов. Можно отметить несколько проверенных примеров по одному.')).wrap);}
      contexts.append(selectionBar,contextList);
      cluster.occurrences.forEach((occ,occIndex)=>{const label=$('label','lexres-context');const checkbox=$('input');checkbox.type='checkbox';checkbox.name='lexres-'+cluster.lp_resolution_cluster_id;checkbox.value=occ.lp_occurrence_id;checkbox.checked=occIndex===0;checkbox.addEventListener('change',()=>{if(checkbox.checked)selectedIds.add(checkbox.value);else selectedIds.delete(checkbox.value);updateSelectionState();});const copy=$('span');const he=$('bdi','lexres-he',clean(occ.sentence_he_niqqud||occ.sentence_he));he.dir='rtl';copy.append(he,$('small','',clean(occ.sentence_ru)));const tech=$('details','lexres-technical');tech.append($('summary','',t('room.resolution.technical','Технические данные')),$('code','',occ.lp_occurrence_id));copy.append(tech);label.append(checkbox,copy);contextInputs.push({input:checkbox,label});contextList.append(label);});
