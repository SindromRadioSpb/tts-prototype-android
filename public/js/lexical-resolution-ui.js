...
(function (root) {
  ...
  function open(options) {
    ...
    function clusterCard(cluster,index) {
      ...
      const contexts=$('fieldset','lexres-contexts');contexts.append($('legend','',t('room.resolution.contexts','Примеры из текста')),$('p','lexres-context-help',t('room.resolution.contextsHelp','Выберите предложение, для которого сохраняете решение. Перед групповым применением просмотрите все примеры.')));const contextList=$('div','lexres-context-list');contexts.append(contextList);
      let selectedId=cluster.occurrence_ids[0],clearButton=null;
      const updateClearState=()=>{if(!clearButton)return;const selected=(cluster.occurrences||[]).find((occ)=>occ.lp_occurrence_id===selectedId);const available=!!(selected&&selected.resolution_event_id);clearButton.setAttribute('aria-disabled',available?'false':'true');clearButton.classList.toggle('is-disabled',!available);};
      cluster.occurrences.forEach((occ,occIndex)=>{const label=$('label','lexres-context');const radio=$('input');radio.type='radio';radio.name='lexres-'+cluster.lp_resolution_cluster_id;radio.value=occ.lp_occurrence_id;radio.checked=occIndex===0;radio.addEventListener('change',()=>{selectedId=radio.value;updateClearState();});const copy=$('span');const he=$('bdi','lexres-he',clean(occ.sentence_he_niqqud||occ.sentence_he));he.dir='rtl';copy.append(he,$('small','',clean(occ.sentence_ru)));const tech=$('details','lexres-technical');tech.append($('summary','',t('room.resolution.technical','Технические данные')),$('code','',occ.lp_occurrence_id));copy.append(tech);label.append(radio,copy);contextList.append(label);});
      ...
      const batchLabel=$('label','lexres-batch');const batch=$('input');batch.type='checkbox';batch.disabled=!cluster.batch_review_eligible;const batchCopy=$('span');batchCopy.append($('strong','',cluster.batch_review_eligible?t('room.resolution.batch','Применить один разбор ко всем примерам'):t('room.resolution.batchUnavailableTitle','Проверяйте эти примеры по одному')),$('small','',cluster.batch_review_eligible?t('room.resolution.batchHelp','Включайте только после просмотра всех примеров: один разбор будет сохранён для каждого из них.'):t('room.resolution.batchUnavailable','Примеры могут означать разное или требуют дополнительной проверки, поэтому общее решение отключено.')));batchLabel.append(batch,batchCopy);editor.append(batchLabel);
      const actions=$('div','lexres-actions');
      const addAction=(labelText,action,helpText,primary)=>{const button=$('button','lexres-button'+(primary?' lexres-primary':''),labelText);button.type='button';button.addEventListener('click',()=>{const selected=(cluster.occurrences||[]).find((occ)=>occ.lp_occurrence_id===selectedId);if(action==='clear'&&!(selected&&selected.resolution_event_id)){setMessage(t('room.resolution.clearUnavailable','Для выбранного примера ещё нет сохранённого решения.'),'error');return;}const analysis=action==='manual_correction'?formAnalysis(form):{};if(action==='manual_correction'&&(!analysis.lemma||!analysis.lp_pos)){setMessage(t('room.resolution.required','Укажите лемму и часть речи.'),'error');return;}if(action==='manual_correction'&&clean(form.elements.pealim_ref.value)&&!analysis.pealim_id){setMessage(t('room.resolution.pealimInvalid','Укажите корректную ссылку Pealim или числовой ID.'),'error');return;}stage(inner,contexts,cluster,selectedId,action,analysis,batch.checked);});actions.append(explain(button,helpText).wrap);return button;};
      ...
      clearButton=addAction(t('room.resolution.clear','Отменить прежнее решение'),'clear',t('room.resolution.clearHelp','Убрать ранее сохранённое решение и снова показать пример как непроверенный.'));updateClearState();
