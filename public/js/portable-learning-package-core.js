...
(function (root, factory) {
  ...
  async function dryRun(verified, inventory) {
    ...
    const planBase = { portable_package_id: verified.manifest.portable_package_id, content_root_sha256: verified.manifest.content_root_sha256, package_mode: verified.manifest.package_mode, actions, conflicts, text_key: textKey, media: { sha256: mediaSha, status: mediaSha ? (mediaPresent ? 'exact' : 'missing') : 'unbound' }, estimated: { node_count: verified.graph.artifacts.length, edge_count: verified.graph.edges.length, row_count: verified.payload.table_revisions.reduce((count, revision) => count + (revision.rows || []).length, 0), write_count: actions.filter((item) => item.action === 'insert').length } };
