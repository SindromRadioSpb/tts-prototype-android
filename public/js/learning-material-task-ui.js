(function(){
  ...
  const journeyWords={
    ru:{stageMedia:'Медиа',stageTranscript:'Транскрипт',stageReview:'Проверка',stageOf:'Этап {i} из {n}',
      ...
      finalMissing:'Без ▶ осталось строк: {k}: их текст не совпал с репликами транскрипта.',finalLocal:'Видео хранится в этом браузере: на другом устройстве плеер его не найдёт.',
    ...
    en:{stageMedia:'Media',stageTranscript:'Transcript',stageReview:'Review',stageOf:'Step {i} of {n}',
      ...
      finalMissing:'Rows without ▶: {k}: their text did not match the transcript cues.',finalLocal:'The video is stored in this browser: the player will not find it on another device.',
    ...
    he:{stageMedia:'מדיה',stageTranscript:'תמלול',stageReview:'בדיקה',stageOf:'שלב {i} מתוך {n}',
      ...
      finalMissing:'שורות בלי ▶: {k}: הטקסט שלהן לא תאם למשפטי התמלול.',finalLocal:'הסרטון שמור בדפדפן הזה: במכשיר אחר הנגן לא ימצא אותו.',
  ...
  async function start(capturedInput){
    ...
    if(!link&&isMediaJob({input})&&operations.reviewTranscript){
      ...
      button(actions,t('reviewTranscript'),async()=>{
        const edited=title.value;d.close();
        // Отказ редактора (нет исправляемой дорожки) не рвёт процесс: задача открывается снова.
        try{await operations.reviewTranscript(input);}catch(_){}
        const fresh=operations.capture();
        await start({...fresh,title:edited||fresh.title});
      });
    }
    ...
    // Локальный медиа-материал: транскрипт уже есть, платна только таблица. Её цена — здесь, до
    // старта, и согласованный потолок едет в журнал задачи: второго окна посреди прогона нет.
    if(!link&&isMediaJob({input})&&operations.estimateTable&&input.provider==='gemini'){
      const price=element('p',t('estimating'));price.setAttribute('role','status');d.insertBefore(price,actions);
      startButton.disabled=true;
      (async()=>{
        try{
          const e=await operations.estimateTable(input);
          quoted=e&&e.table||null;expectedSec=e&&e.minutes?e.minutes*60:null;
          price.textContent=quoted?fill(t('priceTableOnly'),{rows:quoted.highRows,usd:money(quoted.highUsd),min:e.minutes||'—'}):t('estimateFailed');
        }catch(_){price.textContent=t('estimateFailed');}
