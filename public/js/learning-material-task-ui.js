(function(){
  ...
  async function showTask(job,d){
    ...
    else if(job.state!=='ready'&&(job.error!=='ASR_BLOCKED'||recoverableOther)&&job.error!=='ASR_OTHER_EXHAUSTED'&&!(job.error==='TASK_SOURCE_MISMATCH'&&job.error_reason))button(actions,t('resume'),()=>execute(job.id,d));
