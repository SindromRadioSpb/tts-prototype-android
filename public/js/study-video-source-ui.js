(function () {
  'use strict';
  const text = {
    ru:{open:'Видео и строки',source:'Источник видео',url:'Ссылка YouTube',offset:'Смещение, секунды (время видео − время таблицы)',preview:'Проверить видео и строки',confirmed:'Подтверждаю: это тот же ролик, и временная шкала соответствует локальному файлу или проверена по контрольным репликам',save:'Сохранить привязку',detach:'Отвязать YouTube',close:'Закрыть',history:'История источников',saved:'Привязка сохранена',error:'Не удалось выполнить действие. Проверьте ссылку и повторите открытие карточки.',missing:'Добавьте ссылку YouTube к этому материалу.',readOnly:'Источник опубликованного материала доступен только для чтения.',noTiming:'У таблицы нет подтверждаемой временной разметки. Видео можно сохранить без синхронизации.',note:'Полная копия того же ролика без обрезки и изменения скорости: оставьте смещение 0 — пересобирать таблицу не нужно. Если версии отличаются, сверьте начало, середину и конец в предпросмотре. Одно смещение подходит только при постоянной разнице времени. Подтверждение не исправляет неточные или отсутствующие метки.',replaced:'Без YouTube'},
    en:{open:'Video and rows',source:'Video source',url:'YouTube link',offset:'Offset, seconds (video time − table time)',preview:'Check video and rows',confirmed:'I confirm this is the same video and its timeline matches the local file or has been checked against sample lines',save:'Save video source',detach:'Detach YouTube',close:'Close',history:'Source history',saved:'Video source saved',error:'Could not complete the action. Check the link and reopen the material.',missing:'Add a YouTube link to this material.',readOnly:'Published video sources are read-only.',noTiming:'This table has no timing that can be confirmed. Video can be saved without synchronization.',note:'For an untrimmed copy of the same video at the original speed, keep offset 0; the table does not need rebuilding. If versions differ, check the start, middle and end in preview. One offset works only for a constant time difference. Confirmation does not repair inaccurate or missing timestamps.',replaced:'No YouTube'},
    he:{open:'וידאו ושורות',source:'מקור הסרטון',url:'קישור YouTube',offset:'היסט בשניות (זמן הסרטון פחות זמן הטבלה)',preview:'בדיקת הסרטון והשורות',confirmed:'אני מאשר שזה אותו סרטון ושציר הזמן תואם לקובץ המקומי או נבדק מול שורות לדוגמה',save:'שמירת מקור הסרטון',detach:'ניתוק YouTube',close:'סגירה',history:'היסטוריית מקורות',saved:'מקור הסרטון נשמר',error:'הפעולה נכשלה. בדקו את הקישור ופתחו שוב את החומר.',missing:'הוסיפו קישור YouTube לחומר הזה.',readOnly:'מקור של חומר שפורסם זמין לקריאה בלבד.',noTiming:'אין בטבלה תזמון שניתן לאמת. אפשר לשמור סרטון ללא סנכרון.',note:'לעותק מלא של אותו סרטון, ללא חיתוך או שינוי מהירות, השאירו היסט 0; אין צורך לבנות את הטבלה מחדש. אם הגרסאות שונות, בדקו את ההתחלה, האמצע והסוף בתצוגה המקדימה. היסט יחיד מתאים רק להפרש זמן קבוע. האישור אינו מתקן תזמון שגוי או חסר.',replaced:'ללא YouTube'}
  };
