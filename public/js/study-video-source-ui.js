(function () {
  ...
  const playerText={
    ru:{youtube:'YouTube-видео',local:'Локальный файл',sourceGroup:'Источник воспроизведения',pending:'YouTube · нет пригодных таймкодов. Проверьте разметку и смещение в «Источник видео».',changed:'YouTube · строки или тайминги изменились. Проверьте синхронизацию заново.',ready:'YouTube · воспроизведение по таймкодам таблицы',denied:'YouTube запретил встраивание этого ролика. Проверьте источник видео или выберите локальный файл.',failed:'YouTube недоступен. Нажмите «YouTube-видео» ещё раз или выберите локальный файл.',blocked:'Нажмите ▶ в самом плеере YouTube.',savedReady:'Привязка сохранена. Синхронизация включена.',savedLocal:'YouTube отвязан. Доступен исходный локальный файл.'},
    en:{youtube:'YouTube video',local:'Local file',sourceGroup:'Playback source',pending:'YouTube · no usable timestamps. Check timing and offset in Video source.',changed:'YouTube · rows or timing changed. Check synchronization again.',ready:'YouTube · playback using table timestamps',denied:'YouTube does not allow this video to be embedded. Check Video source or use the local file.',failed:'YouTube is unavailable. Select YouTube video again or use the local file.',blocked:'Press ▶ in the YouTube player.',savedReady:'Video source saved. Synchronization enabled.',savedLocal:'YouTube detached. The original local file is available.'},
    he:{youtube:'סרטון YouTube',local:'קובץ מקומי',sourceGroup:'מקור ההפעלה',pending:'YouTube · אין חותמות זמן תקינות. בדקו את התזמון וההיסט במקור הסרטון.',changed:'YouTube · השורות או התזמון השתנו. בדקו שוב את הסנכרון.',ready:'YouTube · הפעלה לפי חותמות הזמן בטבלה',denied:'YouTube אינו מאפשר להטמיע את הסרטון. בדקו את מקור הסרטון או בחרו בקובץ המקומי.',failed:'YouTube אינו זמין. בחרו שוב בסרטון YouTube או בקובץ המקומי.',blocked:'לחצו על ▶ בנגן YouTube.',savedReady:'מקור הסרטון נשמר. הסנכרון מופעל.',savedLocal:'YouTube נותק. הקובץ המקומי המקורי זמין.'}
  };
  ...
  async function db(){
    const ldb=window.__localDB || (typeof window.ensureLocalDB==='function'?await window.ensureLocalDB():await import('/db/local-db.js?v=528'));
