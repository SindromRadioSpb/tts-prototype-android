...
(function (root, factory) {
  ...
  function closeEvidenceDialog(){
    if(!S.evidenceDialog)return;try{var a=S.evidenceDialog._audio;if(a){a.pause();a.src="";}}catch(_){}try{if(S.evidenceDialog._esc)document.removeEventListener("keydown",S.evidenceDialog._esc);S.evidenceDialog.remove();}catch(_){}S.evidenceDialog=null;
  }
  function evidenceChannelBar(item){
    var active=item.construct_id==="READING_TO_NEW_CONTEXT_TRANSFER"?"read":"dictate";
    var bar=el("div","room-train-channels");bar.setAttribute("role","tablist");bar.setAttribute("dir",lang().indexOf("he")===0?"rtl":"ltr");
    [["read","room.morph.study.chRead","📖 Чтение"],["listen","room.morph.study.chListen","🎧 Аудио"],["reverse","room.morph.study.chReverse","🔤 RU→HE"],["dictate","room.morph.study.chDictate","✍️ Диктант"]].forEach(function(c){var on=c[0]===active,b=el("button","room-train-chseg"+(on?" on":" disabled"),t(c[1],c[2]));b.type="button";b.setAttribute("role","tab");b.setAttribute("aria-selected",String(on));if(!on){b.disabled=true;b.setAttribute("aria-disabled","true");}bar.appendChild(b);});
    return bar;
  }
  function evidenceResultCopy(verdict,decisionCode){
    var verdicts={
      CORRECT_UNASSISTED:["room.mentor.evidence.verdictCorrectUnassisted","✓ Верно — написано самостоятельно."],
      CORRECT_ASSISTED:["room.mentor.evidence.verdictCorrectAssisted","✓ Верно после подсказки."],
      NEAR_MISS:["room.mentor.evidence.verdictNearMiss","Почти верно — есть небольшое отличие."],
      INCORRECT:["room.mentor.evidence.verdictIncorrect","Пока не совпало."],
      CORRECT_NEW_CONTEXT:["room.mentor.evidence.verdictTransferCorrect","✓ Верно в новом контексте."],
      INCORRECT_NEW_CONTEXT:["room.mentor.evidence.verdictTransferIncorrect","В новом контексте пока не получилось."],
      AMBIGUOUS:["room.mentor.evidence.verdictInconclusive","Результат нельзя надёжно определить."],
      ABSTAIN:["room.mentor.evidence.verdictInconclusive","Результат нельзя надёжно определить."],
      SOURCE_DRIFT:["room.mentor.evidence.verdictInconclusive","Результат нельзя надёжно определить."],
      DISAGREEMENT:["room.mentor.evidence.verdictInconclusive","Результат нельзя надёжно определить."]
    },decisions={
      NO_EXTRA_TARGETED_PRACTICE:["room.mentor.evidence.adviceNoExtra","Дополнительная тренировка по этому слову сейчас не нужна."],
      ONE_CONTEXTUAL_RETRIEVAL_CANDIDATE:["room.mentor.evidence.adviceContext","Позже можно закрепить слово ещё в одном контексте."],
      ONE_UNSUPPORTED_RETRIEVAL_CANDIDATE:["room.mentor.evidence.adviceRetry","К этому слову стоит вернуться в одной из следующих тренировок."],
      TRANSFER_OBSERVED_ONCE_NO_ACTION:["room.mentor.evidence.adviceTransfer","Перенос замечен один раз; дополнительных действий сейчас не требуется."],
      ONE_CONTRASTIVE_CONTEXT_CANDIDATE:["room.mentor.evidence.adviceContrast","Позже можно попробовать это слово в другом контексте."],
      INCONCLUSIVE:["room.mentor.evidence.adviceInconclusive","Эта попытка сохранена без вывода о знании слова."]
    },v=verdicts[String(verdict)]||verdicts.ABSTAIN,d=decisions[String(decisionCode)]||decisions.INCONCLUSIVE;
    return {title:t(v[0],v[1]),detail:t(d[0],d[1])};
  }
  function openEvidenceDialog(item,section,box){
    if(!item)return;closeEvidenceDialog();
    var isB2=item.construct_id==="READING_TO_NEW_CONTEXT_TRANSFER",sheet=el("div","room-study room-evidence-study"),card=el("div","room-study-card"),back=el("div","room-study-backdrop"),x=el("button","room-study-x","✕"),head=el("div","room-study-head"),title=el("span","room-study-title",t("room.morph.study.title","📚 Учить новые слова")),mode=el("div","room-study-modetoggle"),train=el("button","room-study-seg on",t("room.morph.study.modeTrain","🎯 Тренировка")),body=el("div","room-study-body room-evidence-dialog-body"),foot=el("div","room-train-actions"),status=el("div","mentor-hint room-evidence-dialog-status");
    sheet.hidden=false;sheet.classList.add("room-study-open");sheet.setAttribute("role","dialog");sheet.setAttribute("aria-modal","true");sheet.setAttribute("aria-label",title.textContent);back.setAttribute("data-evidence-close","1");x.type="button";x.setAttribute("aria-label",t("room.morph.close","Закрыть"));x.setAttribute("data-evidence-close","1");train.type="button";train.setAttribute("aria-current","page");head.appendChild(title);card.appendChild(x);card.appendChild(head);mode.appendChild(train);card.appendChild(mode);card.appendChild(evidenceChannelBar(item));
    var badge=el("div","mentor-memory-badge room-evidence-dialog-badge",isB2?t("room.mentor.evidence.badgeB2","Новый публичный контекст · теневая проверка"):t("room.mentor.evidence.badgeB1","Самостоятельное написание · теневая проверка"));card.appendChild(badge);card.appendChild(el("div","mentor-hint room-evidence-dialog-boundary",t("room.mentor.evidence.dialogBoundary","Эта тренировка только проверяет доказательство и не меняет память слова, оценку или расписание.")));card.appendChild(body);card.appendChild(status);card.appendChild(foot);sheet.appendChild(back);sheet.appendChild(card);document.body.appendChild(sheet);S.evidenceDialog=sheet;
    function closeAndRefresh(){closeEvidenceDialog();return renderEvidence(section,box);}
    function fail(){status.textContent="✗ "+t("room.mentor.evidence.actionFailed","Действие не выполнено.");status.hidden=false;}
    function actionButton(label,action,ghost){var b=el("button",ghost?"room-train-skip":"room-train-next",label);b.type="button";b.addEventListener("click",async function(){b.disabled=true;try{var r=await jpost("/api/agent/evidence/"+encodeURIComponent(item.id)+"/action",{action:action});if(!r.json||!r.json.ok)throw new Error();if(action==="ACCEPT"){item=r.json.item;paint();}else await closeAndRefresh();}catch(_){fail();b.disabled=false;}});foot.appendChild(b);return b;}
    async function playAudio(button){if(button)button.disabled=true;try{var a=sheet._audio;if(!a){a=new Audio();a.preload="auto";a.src="/api/agent/evidence/"+encodeURIComponent(item.id)+"/audio";sheet._audio=a;}a.currentTime=0;await a.play();status.hidden=true;}catch(_){status.textContent="✗ "+t("room.mentor.evidence.audioFailed","Аудио не загрузилось. Проверьте соединение и попробуйте ещё раз.");status.hidden=false;}finally{if(button)button.disabled=false;}}
    function showResult(verdict,decisionCode){var copy=evidenceResultCopy(verdict,decisionCode),tone=String(verdict).indexOf("CORRECT")===0?"room-train-reveal-ok":["ABSTAIN","AMBIGUOUS","SOURCE_DRIFT","DISAGREEMENT"].indexOf(String(verdict))>=0?"room-train-reveal-skip":"room-train-reveal-bad";body.textContent="";foot.textContent="";body.appendChild(el("div","room-train-reveal "+tone+" room-evidence-result-panel",copy.title));body.appendChild(el("div","mentor-hint room-evidence-result-advice",copy.detail));var done=el("button","room-train-next",t("room.mentor.evidence.done","Готово"));done.type="button";done.addEventListener("click",closeAndRefresh);foot.appendChild(done);}
    function paint(){body.textContent="";foot.textContent="";status.hidden=true;
  ...
  async function renderEvidence(section,box){
    ...
    var items=(probe.json&&probe.json.items)||[],resume=items.find(function(x){return x.state==="OFFERED"||x.state==="ACCEPTED";});
    var find=el("button","mentor-plan-btn",resume?t("room.mentor.evidence.continue","Продолжить"):t("room.mentor.evidence.find","Найти небольшую проверку"));find.type="button";find.disabled=!(b1||b2);find.addEventListener("click",async function(){find.disabled=true;try{if(resume){openEvidenceDialog(resume,section,box);return;}var r=await jpost("/api/agent/evidence/scan",{});if(!r.json||!r.json.ok)throw new Error(r.json&&r.json.error);var offered=await jget("/api/agent/evidence/offer");if(!offered.json||!offered.json.ok||!offered.json.item)throw new Error("NO_OFFER");await renderEvidence(section,box);openEvidenceDialog(offered.json.item,section,box);}catch(e){msg.textContent=t("room.mentor.evidence.none","Подходящей однозначной проверки сейчас нет.")+" "+String(e&&e.message||"");msg.hidden=false;}finally{find.disabled=false;}});box.appendChild(find);
    if(!items.length)box.appendChild(el("div","mentor-memory-empty",t("room.mentor.evidence.empty","Здесь пока нет проверок.")));
    ...
    items.forEach(function(item){var card=el("article","mentor-memory-card mentor-evidence-card"),isB2=item.construct_id==="READING_TO_NEW_CONTEXT_TRANSFER";card.appendChild(el("div","mentor-memory-badge",isB2?t("room.mentor.evidence.badgeB2","Новый публичный контекст · теневая проверка"):t("room.mentor.evidence.badgeB1","Самостоятельное написание · теневая проверка")));if(item.authority_class==="SELF_REPORTED_RETRIEVAL")card.appendChild(el("div","mentor-hint",t("room.mentor.evidence.selfReportedBadge","Источник наблюдения: самоотчёт в Читальном зале")));var stim=item.stimulus||{};if(stim.sentence)card.appendChild(el("div","mentor-evidence-stimulus",stim.sentence));var actions=el("div","mentor-plan-actions");function button(label,action,ghost){var b=el("button",ghost?"mentor-wr-ghost":"mentor-plan-btn",label);b.type="button";b.addEventListener("click",async function(){b.disabled=true;try{await act(item,action);}catch(_){msg.textContent=t("room.mentor.evidence.actionFailed","Действие не выполнено.");msg.hidden=false;}finally{b.disabled=false;}});actions.appendChild(b);}
      if(item.state==="OFFERED"||item.state==="ACCEPTED"){var open=el("button","mentor-plan-btn",item.state==="OFFERED"?t("room.mentor.evidence.accept","Начать"):t("room.mentor.evidence.continue","Продолжить"));open.type="button";open.addEventListener("click",function(){openEvidenceDialog(item,section,box);});actions.appendChild(open);if(item.state==="OFFERED"){button(t("room.mentor.evidence.later","Позже"),"DEFER",true);button(t("room.mentor.evidence.skip","Пропустить"),"SKIP",true);button(t("room.mentor.evidence.suppress","Не предлагать"),"SUPPRESS",true);}}
      if(item.evaluation){var copy=evidenceResultCopy(item.evaluation.verdict,item.decision&&item.decision.decision_code);card.appendChild(el("div","mentor-evidence-result",copy.title));card.appendChild(el("div","mentor-hint",copy.detail));if(item.evaluation.status==="VALID"){button(t("room.mentor.evidence.dispute","Оспорить"),"DISPUTE",true);button(t("room.mentor.evidence.annul","Аннулировать"),"ANNUL",true);}}
