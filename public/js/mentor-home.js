...
(function (root, factory) {
  ...
  function openEvidenceDialog(item,section,box){
    ...
    async function playAudio(){try{var a=sheet._audio;if(!a){a=new Audio();a.preload="auto";a.src="/api/agent/evidence/"+encodeURIComponent(item.id)+"/audio";sheet._audio=a;}a.currentTime=0;await a.play();status.hidden=true;}catch(_){status.textContent="✗ "+t("room.mentor.evidence.audioFailed","Не удалось воспроизвести проверочное аудио.");status.hidden=false;}}
    function paint(){body.textContent="";foot.textContent="";status.hidden=true;
      ...
      var stim=item.stimulus||{},input;
      if(isB2){if(stim.sentence)body.appendChild(el("div","mentor-evidence-stimulus",stim.sentence));input=document.createElement("select");input.className="mentor-memory-select room-evidence-answer";(stim.options||[]).forEach(function(o){var op=document.createElement("option");op.value=o.id;op.textContent=o.surface;input.appendChild(op);});body.appendChild(input);}else{var listen=el("button","room-evidence-listen",t("room.mentor.evidence.playAudio","▶ Прослушать слово"));listen.type="button";listen.addEventListener("click",playAudio);body.appendChild(listen);input=document.createElement("input");input.className="mentor-memory-input room-evidence-answer";input.maxLength=512;input.autocomplete="off";input.dir="rtl";input.placeholder=t("room.mentor.evidence.answer","Введите услышанное слово…");body.appendChild(input);}
      var submit=el("button","room-train-next",t("room.mentor.evidence.submit","Проверить"));submit.type="button";submit.addEventListener("click",async function(){submit.disabled=true;try{var payload=isB2?{option_id:input.value,input_mode:"closed_option"}:{answer:input.value,input_mode:"keyboard"},r=await jpost("/api/agent/evidence/"+encodeURIComponent(item.id)+"/attempt",payload);if(!r.json||!r.json.ok)throw new Error();body.textContent="";foot.textContent="";body.appendChild(el("div","mentor-evidence-result",t("room.mentor.evidence.result","Детерминированная оценка")+": "+r.json.verdict));body.appendChild(el("div","mentor-hint",t("room.mentor.evidence.shadow","Теневой совет, не действие планировщика")+": "+r.json.decision_code));var done=el("button","room-train-next",t("room.mentor.evidence.done","Готово"));done.type="button";done.addEventListener("click",closeAndRefresh);foot.appendChild(done);}catch(_){fail();submit.disabled=false;}});foot.appendChild(submit);actionButton(t("room.mentor.evidence.skip","Пропустить"),"SKIP",true);
    }
