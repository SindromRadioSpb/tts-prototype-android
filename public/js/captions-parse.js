...
(function () {
  ...
  // W2-S12 T6: 3 ч × ~14 репл/мин + запас; таблица режется чанками ≤120 — серверный кап 400
  // на запрос не задевается (S12). Больше НЕ зеркалит ingest/segTable.js (тот держит per-request
  // кап 400 — сервер не менялся, "zero-server-growth"); это кап на ВЕСЬ вставленный транскрипт
  // до клиентского чанкования, соответствующий тест обновлён.
  var MAX_SEGMENTS = 2800;
  ...
  // пропускает MAX_SEGMENTS/MAX_SEG_TEXT — сырой разбор ничего не отправляет, капам там нечего
  // защищать. Плотный i и CAPTIONS_EMPTY на нуле реплик — одинаковы в обоих режимах. Продуктовый
  // путь (merge:true) капы получает как раньше, ПОСЛЕ слияния. Обход капа этим не открывается:
  // и клиент (здесь), и сервер проверяют размер независимо.
  // W2-S12 update: MAX_SEGMENTS больше НЕ зеркалит серверный per-request кап ingest/segTable.js —
  // это клиентский потолок ОБЩЕГО числа реплик вставленного транскрипта (3ч, см. константу выше);
  // нарезку под серверный кап ≤400 сегментов/запрос делает TableChunks (public/js/table-chunks.js)
  // ПОСЛЕ этого разбора. MAX_SEG_TEXT — по-прежнему прямое зеркало серверного капа на один сегмент.
