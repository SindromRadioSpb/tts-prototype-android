...
(function (root, factory) {
  ...
  async function renderHistory(modal,state){
    ...
    const cards=receipts.map(item=>{
      ...
      const recovery=lifecycle==='archived'?`<button type="button" class="p2-recovery-action" data-action="restore-library" data-receipt="${esc(item.receipt_id)}">${tr('studio.portable.restoreCard','Return card to Library')}</button>`:lifecycle==='repairable'?`<button type="button" class="p2-recovery-action" data-action="choose-repair-package" data-receipt="${esc(item.receipt_id)}">${tr('studio.portable.chooseRepairPackage','Choose Source package and restore')}</button>`:'';
