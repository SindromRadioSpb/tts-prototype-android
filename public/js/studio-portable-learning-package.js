...
(function (root, factory) {
  ...
  async function renderImport(modal,state){
    ...
    const status=body.querySelector('#p2PortableStatus'),file=body.querySelector('#p2PortableFile'),relink=body.querySelector('#p2PortableRelink');actions.onclick=event=>{if(event.target&&event.target.dataset.act==='import'){file.value='';file.click();}};
    file.onchange=async()=>{
      ...
      try{
        ...
        const mediaState=plan.media.status==='exact'?tr('studio.portable.mediaExact','Медиа уже есть на устройстве'):plan.media.status==='missing'?tr('studio.portable.mediaMissing','Медиа нужно связать после импорта'):tr('studio.portable.mediaUnbound','Пакет без медиа');
        status.innerHTML=`<b>${plan.can_apply?tr('studio.portable.verified','Verified — ready to apply'):tr('studio.portable.blocked','Blocked')}</b><div class="p2-plan-grid"><span><strong>${Number(plan.estimated.cue_count)}</strong>${tr('studio.portable.cues','реплик транскрипта')}</span><span><strong>${Number(plan.estimated.row_count)}</strong>${tr('studio.portable.currentRows','строк текущей таблицы')}</span><span><strong>${Number(plan.estimated.caption_revision_count)}</strong>${tr('studio.portable.captionVersions','версий транскрипта')}</span><span><strong>${Number(plan.estimated.table_revision_count)}</strong>${tr('studio.portable.tableVersions','версий таблицы')}</span></div><div class="p2-media-summary"><b>${esc(mediaState)}</b><span>${esc(plan.media.original_name||tr('studio.portable.unnamedMedia','медиафайл'))} · ${esc(formatBytes(plan.media.size_bytes))} · SHA ${esc(shortSha(plan.media.sha256))}</span></div><span>${Number(plan.conflicts.length)} ${tr('studio.portable.conflicts','conflicts')}</span>`;
        if(plan.can_apply){const apply=document.createElement('button');apply.type='button';apply.className='p2-portable-apply';apply.textContent=tr('studio.portable.apply','Apply verified package');apply.onclick=async()=>{apply.disabled=true;status.textContent=tr('studio.portable.applying','Applying in one local transaction…');try{const result=await applyPending(plan.plan_sha256);status.classList.remove('p2-portable-error');status.textContent=result.duplicate?tr('studio.portable.reused','Already present — zero duplicates.'):tr('studio.portable.applied','Imported locally. Durable receipt saved.');if(plan.media.status==='missing'){const button=document.createElement('button');button.type='button';button.className='p2-portable-apply';button.textContent=tr('studio.portable.relink','Relink exact media file');button.onclick=()=>{relink.value='';relink.dataset.receipt=result.receipt.receipt_id;relink.click();};status.appendChild(document.createElement('br'));status.appendChild(button);}}catch(error){status.classList.add('p2-portable-error');status.textContent=formatPortableError(error,selected,plan.media);apply.disabled=false;}};status.appendChild(document.createElement('br'));status.appendChild(apply);}
  ...
  async function renderHistory(modal,state){
    ...
    body.innerHTML=`<section class="p2-receipt-section"><div class="p2-receipt-intro"><span class="p2-scope-eyebrow">${tr('studio.portable.localHistory','LOCAL HISTORY')}</span><h4>${tr('studio.portable.historyTitle','Import history')}</h4><p>${tr('studio.portable.historyHelp','Undo first checks reverse references. It never removes reused objects or media bytes and stops on any unknown reference.')}</p></div><div id="p2ReceiptList">${receipts.length?receipts.map(item=>{const media=item._media,mediaLine=media?(media.media_available?tr('studio.portable.mediaConnected','Медиа связано'):tr('studio.portable.mediaMissingShort','Медиа отсутствует'))+' · '+(media.original_name||tr('studio.portable.unnamedMedia','медиафайл'))+' · SHA '+shortSha(media.media_sha256):tr('studio.portable.mediaNotRequired','Медиа не заявлено');return `<article class="p2-receipt-card" data-receipt-card="${esc(item.receipt_id)}"><div><b>${esc(item.title||tr('studio.portable.importedMaterial','Imported learning material'))}</b><span>${esc(item.package_mode)} · ${esc(formatDate(item.created_at))}</span><code>${esc(String(item.content_root_sha256||'').slice(0,16))}…</code><span class="p2-receipt-media" data-available="${media&&media.media_available?'true':'false'}">${esc(mediaLine)}</span></div><span class="p2-receipt-state" data-state="${esc(item.status)}">${item.status==='committed'?tr('studio.portable.committed','Active'):tr('studio.portable.rolledBack','Undone')}</span>${item.status==='committed'&&media&&!media.media_available?`<button type="button" class="p2-media-relink" data-action="relink-media" data-receipt="${esc(item.receipt_id)}">${tr('studio.portable.relink','Relink exact media file')}</button>`:''}${item.status==='committed'?`<button type="button" data-action="inspect-undo" data-receipt="${esc(item.receipt_id)}">${tr('studio.portable.checkUndo','Check Undo')}</button>`:''}<p class="p2-receipt-result" aria-live="polite"></p></article>`;}).join(''):`<div class="p2-portable-status">${tr('studio.portable.noReceipts','No imported learning packages yet.')}</div>`}</div><input id="p2HistoryRelink" type="file" accept="audio/*,video/*" hidden></section>`;
