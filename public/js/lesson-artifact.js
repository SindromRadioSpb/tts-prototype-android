...
(function (root, factory) {
  ...
  var POLICY_VERSION = "lesson-builder-lb0-v1";
  ...
  function validSourceRefs(refs) {
    ...
    return refs.every(function (r) {
      ...
      return Number.isInteger(Number(r.start_order_index)) && Number(r.start_order_index) >= 0 &&
        Number.isInteger(Number(r.row_count)) && Number(r.row_count) >= 1 && Number(r.row_count) <= 40;
    });
  }
  function validate(draft) {
    if (!draft || draft.schemaVersion !== SCHEMA_VERSION || draft.policyVersion !== POLICY_VERSION ||
    ...
    var exercises = (Array.isArray(draft.exercises) ? draft.exercises : []).slice(0, MAX_EXERCISES).map(function (e) {
      return { type: str(e && e.type, 40), instruction: str(e && e.instruction, 600),
