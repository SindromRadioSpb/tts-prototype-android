...
- **A2 — CLIENT-side export (owner chose client build). Resolved open question:** SRS cards live in **OPFS** (`public/db/local-db.js` `srs.listCards`/`createCard`/`createCardFromNote`); the **server has NO user data** (OPFS-first; `texts` empty in prod, `server.js:342`); precedent `exportBundle` (local-db.js:3534) is client-side; wa-sqlite has no serialize → client uses **sql.js**.
  ...
  - **A2b NEXT — wiring:** build the 17 «LinguistPro SRS Card v1» fields CLIENT-side from OPFS (`srs.listCards` + linked `notes_v2` body_json word/niqqud/root/binyan/meaning/pealim_id/context + sentences) — replicate the field mapping from server `buildSrsAnkiPreview` (server.js ~5056–5074) but read OPFS not server DB. Then «📦 Скачать .apkg» button in Studio Trainer (`public/check_script.js` `v3SrsTrainer*` ~8937–9200) → `AnkiApkg.downloadApkg(spec, name)`. Load `/db/anki-apkg-core.js`+`/db/anki-apkg.js` in index.html; **bump SW** (index.html shell changes). Browser-verify @380px; re-import-updates-not-dupes = owner device-smoke. Model name+fields+templates+css must MIRROR server `getSrsAnkiModelSpec` (server.js:4910) so .apkg ≡ AnkiConnect cards.
