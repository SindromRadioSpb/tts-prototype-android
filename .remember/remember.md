...
## STATE — main HEAD `b2294b1`(+this closure commit), SW `v3.10.82-export-perf-p` (no code change this session)
...
## Опц. будущее (НЕ T3, по слову владельца)
- **P2 — скорость фетча** (настоящая боль 16 мин): batch-эндпоинт (POST список ключей → один tar/multipart-ответ), убирает N round-trips. Дизайн до кода. Деградация темпа под нагрузкой стоит проверить.
- **Лёгкий retry** для ~0.5% клипов, что уходят в `missing_audio.json` на длинных прогонах (сейчас один проход, 8с таймаут, без повтора).
