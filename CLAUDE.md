...
Несмотря на имя, это **Node.js-приложение** (PWA + сервер), не нативный Android. v3.11.256 (прод). Google Cloud Text-to-Speech + Gemini, билингвальные таблицы иврит↔русский, морфология, квизы.
...
- **Импорт контента в Студию** (Wave 1 SHIPPED v3.11.242; Wave 2: S4 аудио/видео→сегмент-караоке v3.11.246-248, S5a субтитры+YouTube-плеер v3.11.250-253, S12 длинные медиа v3.11.256): диалог «Импорт» (`#v3ImportModal`, `public/js/studio-import.js`, три вкладки Статья/Видео/Файл) принимает URL/файл/фото/аудио/видео/субтитры → текст+сегменты в поле ввода; серверные модули `ingest/` (S1 fetch-url анти-SSRF, S2 docx + S8 фото/PDF BYOK Gemini OCR); аудио/видео — браузер→Gemini Files API (BYOK, `gemini-files.js`), **потолок 3 часа**: чанк-ASR окнами 15 мин (`runWindowedAsr`, range-промт, coverage-валидация+добор дыр) + чанк-таблица по 120 сегментов (`table-chunks.js`, цикл `v3TranslateTableChunked` в index.html, прогрессивная отрисовка, серверный кэш куска = резюм) — стена одного вызова ≈287 строк (65к out-ток), замеры `docs/research/studio-ingest-longmedia/2026-07-28/`. Сегмент-караоке `studio-media-karaoke.js`; обратное направление `direction=any-he`. Гейты: `smoke:ingest`, `smoke:studio-chunks`, `smoke:captions-parse`, live `ingest-longmedia-live-smoke.js`. Канон: `docs/planning/STUDIO_INGEST_MULTIMODAL_DECISION_PACKET_2026_07_25.md` (§4/§7 — живой статус слайсов).
