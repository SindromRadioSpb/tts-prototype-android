...
Несмотря на имя, это **Node.js-приложение** (PWA + сервер), не нативный Android. v3.11.181 (прод). Google Cloud Text-to-Speech + Gemini, билингвальные таблицы иврит↔русский, морфология, квизы.
...
- **Импорт контента в Студию** (Wave 1 SHIPPED v3.11.242, прод-верифицировано): диалог «Импорт» после поля ввода (`#v3ImportModal`, `public/js/studio-import.js`) принимает URL/файл/фото → текст в поле ввода, через серверные модули `ingest/` (`geminiKey.js`, `ssrfGuard.js`, `urlExtract.js`, `docxExtract.js`, `routes.js`: S1 fetch-url с анти-SSRF, S2 docx детерминированно + S8 фото/PDF через BYOK Gemini OCR). Плюс обратное направление перевода (любой язык → иврит-таблица) — параметр `direction` в `/api/translate-table` + селектор `#tableDirectionSelect`. Канон: `docs/planning/STUDIO_INGEST_MULTIMODAL_DECISION_PACKET_2026_07_25.md` + `docs/planning/STUDIO_INGEST_W1_IMPLEMENTATION_PLAN_2026_07_25.md`.
