...
### 3.4.1. Library / Gemini / TTS anti-regeneration
Триггер:
- изменения в `AI Перевод+SAVE`
- изменения в `playTTS` / row-TTS
- изменения в `text_audio` / `sentence_audio` reuse

Ручной regression:
- открыть текст из `Library`
- не меняя исходный текст, нажать `AI Перевод+SAVE`
- ожидаемо:
  - таблица восстанавливается из `Library`
  - не идёт новый запрос в Gemini
  - статус не должен говорить `сгенерирована через Gemini API`

- для того же текста нажать `Озвучить` дважды с теми же `voice/rate/pitch`
- ожидаемо:
  - при повторе воспроизводится уже сохранённый audio asset
  - не нужен новый запрос в TTS provider

- для строки из `Library` нажать `▶` дважды с теми же `voice/rate/pitch`
- ожидаемо:
  - при повторе играет `/api/audio/:assetKey`
  - не создаётся новый `sentence_audio` default

- изменить хотя бы один из параметров `voice/rate/pitch`
- ожидаемо:
  - reuse прекращается
  - создаётся новый asset для нового профиля
