...
- [~] **Direction 5** — SRS + Library smart-sort — *partially complete*. **Done:** activity heatmap в Dashboard (GitHub-style 30-day grid с 4-band quantile colour scale, hover tooltips, log-scale binning); foundation helpers в local-db: `getActivityHeatmap`, `getStrugglingTexts`, `getMasteredTexts`, `getTextsCreatedAfter`. **Pending:** UI-чипы в Library для smart-фильтров (struggling/mastered/new-since-last-visit) — требует более глубокой интеграции с существующим Library-render flow; отложено на сессию 4 (i18n) или отдельную короткую сессию.
