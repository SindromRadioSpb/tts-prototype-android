...
- [x] **Direction 3** — Full i18n coverage — *complete*. **Phase 1** (Сессия 4): smart-chip strings + 8 high-traffic toasts мигрированы (textPasted, clipboardEmpty/Unavailable/ReadFailed, textArchived, archiveFailed, textDeleted, restored). **Phase 2** (Сессия 5, 2026-05-09): остальные ~120 hardcoded `showToast("…")` callsites (плейлист, аудио ошибки, импорт/экспорт edge-cases, Anki, GCP/TTS keys, DOCX, notes, offline, ZIP-bundle progress) мигрированы на `t("toast.*")`. Добавлено ~120 новых ключей в `toast.*` namespace всех 3 локалей (ru/en/he); параметрические сообщения через `{error}`, `{count}`, `{done}/{total}` etc.; составные сообщения (deleted/archived + failedSuffix) переписаны через композицию `t() + (cond ? t() : "")`. **Verification:** `grep -cE 'showToast\(["` + "`" + `]' public/index.html` = 0. Сделано до Direction 7 (Performance/PWA), как было запланировано — структурный рефактор не должен ломать механический i18n-долг.
...
- [x] **Direction 7** — Performance / PWA — *complete (Сессия 5, 2026-05-10)*. Phased ship:
  - **Phase A — Foundation:** killed vestigial `fonts.googleapis.com` `<link>` из `<head>` (Direction 1 self-hosted woff2 сделал его dead code); добавлен `<link rel="preload">` для primary Hebrew font; `manifest.json` с id/scope/start_url/standalone display, тремя shortcuts (Library / SRS / Dashboard); набор иконок — vector SVG + 192/512/512-maskable/180/32 PNG, hand-drawn LP monogram на slate-900 с blue accent bar, генерируется pure-Node скриптом `scripts/generate-pwa-icons.js` (без external deps, re-runnable через `npm run pwa:icons`); PWA meta tags (theme-color light/dark, apple-touch-icon, apple-mobile-web-app-* trio, application-name, description); tiered `Cache-Control` на статику в `server.js` — fonts/icons immutable (1y), JS modules must-revalidate (1d), shell entry points no-cache.
  - **Phase B — Bundle hygiene:** lazy-load JSZip (~95 KB сэкономлено на cold start для юзеров, которые не делают экспорт). Новый идемпотентный helper `v3LoadJSZip()` зеркалит существующий паттерн `v3FbLoadQr()` (qrcode.js уже был lazy). Sherpa adapter (~13.7 KB) deferred в v3.2 из-за чувствительности TTS startup-sequence.
  - **Phase C — Service Worker:** `public/sw.js` с тремя стратегиями — precache (install) для полного app shell + i18n + DB layer + TTS layer + fonts + icons; stale-while-revalidate (runtime) для всего остального same-origin; network-first с timeout 2.5s + cache fallback для `/api/client-config`; network-only для всех остальных `/api/*`. Premium update UX: новый SW устанавливается в `waiting` и показывает "Доступно обновление" toast с кнопками "Обновить" / "Позже" (зеркало `v3UndoToast`); user-initiated `skipWaiting` + `controllerchange` reload. Cache invalidation: versioned cache names (`linguistpro-precache-v3.1.0-pwa-1`), purge на activate, `clients.claim()`. Регистрация на `load` чтобы install не конкурировал с critical fetches. Новые i18n ключи `app.updateAvailable / updateNow / updateLater / offlineReady` в ru/en/he.
  - **Phase D — Polish:** добавлены `<link rel="modulepreload">` hints для `sqlite-api.js` / `local-db.js` / `i18n/index.js` чтобы они грузились параллельно с HTML parsing; написан `docs/PWA.md` (install, offline, update lifecycle, troubleshooting, cache versioning, icon regeneration, deferred items).
  - **Deferred в v3.2 (явно):** functional code-split (Dashboard / SRS / IDE → отдельные dynamic-import модули) — требует extraction inline `<script>` блоков из 30k-line монолита в ES-модули с явными imports/exports вместо `window.*` глобалов. Out of scope для v3.1.0 чтобы релиз остался стабильным; v3.1.0 шипает PWA как **продукт** (install, offline, fast), а не как архитектурный refactor монолита.
