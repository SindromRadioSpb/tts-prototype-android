...
### 4.3 Calibrated diagnostic quiz *(v3.3.5 Direction 13)*

Третий путь — **in-app diagnostic quiz** (20-item Hebrew Rasch instrument). Студент в приложении: 📊 → "📝 Сдать диагностику" → отвечает на 20 вопросов с прогресс-навигацией ← / →.

Это POST'ит payload c:
```json
"metrics": { "outcome": {
...
**Hard privacy invariant.** Ответы на каждый вопрос (`Q01`..`Q20`, `responses_transient`) НЕ покидают устройство. Серверу уходит только агрегированный балл + измерительная неопределённость (`quiz_se`). Item-level pinning через `scripts/quiz/privacy-smoke.js` (5 cases) + `scripts/quiz/client-submit-smoke.js` case 8 (ITEM_LEVEL_LEAK reject).

**Authority hierarchy:** teacher CSV всё ещё authoritative для `post_test_score`/`pre_test_score`. Quiz fields (`quiz_score_normalized`, `quiz_cefr_band`, `quiz_se`) **сохраняются на merge** — dashboard показывает обе колонки рядом. То есть когда у студента есть и teacher CSV row, и quiz row — `post=88` (учитель) и `quiz=42, CEFR=B1, SE=0.55` (диагностика) видны одновременно.

**Reverse mapping (для статанализа):** `theta_estimate = (raw_score / 100 * 6.0) - 3.0` (Rasch latent ability ∈ [-3, +3]).

**Сброс одной попытки.** Если студент хочет пересдать ИЛИ ты убираешь тестовую строку:
```bash
RESEARCH_DATA_DIR=research-data \
  node scripts/research/reset_quiz_for_student.js \
  --cohort COHORT-CODE --student-id <UUID> --reason "Student request: retake"
```
Стрипает `quiz_*` поля из jsonl и пишет audit-line в `<cohort>/quiz_reset_audit.log` (`ISO ts + cohort + student_id + reset_count + reason`). Идемпотентно: второй запуск даст `reset_count=0` и не тронет файлы.

**Recalibration trigger:** при ≥30 студентах прошедших диагностику в когорте — экспортируй `cohort_<code>_aggregates.csv`, прогоняй через `scripts/quiz/recalibrate-from-data.R` (deferred to v3.4), обновляй `difficulty_logit` в `public/quiz/ulpan_diagnostic_v1.json`, bump-ай `instrument_id` до `ulpan_diagnostic_v2`.
...
### 5.3 CSV export — три вида (single-cohort) + кросс-когортный CSV
...
| ⬇ Cross-cohort CSV *(v3.3.2)* | `cross_cohort_aggregates_<date>.csv` | cohort-wide per-day rows для всех загруженных когорт (см. §5.4) |

Schema column-by-column → `RESEARCH_METRICS_SCHEMA.md` §12.

### 5.4 Multicohort mode *(v3.3.2)*

Dashboard поддерживает несколько cohort credentials одновременно. Полезно когда ulpan-курс делится на несколько групп (ULPAN-A, ULPAN-B, …) и хочется сравнить их без logout/login между ними.
...
ULPAN-C-W2026  mno789pqr...
```

Каждая строка: `CODE  TOKEN` через пробел или таб. Validation per-cohort через concurrent `fetchAggregates` — успешные попадают в `localStorage.teacherDashCohorts_v2` (JSON array), failed (401/403/404) показывают error-summary с указанием неудачных кодов.

**Chip strip (в шапке после login):**

- Каждая когорта = один chip с monospace кодом + dot статуса (green=ok, red=error, grey=unknown) + `×` для удаления.
...
- Side-by-side per-cohort summary tiles с color-keyed left border (deterministic hash per cohort code).
- Overlaid SVG charts (engagement timeline, audio playback, SRS reviews) — одна линия на когорту + legend.
- **Per-student / outcome correlations / scatter sections HIDDEN** независимо от k-status любой когорты (privacy invariant из `PHASE_PLAN_v3_3_2.md §7` — предотвращает cross-cohort re-identification).
- `⬇ Cross-cohort CSV` button становится видимым.

**`cross_cohort_aggregates_<date>.csv` schema** (long-format, sorted by cohort_code ASC + date ASC):

```
cohort_code,date,students_active,cohort_size_total,k_anonymity_met,
active_minutes_real,audio_play_ms_total,sessions_count,cards_reviewed,
notes_created,cohort_meta_consent_version,cohort_meta_retention_until
```
...
library(dplyr)
df %>% group_by(cohort_code) %>%
  summarise(total_min = sum(active_minutes_real),
            avg_active_students = mean(students_active))
```

**Storage:** `teacherDashCohorts_v2` (JSON массив `{code, token, added_at, last_ok_at, nickname?}`) + `teacherDashActiveView_v2` (cohort code | `'ALL'`). Legacy v1 keys (`teacherDashCohort_v1`/`teacherDashToken_v1`) auto-мигрируют на первом v3.3.2 boot и удаляются.

**Out of scope для v3.3.2** (deferred to v3.4 per master plan D3 + D12):
- Cross-cohort student moves
- Cohort merge
- Researcher-token rotation UI (CLI shipped в v3.3.1 A2 — `npm run research:rotate`)

---

## 6. Cross-text "Где встречается" hub *(v3.3.2)*

При работе с word-study заметкой в любом тексте можно за один клик увидеть, в каких ещё текстах библиотеки встречается это слово или его корень.

**Триггер:** кнопка `🔎 Где встречается` в `word_study` notes template (рядом с полями word + root).

**Что показывает (side-panel из правого края, full-screen на mobile):**

- Header: запрошенное слово (RTL); под ним badge с root + binyan если морфология их разрешила.
- Toggle `Включая другие инфлексии корня` — если включён, дополнительно подтягивает occurrences того же корня (через `MorphProvider.analyze()` → root → cross-text index).
- Per-text collapsible groups (`<details>`) — первые 3 открыты по умолчанию. Каждое предложение с `<mark>` на матче + sentence number.
- Click sentence → закрывает панель + открывает текст в classic view + scroll к этому sentence (через существующий `v3LibraryOpenText(textId, {resumeSentenceId})`).

**Privacy invariants (per `PHASE_PLAN_v3_3_2.md §12`):**

- **Все запросы локальные.** Никаких `fetch()` в `crosstext.js` или `crosstext-ui.js`.
- **Никакая телеметрия по словам.** Ни один lookup не эмитит event в `events` table. `MorphProvider.analyze()` уже respects этот invariant с v3.2.0.
- **Никакие новые collected fields.** Cross-text работает на существующих `sentences` и `texts` записях.

> **Карта знаний (v3.3.6, Direction 14) — НЕ researcher-facing.** Это локальная read-only визуализация связей заметок/текстов/корней. Ноль `fetch`/XHR/`sendBeacon`, ноль записей в `events`, ноль мутаций research-payload, никаких новых endpoint'ов и полей `metrics.outcome`, `CONSENT_VERSION` без изменений (пинируется `notes-graph/privacy-smoke.js`, 8 кейсов). Навигация по графу НЕ телеметрируется. Дипломному исследованию граф данных не поставляет; ручной NVDA/VoiceOver/мобильный аудит — рекомендация перед реальным развёртыванием, не блокер релиза.

**Performance:**

- Inverted index Map<normalized_key, IndexEntry[]> строится lazy на первый запрос (≤600ms для library ~100 текстов).
- Cached в памяти до visibility-change idle > 5 min OR explicit `CrossText.invalidate()`.
- Cache hit на повторный lookup того же слова: ≤30ms.
- `MorphNormalize.normalizeHebrew()` — единственная точка нормализации (final letters + niqqud strip + ZWJ + NFC). Niqqud-insensitivity получается даром.

**API surface** (для разработчиков):
- `window.CrossText.findOccurrences(word, opts)` → `Promise<Occurrence[]>`
- `window.CrossText.invalidate()` / `window.CrossText.getStats()`
- `window.LinguistProCrossTextUI.openForCurrentWord()` / `.open({word, root, ...})` / `.close()`

Smoke runner: `npm run smoke:crosstext` (13 cases — 8 service + 5 UI).

---

## 7. Statistical analysis в R / Python / SPSS
...
## 8. Withdrawal — что делать когда студент отзывает
...
## 9. Operational checks (перед deployment в реальной группе)
...
## 10. Что не входит (defer to v3.4+)
...
**Last updated:** 2026-05-16 (v3.3.6 — Direction 14 Knowledge Graph shipped: local read-only, not researcher-facing, no telemetry/endpoints/collected-fields, `CONSENT_VERSION` unchanged. v3.3.5 ulpan pre-deployment gate still open for real-cohort use of `ulpan_diagnostic_v1`).
