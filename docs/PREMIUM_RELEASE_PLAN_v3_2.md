...
- [~] **Direction 9** — Premium Notes Redesign *(Phase 9.1 complete + deployed to Railway prod 2026-05-11; Phase 9.2 next)*
  ...
  - [x] Phase 9.3 — Linking + Templates + SRS micro-cards *(branch `phase-9-3-templates-links-srs`, 2026-05-11)*. M3 templates (4 forms with required-field validation), M4 bidirectional links + backlinks (collapsible panel + atomic API), M6 note→SRS conversion (migration 026 seeds 4 note-card templates + `srs.createCardFromNote`). **57/57** + 23/23 + 0 JS errors. Trainer-side rendering of `card_kind='note'` originally deferred to v3.3 but PROMOTED into Phase 9.3.5 as Workstream D (broken-promise closure).
  - [~] Phase 9.3.5 — **Foundation Reinforcement** *(branch `phase-9-3-5-foundation-reinforcement`, 2026-05-11, ~5–7 days)*. Closes 6 product gaps surfaced by post-9.3 strategic retrospective before launching Phase 9.4 (which would otherwise add morphology on top of unstable UX). Workstreams: **B** multi-note row panel, **D** SRS note-card trainer renderer, **A** target × type guidance matrix, **C** 7 new privacy-safe events (research analytics depth), **E** cross-text "Где встречается" hub (light MVP), **F** word-token disambiguation (Phase 9.4 prereq). See `docs/research/9_3_5_STRATEGIC_REVIEW.md`.
