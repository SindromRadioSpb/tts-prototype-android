...
|------|--------|-------------|--------------|
| 0 — Инфраструктура | ✅ Done | `public/db/`: wa-sqlite (sync `wa-sqlite.mjs` + async Asyncify `wa-sqlite-async.mjs` builds), **VFS fallback chain в db-worker.js: AccessHandlePoolVFS → IDBBatchAtomicVFS** (премиум кросс-устройство — работает на iPhone iOS 17+/Android Chrome modern через OPFS sync handles; на iOS 15+/Android Chrome 80+/любых старых десктопах через IndexedDB+WebLocks). sqlite-api, WebLocks, IDBContext, migrations (19 шт.), local-db API shim со sticky VFS preference в localStorage (browser-upgrade не «осиротит» существующие данные), jszip. Тестовая страница `/db/db-init-test.html` — **14 проверок** (init, схема, CRUD текста/предложения/заметки/прогресса/события, OPFS persistence, save-as-new flow, reorder UNIQUE-конфликта, audio-asset link survival, аналитика, exportBundle unified format, round-trip import). COOP/COEP заголовки в server.js. | — |
...
| 6 — Default-on + cleanup | ✅ Done (2026-05-08) | Документация актуализирована. **One-time migration helper**: кнопка «Импорт из облака» в Library toolbar (всегда видна в LOCAL_MODE) — выгружает данные с Railway через `/api/library/export` и применяет через `importBundle` (idempotent, mode=skip). **Card-level TTS profile auto-apply** при открытии карточки + cache-first playback (см. строку «UX»). **Mobile-ready VFS chain** (см. строку 0). **A3 cross-device ZIP roundtrip** (2026-05-07): Android v2 strict-schema (`kotlinx.serialization`) compliance — `audio_assets.language` defaults to `'he-IL'`, `audio_assets.content_hash` emitted explicitly, `texts.created_at`/`updated_at` fall back to export ts, `tags` coerced to `List<String>`. Подтверждено пользователем: web → Android v2 импорт работает. **A1 first-open migration prompt** (2026-05-07): модал на трёх кнопках (перенести/остаться/решить позже) с автоматической silent-миграцией при принятии; гейт `?phase6Prompt=1` или `localStorage.phase6PromptOptIn='1'` пока default-flip не сделан. | Включить `localMode` по умолчанию (нужно подтверждённое dogfooding-окно на mobile + desktop); удалить серверные library/SRS-routes (или перевести в опциональный sync); обновить публичные docs / changelog / onboarding. Финал DoD: e2e cross-device round-trip ZIP подтвердить вручную (web → Android v2 и обратно). |

**Post-Phase-6 hotfix (2026-05-08):** rate-limit на `/api/audio/cache/upload` (B4) был слишком жёсткий для bulk ZIP-импорта (200/min vs ~2400 MP3 в типичном бандле). 800 успехов / 600 ошибок при тестовом импорте. Функционально на этом конкретном сценарии не сказалось (Railway-кэш уже содержал MP3 с прошлой генерации, re-upload был бы идемпотентным), но для нового пользователя cross-device это сломало бы playback на 600 сентенсов. Fix: лимит → 2000/min; client-side `uploadOne` теперь делает retry на 429 (max 3 попытки, респектит Retry-After + экспоненциальный backoff с jitter).

**Phase 6 release (2026-05-08):** localMode default-on flip + aggressive
server cleanup. Mobile re-dogfood прошёл успешно перед flip'ом.
- `LOCAL_MODE` теперь true по умолчанию: `localStorage.getItem('localMode') !== '0'`. Server-mode остаётся доступен только через D5 kill switch или явный `localStorage.localMode='0'` (но т.к. серверные endpoint'ы 410, в server-mode библиотека не работает).
- A1 prompt теперь fires для всех первого-разовых посетителей (dogfood-гейт `?phase6Prompt=1`/`phase6PromptOptIn` снят); copy переписан под пост-flip реальность («Перенести с сервера» / «Начать с чистого листа» / «Решу позже»). «Начать с чистого листа» теперь оставляет в localMode без миграции (ранее ошибочно set'ил `localMode='0'` → server).
- Серверный cleanup (aggressive): middleware `gone410` возвращает 410 Gone на всех stateful library/SRS/progress/history/search/nav/notes endpoint'ах. Список keep'нутых stateless эндпоинтов и обнулённых stateful — в `docs/OPFS_USER_GUIDE.md`. Last-mile recovery: `GET /api/library/export` и `/api/library/export/bundle` оставлены read-only для тех, кто ещё не перенёс данные.
- Тест `tests/storage_location_audit.test.js` (документировал pre-Phase-6 storage contract) удалён как obsolete — invariants теперь в OPFS_USER_GUIDE.

**Dogfood bug-fixes (2026-05-08):**
- **FK constraint после bulk-delete + cache-restore**: пользователь bulk-удалял карточку, потом вводил такой же исходный текст; локальный Gemini-cache восстанавливал таблицу без LLM-вызова, в save-modal оставалась кнопка «💾 Обновить» (session всё ещё ссылался на удалённый textId). При нажатии — `addSentence` падал с `FOREIGN KEY constraint failed`. Fix: `v3SessionForgetTextIfStale(deletedId)` зачищает session/active-meta при каждом удалении (single, bulk, wipe-all). Defense-in-depth в `v3LibraryUpdateCurrentCore` — проверяет `getTextById` перед `updateText`; если null — gracefully fall back на `v3LibrarySaveCurrentCore` с toast «Карточка не найдена — создаю новую».
- **«Сохранить как новый» теперь всегда доступен при наличии update-target**: раньше кнопка показывалась только если `baseTextId` (т.е. fork-сценарий после draft-mode). Премиум-сценарий: пользователь может одну и ту же таблицу сохранить как новую копию рядом с исходной. Fix: `v3SaveMetaSyncMode` теперь показывает кнопку всегда когда `updateId` есть.
- **`v3Phase6ResetDecision` не позволял повторно триггерить промпт**: функция чистила только `phase6FirstOpenSeen`, но `v3Phase6PromptShouldShow` также бейлится на `localMode !== null`. Fix: ResetDecision теперь чистит `localMode` и `phase6LastMigration` тоже.
...
- **B. Strong recommendations (премиум полировка):** B1 quota monitoring ✅ (`navigator.storage.estimate()` + dashboard widget + 80%/95% thresholds в save-path), B2 migration failure rollback ✅ (per-text atomicity в importBundle через `deleteText` cascade + `rollbackImportedTexts` + `window.v3Phase6UndoLastMigration` сохраняет последний пакет в localStorage), B3 in-memory undo для delete-text ✅ (snapshot через `exportBundle({textIds})` + `v3UndoToast` 7s window; full TTL-based trash bin отложен до post-Phase-6), B4 rate-limiting на stateless endpoints ✅ (in-memory sliding-window per-IP: 60/min transliterate, 30/min export-docx, 200/min audio-cache-upload), B5 pre-Phase-6 telemetry ✅ (localStorage ring buffer captures init success/failure + chosen VFS, ZIP-import success/failure, migration success/failure; surfaced via `window.v3OpfsTelemetry.list()` / `.summary()`).
- **C. Nice-to-haves (после Phase 6):** FTS5, cloud sync, A/B framework, .apkg offline, RTL polish, hotkeys, markdown notes.
- **D. Слепые пятна:** D1 OPFS data-location docs ✅ (`docs/OPFS_USER_GUIDE.md` — где физически живут данные на каждой ОС, что делает «Clear browsing data», как делать резерв через ZIP, кросс-устройство), D2 header trust audit ✅ (`requireSameOriginJson` middleware на /api/transliterate + /api/export/docx: validate Content-Type=application/json + same-origin (Origin/Referer); audio-cache/upload оставлен на существующем `v3AudioPrefetchIsAllowed` (env/X-Local-Mode/local-IP) — multipart, не CSRF-чувствителен в нашей модели; полная таблица доверия в OPFS_USER_GUIDE), D3 PRAGMA integrity_check on startup ✅ (idle-time after init via `requestIdleCallback`; result push'ится в telemetry; на failure — error toast «⚠ База OPFS повреждена»), D4 notes audit ✅ (Test 16 в db-init-test.html: multi-line/Unicode/2KB/RTL/JSON-shaped notes survive roundtrip + CASCADE при deleteText не оставляет orphans), D5 kill switch ✅ (env-var `KILL_LOCAL_MODE=1` на Railway → `/api/client-config` возвращает `flags.killLocalMode=true` → клиент кэширует на 15 мин и принудительно сбрасывает LOCAL_MODE при следующей загрузке без app-deploy).
