...
- **D. Слепые пятна:** D1 OPFS data-location docs, D2 header trust audit, D3 PRAGMA integrity_check on startup ✅ (idle-time after init via `requestIdleCallback`; result push'ится в telemetry; на failure — error toast «⚠ База OPFS повреждена»), D4 notes audit, D5 kill switch (remote feature flag).
