...
- **D. Слепые пятна:** D1 OPFS data-location docs, D2 header trust audit, D3 PRAGMA integrity_check on startup ✅ (idle-time after init via `requestIdleCallback`; result push'ится в telemetry; на failure — error toast «⚠ База OPFS повреждена»), D4 notes audit ✅ (Test 16 в db-init-test.html: multi-line/Unicode/2KB/RTL/JSON-shaped notes survive roundtrip + CASCADE при deleteText не оставляет orphans), D5 kill switch ✅ (env-var `KILL_LOCAL_MODE=1` на Railway → `/api/client-config` возвращает `flags.killLocalMode=true` → клиент кэширует на 15 мин и принудительно сбрасывает LOCAL_MODE при следующей загрузке без app-deploy).
