...
1. **i+1 «Следующий для тебя» (BRR-P1-007)** — ✅ recon-замер ВЫПОЛНЕН + дизайн УТВЕРЖДЁН + **S1
   ...
   гипотеза (validate-in-prod). S1 = producer `build-corpus-vocab.js` + гейт `smoke:corpus-vocab` 15/15
   (tooling-only). **NEXT = S2** (клиент ленивый загрузчик + two-channel coverage + SW-бамп + коммит данных).
