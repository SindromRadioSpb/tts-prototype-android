...
**Замер «до» (2026-09-26, 3.11.646, 380, чистый профиль, `verify-r8.js before`):**
- на 1,2 с видны две вкладки («Доступная», «Литературная»), к 3 с появляется третья («Библиотека»), и выбор перескакивает на неё;
- на 380 «Литературная» обрезана до «Литерату…» (`.room-tab` 13 px, `text-overflow: ellipsis`, три по 111 px);
- до 8 с на экране спиннер «Загрузка…», затем «Собираем следующий шаг…»; полок нет, скелета нет;
- у пользователя без чтения шапка «Ваше чтение / Продолжим с нужного места»;
- «Сегодня · Короткий текст · 1 строк» (нет склонения);
- автор в строке полки синий, как ссылка.

Причины в коде:
- загрузка последовательная: `autoImportCanon → loadData → loadCorpusCatalog → …`;
- `#tabCorpus` стоит `hidden` до ответа каталога;
- `.learning-home-ready-list .corpus-work-col` ставит автора и бейдж компаса в одну строку сетки, отсюда наложение;
- конец текста рисует высокие `renderCorpusCard`, а полки — `renderCorpusWorkRow`.

**Задачи (TDD, тесты — `tests/roomShelvesR8.test.js`):**
1. **Вкладки.**
   - Набор постоянный, «Библиотека» первая и выбрана в разметке: она и так вкладка по умолчанию.
   - Прячется только если каталог не загрузился; раньше при сбое она оставалась видимой пустой.
   - Вид — сегментированный контрол: общий фон, сегменты 44 px, `flex: 1 1 auto` по ширине подписи, без `ellipsis`.
2. **Скелет.**
   - Вместо спиннера «Загрузка…» / «Готовим библиотеку…» / «Собираем следующий шаг…» — скелет: блок «главной карточки» и четыре строки.
   - Текст состояния остаётся для скринридера (`role=status`, визуально скрыт).
   - Корень каталога запрашивается параллельно с импортом канона.
   - `prefers-reduced-motion` — без пульсации.
3. **Честность для нового профиля.**
   - Чип компаса не рисуется для `NEEDS_PROFILE` и для нижней границы, округлённой до 0 %; подробности остаются в ⓘ.
   - «0/77 зафиксировано знакомыми» заменяется на «Хороший первый текст · частотная лексика».
   - Шапка без продолжения: «Первый шаг» / «Начнём с короткого текста» (не повторяет кикер «С чего начать» карточки под ней).
4. **Один компонент «материал».**
   - Подборка в конце текста рисуется строками `renderCorpusWorkRow` (с автором), как полки.
   - В списке «Готово к чтению» компас получает свою строку сетки.
   - Автор — вторичный цвет (`--text-secondary`); подчёркивание по наведению и фокусу оставляет его узнаваемой ссылкой.
5. **Склонения.**
   - `roomCountLabel(n, 'rows'|'parts')` на `Intl.PluralRules` и ключах `room.units.rows.{one,few,many,other}` и `room.units.parts.{…}` в ru/en/he.
   - Используется в `corpusLengthLabel`, «Сегодня» и запасной мете.

**Приёмка:**
- на 380 с первого кадра видны три вкладки без обрезки, выбор не прыгает;
- до готовности полок — скелет, а не спиннер;
- у нового профиля нет «Нужен профиль слов», «Не менее 0 %», «0/N»;
- в конце текста строки той же высоты, что на полке;
- «1 строка», «17 строк»; RTL, тёмная тема, контраст.
...
**Сверка с кодом (2026-09-26, 3.11.647):** цепочка живёт в `room.morph.*`, общей для Зала и Студии (`reader-morph.js` и `morph-host.js`).
- кнопка — `save` «＋ Сохранить слово»;
- тост — `savedToast` «Слово сохранено в заметки»;
- бейдж — `life.created` «🆕 в заметках»;
- лист — `study.open` / `study.title` «📚 Разобрать слова», режимы `study.modeList` / `modeTrain` «📋 Список / 🎯 Тренировка»; в режиме повторения по сроку заголовок уже «Повторение» (`reviewTitle`), как пункт навигации;
- у кнопок шапки листа 📅 и 📊 подписи есть только в `aria-label`;
- тост Зала (`.room-toast`) и Студии (`#toastContainer`) внизу и ложится на нижний лист карточки слова (`.rm-sheet.rm-open`).

**Задачи (TDD, `tests/wordsVocabularyR9.test.js`):**
1. **Словарь терминов** `docs/planning/UI_GLOSSARY_2026_09_26.md`:
   - Зал, Медиатека, Студия, Мои слова, Повторение, Добавить в мои слова — ru/en/he, где используется, чего не писать;
   - открытый вопрос владельцу: «Библиотека» — это и вкладка Зала (корпус), и Library Студии (сохранённые карточки).
2. **Цепочка слов (ru/en/he + запасные строки в JS):**
   - `save` → «＋ Добавить в мои слова»;
   - `savedToast` → «Добавлено в мои слова»;
   - `life.created` → «🆕 в моих словах»;
   - `study.open` / `study.title` → «📚 Мои слова»;
   - `modeTrain` → «🎯 Повторение», `trainTitle` → «Повторение».
3. **Подписи у кнопок шапки листа:** «📅 Календарь» и «📊 Запоминание» — видимый текст, а не только `aria-label`.
4. **Тост над нижним листом:** пока открыта карточка слова или лист «Мои слова», тост Зала и Студии встаёт наверх (`body:has(...)`, безопасная зона сверху).

**Приёмка:**
- от тапа по слову до повторения одно имя: «Мои слова»;
- у кнопок листа видны подписи;
- тост после «Добавить в мои слова» не закрывает кнопки карточки на 380;
- RTL, тёмная тема, контраст.
...
**Замер «до» (2026-09-26, 3.11.648, 1280×860, материал Медиатеки с YouTube, учебный режим, `verify-r10.js before`):**
- видео во всю ширину 1256×340;
- таблица под ним (y = 374) прокручивается в своём окне;
- карточка слова — модальный лист 560×400 по центру поверх таблицы, с затемнением: закрывает строки, которые читаешь.

В коде:
- `body.room-study #roomReader` — фиксированная flex-колонка;
- `roomMediaApplyLayout()` знает, видно ли медиа (`mediaVisible`), но наружу этого не отдаёт;
- `.rm-sheet` (`reader-morph.js`, общий со Студией) — `position: fixed; inset: 0` с подложкой.

**Задачи (TDD, `tests/desktopStudySplitR10.test.js`):**
1. **Признак раскладки.**
   - `roomMediaApplyLayout()` ставит `body.room-word-docked`, когда учебный режим, медиа видно и ширина ≥ 1024 px.
   - Пересчёт — при `resize`, при смене учебного режима и при появлении медиа.
2. **Две колонки.**
   - `#roomReader` становится сеткой `480px | 1fr`, отступы 20 px.
   - Шапка, поиск и «Аа» — на всю ширину.
   - `#roomMediaBar` — левая колонка; `#roomReaderTable` — правая, на всю высоту, со своей прокруткой.
   - Видео не уезжает при чтении: оно не в прокручиваемом окне.
3. **Карточка слова в левой колонке.**
   - Лист в этой раскладке — фиксированная панель под видео (`--room-dock-top` считает JS), без подложки, со своей прокруткой.
   - `aria-modal="false"`; таблица справа остаётся кликабельной, тап по другому слову меняет карточку.
   - Пока слово не выбрано, на этом месте подсказка `room.reader.wordDockHint` (ru/en/he).
4. Телефон и десктоп без медиа не меняются.

**Приёмка на 1280:**
- видео слева 480 px и остаётся на месте при прокрутке таблицы;
- карточка слова открывается под видео и не закрывает таблицу;
- RTL-интерфейс зеркалит колонки;
...
**Деление (2026-09-26):** R11a — иконки на поверхностях чтения и слов, кнопки-иконки Студии, футер. R11b — жаргон, HE-названия, bidi-скобки.

**Инвентарь «до»** (`inventory-r11.js`, 380, 3.11.649, файл `R11/emoji-inventory-before380.json`), видимые эмодзи в роли иконок:

| Экран | Сколько | Что |
|---|---|---|
| главная Зала | 5 | 🔬, футер: 🔒 ❤️ 💬 🔧 |
| ридер | 11 | 🔍 ▶ в шапке, 👆 📚 в подсказке, футер |
| «Аа» | 15 | 🎬 🎨 📚 🎯 |
| карточка слова | 15 | 🔊, 🤖 «Объяснить» |
| лист «Мои слова» | 23 | 📚 📅 📊 📋 🎯 🚫 🔊 |
| Студия | 15 | 📥 ✨ 🔊 🌐 💾 🧩 🎨 📚 🎯, футер |
| Медиатека | 0 | — |

**Задачи R11a (TDD, `tests/iconsFooterR11a.test.js`):**
1. **Спрайт.**
   - Четыре иконки Lucide 1.27.0 с того же закреплённого коммита: `calendar`, `chart-column`, `inbox`, `sparkles` → `lp-icon-calendar`, `lp-icon-chart`, `lp-icon-import`, `lp-icon-sparkle`.
   - В журнале происхождения — SHA-256 исходников; подлинность коммита подтверждена совпадением хеша `search.svg`.
   - Контракт VF0 расширяется с 16–24 до 16–28 (ruling).
2. **Кнопки-иконки** получают символы спрайта (эмодзи остаётся только запасным вариантом):
   - «🔍» ридера → `lp-icon-search`;
   - 🔊 в карточке слова и в листе → `lp-icon-audio`;
   - 🤖 «Объяснить» → `lp-mark-mentor`;
   - 📅 / 📊 листа → `lp-icon-calendar` / `lp-icon-chart`;
   - 📥 / ✨ Студии → `lp-icon-import` / `lp-icon-sparkle`.
3. **Текстовые подписи без эмодзи-префиксов** (ru/en/he и запасные строки в JS):
   - «Аа»: 🎬 🎨 📚 🎯;
   - подсказка ридера: 👆 📚;
   - лист: 📚 📋 🎯 🚫;
   - Студия: 💾 🔊 🌐 🧩 🎨 📚;
   - футер: 🔒 💬 🎯.
4. **Футер пользователя:**
   - убрать «Made with ❤️ by …», GitHub, «Диагностика системы», номер версии, контакт по авторским правам (Зал);
   - всё это уже есть или переносится в «О Зале» / «О приложении»: версия и репозиторий там уже есть; добавляются автор и «Диагностика системы».

**Приёмка:**
- инвентарь «после» на тех же экранах без эмодзи-иконок, кроме запасных вариантов и содержимого (глоссы словаря, ▶︎ и ↗ как знаки текста);
- в футере не больше 6 ссылок;
- RTL, тёмная тема, высокий контраст (иконки `currentColor`).
...
## R12 — Общий футер (решение владельца 2026-09-26)

**Запрос владельца:** во всех трёх разделах один типизированный набор внизу — «Данные на этом устройстве», «Сообщить о проблеме», «О приложении» (одно имя вместо «О приложении / О продукте»), «Приватность», «О Зале / О Студии / О Медиатеке» (опционально для раздела), «Документация» — и **всегда актуальная версия** в этой же строке. Повод: после R11a версия ушла из футера в «О…», и пользователь во внешнем окне её не видит.

**Решение:**
1. Модуль `public/js/app-footer.js` (как `app-nav.js`) рисует строку в `<footer data-lp-footer="room|studio|mediatheque">` каждой оболочки. Порядок и подписи одинаковые; ключи локалей общие (`footer.*`).
   - Раздельный пункт есть только там, где есть окно раздела: «О Зале» (`#roomAboutLink`, прежняя привязка).
   - «О приложении»: в Студии — её окно `v3AboutOpen`; в Зале и Медиатеке — общее окно модуля (версия и статус, автор, GitHub, «О продукте», диагностика).
   - «Сообщить о проблеме»: в Студии — форма `v3FeedbackOpen`, в остальных — GitHub Issues.
2. **Версия** — та, на которой работает открытая страница: service worker отвечает на `GET_VERSION` своей `CACHE_VERSION`. Последняя выкаченная — `/api/client-config`. Если они расходятся: «v3.11.651 · есть новая версия» (клик открывает «О приложении»).
3. «Тур» Студии переезжает в её окно «О приложении»; атрибуция Бен-Иегуды остаётся в «О Зале».
4. Все ссылки ≥ 44 px, тёмная тема трёх оболочек, RTL.

**Тесты:** `tests/appFooter.test.js`.
