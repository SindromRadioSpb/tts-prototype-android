...
## 5g. AA4 слайс 4a+4b SHIPPED (v3.11.208) — get_progress_delta + бейдж предложений

Owner выбрал roadmap «4a → 4b → 5» и скомандовал старт; 4a+4b отгружены одним билдом. Адверсариальная критика до кода (R15+R17+R14+R4/R11, один комбинированный критик) — 2 BLOCKER пойманы в дизайне:
- **new_marks_total была бы ложью**: seed-ids контент-хэшированы, ре-сиды легальны → считаем DISTINCT item_key и зовём `new_items_scheduled`;
- **consent-строка `NO_GRADES…` противоречила grade-производному struggle** в top_items → struggle УБРАН, дельта = чистый activity-read (выходной валидатор физически не имеет полей struggle/grade — excludes истинен by construction).

**`get_progress_delta`** (scope `review.activity.read` — НОВЫЙ, не расширение review.items.read: у выданного scope purpose=DUE-слова, молчаливое расширение = consent-drift; прецедент шаринга get_access_window валиден только при идентичных purpose+data_class). Миграция 046: rebuild grants-CHECK до 12 scope (паттерн 044; проверено — на agent_connection_grants никто не ссылается FK) + индекс `ix_cloud_rl_user_kind_time` (обслуживает также getRecentStruggles/getTodayActivity). Механика: ОДИН выбор строк + JS-fold ПОСЛЕ annul-фильтра (SQL-агрегаты не умеют annul — паттерн getTodayActivity); дни складываются в USER-LOCAL зоне (db/localtime + tz нудж-préfs — одна истина с heatmap, НЕ «стрик»); top-слова считают ТОЛЬКО kind='review' (per-item skip-счёт = MNAR-сигнал избегания, skips только агрегатом); детерминированные tie-break'и; overflow-guard 100k строк; окно since ∈ [now-90d, now] в handler'е → typed `AA_ACTIVITY_SINCE_OUT_OF_RANGE` (CLIENT_FAULT, retryable:false). Выход: totals/distinct/new_items_scheduled/active_days/by_channel(≤8, lump→other)/top_items{display,gloss,times}(≤20). 13 инструментов, 12 scope.

**Бейдж «Предложения агента» (4b):** `GET /api/agent-access/proposals/summary` → `{pending_total}` = буквально `listPending().length` (by construction тот же предикат, что панель — бейдж не может over-claim; критика поймала ловушку: predicate капа в create() НЕ фильтрует revoked-подключения — его копировать нельзя). Чипы: Зал (`#roomAgentProposals`, outlined-стиль под due-CTA, home 380px) + Студия (v3.11.209: чип живёт в CLASSIC-режиме `#v3ClassicAgentProposals` — owner не пользуется IDE; первоначальный IDE-header-чип удалён по owner-фидбэку). Count-only by design: agent-текст рендерится ТОЛЬКО на enforced-CSP `/agent-access.html` (index/library — Report-Only CSP). Quiet-fail: без сессии/при ошибке чип скрыт (может under-claim, не over-claim — осознанное решение, silent-empty здесь = спрятанный вход, панель доступна своим путём); refetch на visibilitychange/pageshow (долгоживущие PWA-вкладки). Локали ru/en/he (`room.agent.proposalsChip`, `ide.agentProposals`) + SW v3.11.208.
...
Гейты: domain 35 / mcp 54 (14 tools) / production-handlers 61 + oauth/deployment/control-plane/api; миграция 047 upgrade-путь с живым токеном (сохранён вкл. work_id; nullable+DEFAULT работают). **Live на проде:** Hermes-токен сминтил ссылку → клик → Зал открыл РЕАЛЬНУЮ due-сессию владельца (154 к повторению, карточка «Диктант» 1/12) → закрыто через ✕ без оценок (live-verify правило). Re-consent №4 на 13 scope; `hermes mcp test` → 14 tools. **OWNER-VERIFIED 2026-07-18:** в живом разговоре Hermes сам сминтил ссылку, владелец кликнул → «Учить новые слова» открылась; агент осознанно НЕ кликал и не отвечал в сессии (уважил live-verify правило — этикет закрепился). Мелочь в его прозе: «откроет самую просроченную карточку» — на деле сессия ранжирует по слабости (rankByWeakness), не строго по просроченности; не баг продукта.
