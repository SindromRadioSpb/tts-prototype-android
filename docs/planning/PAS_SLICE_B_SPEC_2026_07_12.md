...
>
> **Owner-фидбэк раунд v3.11.160 (476a2dd, 2026-07-12, по скриншотам владельца):** (1) «Пересказ проще» РАСШИРЕН на личные тексты Зала — **записанное решение владельца**: scope `sentence_window_5` = понимание + пересказ (consent-копия ×3 обновлена; personal-драфт facts[0]=`user_window` → тумбстоунится на revoke, corpus переживает — teeth в гейте); (2) [Открыть в Студии] ПЕРЕДЕЛАН: НЕ createText, а НЕСОХРАНЁННАЯ карточка через штатный пайплайн — handoff `studio.agentDraftHandoff` (localStorage, TTL 10 мин) → композер `#inputText` → `translateTable()` (огласовки/транслит как у дефолтной таблицы); (3) модал Студии = паритет с Залом: 🔊 (штатная per-row озвучка), перевод ряда, «🧠 Проверь меня по абзацу» (personal window_5, общий first-use ack), «✍️ Пересказ проще» → «В редактор» (same-page consumer). Гейты: agent-material 51/51 · studio-agent 163+ (52 ключа ×3) + полная регрессия. Live-verify через kapture прерван обрывом соединения с вкладками (SW-автообновление) — верификация владельцем.
