...
| **D3** | Видимый due-счётчик | R4/R5 | C2 пишет srs_due → UI-only, низкий риск |
...
**Рекомендация на старт следующей сессии:** начать с **D3** (видимый due-счётчик) — чистый выигрыш на уже отгруженном C2, UI-only, низкий риск, сразу делает невидимое SRS-расписание видимым (замыкает обратную связь). Параллельно дёшев **D4** (weakness-weighting — ранжирование по srs_lapses). Перед кодом — R10/R2-линза + measure (есть ли у владельца уже накопленные srs-строки: `getSrsSchedule()` вернул 0 на момент хэндоффа — счётчик увидит данные после первых recall-сессий).
