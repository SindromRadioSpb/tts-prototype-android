...
**СОСТОЯНИЕ — АКТУАЛЬНО 2026-06-29 (ЭТА секция авторитетна над §0–§5):** прод **`v3.11.40`**, SW синхронизирован (**auto-skipWaiting** — свежий код с первого reload), ветка `main` чистая, посл. код-коммит `4f7d449`. **Эпик 4 (петля удержания) ЗАВЕРШЁН + Phase D: D1·D3·D4·D5·D7 ОТГРУЖЕНЫ (D1–D5 live-подтверждены; D7 prod-verified Node no-store + adversarial-review pass — live-verify training-флоу за владельцем на ОДНОРАЗОВОМ тексте).** Следующее по очередности — **D6 (reverse/listening/dictation, R8/R2)**.
...
- **СЛЕДУЮЩЕЕ = D6** (reverse RU→HE / listening / dictation, R8/R2); затем **D2** (cross-text due — нужны тела работ). Опц. **D7.1**: месячный heatmap из `study_day`.
