...
**Рекомендация на старт следующей сессии:** D1+D3+D4 ОТГРУЖЕНЫ (v3.11.33–36). Следующее по очередности — **D5 (teach-before-test, R2/R8):** перед ПЕРВЫМ тестом нового слова — краткий показ (форма + значение + аудио) → первая встреча перестаёт быть гарантированным провалом/угадыванием → засевает след памяти. Reuse `openWordCard`/`speakWord`; гейт — НЕ засчитывать обучающий показ как recall (не писать srs/level от показа). Затем D7 (геймификация) · D6 (reverse/listening) · D2 (cross-text due — нужны тела работ). Перед кодом — краткий план + развилки + measure (как обычно).
