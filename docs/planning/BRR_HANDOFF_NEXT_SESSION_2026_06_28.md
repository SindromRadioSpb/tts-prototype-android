...
**Рекомендация на старт следующей сессии:** D3+D4 ОТГРУЖЕНЫ (v3.11.33/34). Следующее по очередности — **D1 (полный slot-inflection дистракторов, R10):** дистракторы склоняются в ТУ ЖЕ грамм. позицию, что и пропуск → MC проверяет ЗНАЧЕНИЕ слова, а не «грамматически подходит только ответ» (моат R10 в полную силу). B1 (огласованность дистракторов) — уже частичный шаг; нужен офлайн slot-inflection из Pealim-парадигм (`pickDistractors` в reader-morph.js). ⚠ measure-first: замерить долю MC-вопросов, где форма-tell реально выдаёт ответ, до кода. Перед кодом — краткий план D1 + развилки владельцу.
