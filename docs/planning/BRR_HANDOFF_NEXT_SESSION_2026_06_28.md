...
**СОСТОЯНИЕ — АКТУАЛЬНО 2026-06-29 (ЭТА секция авторитетна над §0–§5):** прод **`v3.11.42`**, SW синхронизирован (**auto-skipWaiting** — свежий код с первого reload), ветка `main` чистая, посл. код-коммит `163cda3`. **🎉 Эпик 4 (петля удержания) + Phase D (D1–D7) ПОЛНОСТЬЮ ЗАВЕРШЕНЫ** (D1 slot-MC · D2 cross-text-due · D3 due-счётчик · D4 weakness · D5 teach-first · D6 reverse/listen/dictation · D7 soft-gamification; D6·D7·D2 prod-verified Node no-store + adversarial-review pass; D1–D5 live-подтверждены). **🟢 УТВЕРЖДЁННЫЙ ПОРЯДОК (владелец 2026-06-29): → ЭПИК 5 (graded-импульс, R8 — СЛЕДУЮЩИЙ) → остаток 3b (function-usage, P2, нужен content-authoring+sign-off) → Эпик 6 (curated library) → Эпик 9 (karaoke UX); опц. D7.1 heatmap в любой момент.** Старт Эпика 5 — см. §«ЭПИК 5 — СТАРТ» ниже. Канон приоритетов/волн — `BRR_UX_AUDIT_2026_06_25.md` (D7.1 добавлен туда как опция). ⚠ live-verify training-флоу за владельцем на ОДНОРАЗОВОМ тексте.
...
- **D2 ✅** cross-text «due today» — ежедневная очередь по ВСЕМ текстам (v3.11.42 `163cda3`) — mig 060 source-якорь на `word_status` (`srs_text_key/sentence_id/order_index/surface`, COALESCE-preserve); `local-db.getDueWithSource`(ordered)/`getSentenceForReview`(re-anchor)/`getDueReviewCount`(sourced-only); `startDueReview()` cross-text сессия (переполучить предложение→cloze→`_launchTrainSession`, общий со `startTraining`); НЕ нужен открытый текст; due-без-предложения→SKIP (R11); home-CTA `#roomDueCta` (sourced-count, скрыт при чтении); аудио унифицировано на `_built.audioAssetKey`. Adversarial-review pass (CTA hide-on-open + honest-count + ordered-fetch). Prod-verify v3.11.42.
- **🎉 Phase D ЗАВЕРШЁН.** 🟢 **УТВЕРЖДЁННЫЙ ПОРЯДОК (владелец 2026-06-29): → ЭПИК 5 (graded-импульс, СЛЕДУЮЩИЙ) → 3b (function-usage, P2, нужен content-authoring+sign-off) → Эпик 6 (curated library) → Эпик 9 (karaoke UX); опц. D7.1 heatmap в любой момент.** Канон волн/приоритетов — `BRR_UX_AUDIT_2026_06_25.md` (D7.1 добавлен туда опцией). Старт Эпика 5 ↓.

---

## 🟢 ЭПИК 5 — СТАРТ (graded-momentum / импульс graded-reading · P1 / L · R8) — СЛЕДУЮЩИЙ
**Зачем (стратегия §3.7 аудита):** Эпики 3/4/5 = цикл **learn→recall→next-step** (моат LingQ/graded). Эпик 4 + Phase D закрыли **recall**; Эпик 5 — недостающий **next-step**: «никаких тупиков, всегда что дальше». Канон находок/последовательности/рисков — `BRR_UX_AUDIT_2026_06_25.md` §«ЭПИК 5» (стр. ~61–64).

**Компоненты (приоритет из аудита):**
- **`continue-mark-read`** (P1 / S, R8) — «Продолжить» нельзя завершить; 98%-текст висит вечно. → отметить «прочитано» (полка не копит дочитанное).
- **`end-of-text-handoff`** (P1 / M, R2/R8) — дочитал → тупик в сетку корпуса; нет «что дальше»/следующего i+1 в момент макс. мотивации.
- **`difficulty-signal-profile-independent`** (P1 / M, R8/R7/R9) — без профиля сложность невидима; `ez`/archaica-loadFlag есть в сайдкаре (лишь сортируют cold-start); archaica = свойство ТЕКСТА, не читателя (R7-антипаттерн) → derived/approximate бэнд с провенансом, НЕ жёсткий CEFR.
- **`in-text-coverage-chip`** (P2 / S, R8) — i+1/покрытие исчезает при открытии текста.
- **`niqqud-fade-graduation`** (P2 / M, R8/R9) — флагманский адаптивный никуд-fade по умолчанию «всегда огласовано» и закопан → у большинства НИКОГДА не затухает («леса, что не затухают»).

**Measure-before-code (R10) — куда смотреть (данные/код уже есть):**
- **progress / continue:** `text_progress.last_row_idx` (mig 003) + `localDb.setProgress`/`getProgress`/`getContinueReading`/`touchOpened`; `recordProgress`/`restoreReaderPosition`/`closeReader` в library-ui; «Продолжить чтение» — corpus home (`corpusRefreshL1Body`).
- **difficulty:** corpus-каталог сайдкар (`ez`/archaica/loadFlag — `build-corpus-catalog.js`); эпоха/QID из R9-обогащения (derived, провенанс).
- **i+1/coverage:** `ReaderMorph.collectNewWords`/`collectReviewItems` (фронтир) + `getKnownWordStates`/word_status (известное) → доля известных в открытом тексте.
- **niqqud-fade:** `readerCfg.niqqudMode` (`adaptive`/`full`/`off`, persist `room.niqqudMode`) + scaffold-движок (`smoke:reader-scaffold`); адаптивный fade уже есть — измерить, ПОЧЕМУ не затухает по умолчанию.

**Инварианты/норма (как везде):** Room-only, **parity-safe** (index.html + reader-core НЕ трогать; `smoke:reader-parity`); measure-before-code → краткий план + развилки (AskUserQuestion) с рекомендацией → **владелец решает** → имплементация → гейты (§4) → bump version+SW+футер → push → Coolify → prod-verify Node no-store → live-verify; @380px ru/he свет+тёмная ДО `git add`; mark plan в `docs/planning/`. ⚠ training/прогресс live-verify — на ОДНОРАЗОВОМ тексте/движок-eval (не рабочий профиль).
...
- **Smoke:** `scripts/premium/reader-morph-smoke.js` (collectNewWords/collectReviewItems/buildClozeForTarget/pickDistractors/nextSrs/openWordCard + **D3 dueCounts · D4 rankByWeakness · D1 findSlot/buildMcSlotOptions · D7 streakFromDays/streakView (fold/grace/rest-credit/future-clamp, +13) · D6 availableChannels**), `scripts/premium/reader-word-status-smoke.js` (+миграция 058/059/060 + persist/preserve + **D7 study_day леджер · D2 getDueWithSource/getSentenceForReview(re-anchor)/getDueReviewCount + COALESCE source-preserve + future-due-exclude**), **`scripts/premium/reader-cloze-audit.js`** (`smoke:reader-cloze:audit` — D1 slot-coverage аудит, 96.5%, отчёт → `docs/research/epic4-3b-d1-slot/2026-06-29/`).
