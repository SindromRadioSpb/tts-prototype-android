...
**🔵 NEXT (активация/rollout — прод-плюминг + большой bake):**
1. **Bake полный корпус:** `npm run build:proclitic-overlay -- --bake` по всем 796 baked-работам (Dicta-батч, resume-able). Растёт с coverage.
2. **Push оверлеев на том:** нужен `scripts/premium/push-proclitic-overlay.js` (клон `push-corpus-works.js`) + server upload-route `POST /api/benyehuda/proclitic/upload` + static mount `/data/benyehuda/proclitic` (клон works/ на `server.js:3457`/`:414`, allowlist `^[A-Za-z0-9_-]{1,40}\.json$`, X-Audio-Upload-Token). Карта интеграции — в этой сессии (Explore-агент).
3. **Wire в `publish-corpus-batch.js`:** produce-step (bake) до buildVNext + bodies-first push-line. Cache-busting бесплатно через CORPUS_CATALOG_VERSION.
4. **Push v3.11.66 на main** (деплой рендера+движка; до push оверлеев фича невидима = безопасно).
5. **Решения владельца:** (a) push сейчас (фича спит до оверлеев) или после bake+push? (b) показывать rich-офлайн-hedge (`procliticsRaw`,非-trivial minority ~5.5%) или строго confident-only? (c) recall-boost: **Dicta-attested-whole-word лексикон** ЗАМЕРЕН do-no-harm-clean (+только на выборке; корпус-vocab pass даст больше) — строить?
