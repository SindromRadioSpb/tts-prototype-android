...
9. **Replay ledger = audit, short `auth_date` TTL = the real bound (implemented 2026-07-09):** `query_id` is **NOT** an official nonce (verified — documented use is only `answerWebAppQuery`; absent for keyboard/inline launches). We record `(user_id, sha256(hash), auth_date)` in `miniapp_initdata_seen` for observability, but do **not** hard-reject a replay: a legitimate lost-response retry reuses the *same* initData and must still receive a session, and "return the same session" is impossible because the session secret is never stored (only its sha256). The replay window is therefore bounded by the short `auth_date` TTL (step 6), and a replay within it is logged (`miniapp_auth_replay`), not blocked. Capturing initData requires MITM/XSS (larger compromise); the TTL caps the exposure.
