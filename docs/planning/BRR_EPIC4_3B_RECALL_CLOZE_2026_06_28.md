...
### Phase B — next: B1 дистракторы под слот-форму (все огласованы, грамматически правдоподобны) · B2 «Не знаю»/skip→reveal+мягкая деградация · B5 терпимость ввода (ktiv male/chaser, проклитики/любая словоформа леммы).
### Phase C1 — тап-сборка слова (word-bank) между MC и вводом (ивритский ввод на мобиле).
### Phase C2 — «на сегодня» по времени: word_status nextDue/lastCorrect/reps (миграция) + FSRS-lite + кросс-сессионная очередь.
