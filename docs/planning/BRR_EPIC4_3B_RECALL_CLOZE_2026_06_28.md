...
**Верификация:** каждая фаза — headless-смоуки (бутят реальный library.html + миграции вкл. 058 + движок на shipped-датасете), @380px-скрин через РЕАЛЬНЫЙ путь (study-shot → тренировка → MC/тап-буквы/«Не знаю»/мультибланк), прод-маркеры Node-fetch. **Live-Kapture:** измерения движка на реальном профиле во время аудита — успешно (collectReviewItems 36/973, морфо-честные дистракторы, повтор-утечка 9/987). Post-ship live-кликтру на устройстве владельца ЗАБЛОКИРОВАН мульти-таб OPFS-локом (открыт другой linguistpro-таб) + eval отключён в текущей Kapture-сессии → требует закрыть прочие linguistpro-вкладки.

## v2-остаток — что даст каждое требование по факту реализации
| Требование | Что даст по факту |
|---|---|
| **Полный slot-inflection дистракторов** | Дистракторы склоняются в ТУ ЖЕ граммати­ческую позицию, что и пропуск → исчезает подсказка «грамматически подходит только ответ»; MC начинает проверять ЗНАЧЕНИЕ слова, а не угадывание по форме/слоту → честнее и сложнее узнавание (моат R10 в полную силу). |
| **Cross-text «due today» (нужны тела работ)** | Единая ЕЖЕДНЕВНАЯ очередь повторения по ВСЕМ прочитанным текстам, а не только по открытому → настоящая привычка интервального повторения («сегодня к повторению N слов» независимо от того, что читаешь) = петля ежедневного удержания Anki/Busuu. Требует доступ к предложениям из других работ (телам) для клоуза. |
| **Видимый due-счётчик** | Делает уже работающее (но невидимое) интервальное расписание ВИДИМЫМ: бейдж «N к повторению» → пользователь понимает, когда и почему слова возвращаются, и получает мотив вернуться → замыкает обратную связь SRS. |
| **Weakness-weighting** | Сессия взвешивается в сторону слов, которые ты чаще ПРОВАЛИВАЕШЬ (история lapse) → усилие тратится там, где память слабее → быстрее к мастерству (Quizlet/FSRS difficulty), вместо равномерного покрытия. |
| **Teach-before-test** | Перед ПЕРВЫМ тестом нового слова — краткий показ (форма + значение + аудио) → первая встреча перестаёт быть гарантированным провалом/угадыванием → засевает след памяти → честнее и лучше удержание новых слов. |
| **Reverse / listening / dictation** | Доп. КАНАЛЫ извлечения по тем же словам: RU→HE (производство), «услышал→опознал» (аудирование), «впиши услышанное» (диктант) → многомодальное кодирование углубляет память + тренирует слух/правописание, а не только узнавание-при-чтении. Переиспользует baked-аудио. |
| **Gamification (стрик/дневная цель)** | Внешнее давление возврата (стрик, XP, дневная цель) → двигает ЕЖЕДНЕВНЫЕ повторения (привычка — то, что вообще заставляет SRS работать) → выше удержание за счёт регулярности; в паре с due-очередью. |
