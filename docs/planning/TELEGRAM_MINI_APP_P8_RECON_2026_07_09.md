...
> **CHARTER EVOLVED 2026-07-10 (owner, после P8.1–P8.3 owner-live + §2.3-замера 84%):** роль P8 расширена с «context micro-session» до **автономного клиента ежедневных тренировок**: primary CTA = умная тренировка (селектор), второй полноценный путь = ручной выбор МОДАЛЬНОСТИ (сервер выбирает eligible-слова; `selection_origin='manual'`; grader/binding/cooldown/scope/review_log не обходятся). Анти-цель «не generic flashcard app» сохраняется через manual-eligibility contract; полный ручной тренажёр остаётся в браузере. Канон: **`TELEGRAM_MINI_APP_P8_4_CHARTER_REV_2026_07_10.md`**.
>
> **RESOLVED 2026-07-09 (owner):** Fork 1 → **A (context micro-session; evolved 2026-07-10 ↑)** · Fork 2 → **A (owner-pilot now)** · Fork 3 → **A (reading-first)** · Fork 7 → **A (scoped cookie on `user_sessions`)**. Forks 5, 6, 8, 9, 10 **default to their recommendations** (opaque handoff token · single active challenge across surfaces · coordinated MainButton+DOM · today-completed+due-remaining+reading CTA · owner→closed-pilot→wider-flag).
