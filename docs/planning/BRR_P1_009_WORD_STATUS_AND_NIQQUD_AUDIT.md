...
- 2026-06-11: **Stage 2 ОТГРУЖЕНО-как-код** (SW v3.10.28-room-conj, owner-запрос «1:1 + озвучка форм»):
  ...
  **тап формы → озвучка** browser-TTS, как в Студии; R1: формы из Pealim, голос = TTS, не запись).
  ...
  Pealim-бейдж / 0 pageerror.
- 2026-06-11: **Stage 2 PROD-ВЕРИФИЦИРОВАНО** (linguistpro.kolosei.com, SW v3.10.28-room-conj, `2a8db43`):
  השחר → 24 ячейки / 24 tap-озвучки / 5 корне-чипов / Pealim-бейдж / 0 pageerror; @380px скрин.
  **BRR-P1-009 ЗАКРЫТ (Stage 1+2 в проде).** Reading Room learner-loop полон: читай→тап→1:1-карточка
  (морфо+корне-семья+таблица+озвучка форм)→Сохранить→цвет-статус→Anki→i+1-ready. **NEXT = i+1
  «следующий для тебя» (BRR-P1-007)** (per-work vocab-профиль на publish-time) ИЛИ полиш/опц.
  eager-auto-notes. Owner/бейк-трек: перебейк 110 работ <60% никуда.
- 2026-06-11: **Качество резолва — measured deep-analysis + ТИР 1 ОТГРУЖЕН** (после owner-вопроса
  «как реально поднять соответствие карточка↔Pealim»). Измерено на work 34704 (silver=Dicta): 74%
  служебных слов получали ЧУЖОЙ контент-глосс (אֵין→«уничтожить», עָלֵינוּ→«лист»). Введена роль **R10**
  (вычислит. морфолог; `PROJECT_ROLES.md`). Опытный офлайн-гейт (стоп-лист+сегментация) испытан ДО
  выдачи: recall 46%→77%. **Реализован «честный гейт глосса»** в `reader-morph.functionGate`+`resolveCore`:
  функц.-форма → честное служебное чтение, БЕЗ ложной таблицы/глосса; контент не тронут. Замер
  (`qa-verify-gate.js`): recall **87%**, реальных FP **0**. Гейты зелёные (+R10-кейсы в `smoke:reader-morph`),
  reader-parity (index.html не тронут), SW `v3.10.30-room-gloss-gate`. Анализ+решение →
  `docs/planning/BRR_RESOLVER_CORRESPONDENCE_ANALYSIS_2026_06_11.md`. Roadmap: Тир 2 (перебейк никуда,
  бейк-трек) · Тир 3 (Dicta «точный режим» opt-in — egress РАБОТАЕТ: миф «закрыт» был curl-артефактом;
  возможно сервер-сайд `/api/morphology` ИЛИ client `reader-dicta.js`).
  **PROD-ВЕРИФИЦИРОВАНО** (`e63dd8b`, SW v3.10.30, 0 pageerror): אֵין→«нет»/עָלֵינוּ→«на нас»/עוֹד→«ещё»/
  אֲפִלּוּ→«даже» (служебное+поиск, без таблицы); контент цел (יֶלֶד/לִבֵּנוּ +таблица+direct). **Тир-1 в проде.**
- 2026-06-11: **ТИР 2 (niqqud-only re-bake) ВЫПОЛНЕН локально** (data-рычаг для контентных слов). NEW
  `scripts/premium/reniqqud-fill.js` (`reniqqud:fill`, гейт `smoke:reniqqud` 6/6) — пер-словная text-safe
  реконструкция огласовки из Dicta. **109 Hebrew works 40%→91%, 0 порчи; resolver exact|likely 2%→51%
  (11778).** Adversarial Workflow (16 агентов) поймал **1 идиш (47097)** → исключён + авто-гейт по идиш-
  токенам; вердикт **ship-with-notes (13/15 pass)**. Тир-3a honest-degradation (`morphologyGateway`
  `degraded:true` на пустой Hebrew-ответ) пофикшен. **Публикация 109 тел — owner-gated** (token); runbook
  `docs/planning/BRR_TIER2_RENIQQUD_RUNBOOK_2026_06_11.md`. Бэкап оригиналов `.tmp/benyehuda/reniqqud-backup/`.
