...
| **CLG-P6** | Agent Runtime: planner/tutor/reviewer/explainer/recommender/grader, tool router, context pack builder, лимиты §7/§11 (pre-call reserve); **вход-условие: авто-prune docker-образов включён (§11) — ✅ 2026-07-05 (ручной prune 85%→23% + владелец включил Coolify Force Cleanup)** | Агент проходит §7-цикл на синтетике + владельце; **prompt payload отсутствует в stdout/логах agent-сервиса**; лимиты срабатывают на burst-тесте; R17-гейты 1–8 (PROJECT_ROLES.md) | 🟡 **слайс 1 SHIPPED v3.11.103 (2026-07-05):** мигр. 026 (agent_profiles / agent_tasks / agent_explanations / llm_usage_ledger; threads/messages — с чат-поверхностью P7, мёртвые таблицы не создаём; все с user_id → динамический export/delete sweep покрыл автоматически — проверено гейтом) · `db/agentRepo.js` (ledger **pre-call check-and-reserve** под txnLock — атомарен на параллельном burst) · `agent/{runtime,tools,llm,planner}.js`: **закрытый tool router** (user_id ТОЛЬКО из принципала — B2-reject даже совпадающего; `record_review_answer`/`synthesize_audio` = disabled-SKELETON до гейтов 4.8 — честный TOOL_DISABLED) · провайдер-абстракция gemini\|claude\|mock + kill-switch (проверяется ДО резерва — бюджет не жжётся) · **сценарий №1 `/plan`** (read-only: MNAR-гейт «review_log нетронут» в смоке; D1-секция production_gap «узнаёт при чтении — проваливает письмо → сегодня диктант» из channel_stats; R11 honest counts == /due; R17-категория на каждом пункте) · endpoints `/api/agent/{plan,status,tasks,profile}`. **Гейт `smoke:agent-plan` 26/26** (+ stdout-гигиена класса D: sentinel-ключи не утекают в логи). **Топология:** пока in-process модуль main-контейнера за швом tools.js (§13.4-буква = отдельный сервис; single-writer сохранён — agent/* не трогает SQLite напрямую, только db/agentRepo; выделение в отдельный Coolify-сервис — при P7-webhook или RAM-давлении). **UI-слайс v3.11.104:** кнопка «🧭 План на сегодня» в ☁-модале Зала (render строго textContent — LLM-текст не HTML; деградация подписывается «без LLM»; i18n ru/en/he + SW-бамп; 380px light скриншот; известный трап SW-автообновления в headless — вторая загрузка). **Live-hardening 2026-07-05→06 (owner real-key тестирование, backend-only — версия/SW не бампались):** pid-ключи в плане резолвились в `pid:N` вместо слов → `keyingService.displayForItemKey()` — обратный индекс pid→lemma по всему датасету paradigms (проверено на реальных pid владельца); LLM изредка эхнул JSON вместо прозы → `isCleanProse()` пост-фильтр + усиленный промпт (честная деградация `LLM_OUTPUT_INVALID`, вызов всё равно списывается — quality-провал не бесплатен); транзитный 503 у Gemini free-tier → retry-once с backoff 700мс строго на 503/429 (ledger резервирует один слот на клик — retry бесплатен для бюджета). **Второй провайдер — OpenRouter (3bceb80, 3272f13):** `AGENT_OPENROUTER_API_KEY` (тот же строгий паттерн без фолбэков) → деф. `nvidia/nemotron-3-super-120b-a12b:free` (Poolside Laguna M.1 отклонён — карточка модели разрешает тренировку на free-tier input, нарушает R15); reasoning-модель требует `reasoning:{enabled:false}` в теле запроса, иначе льёт chain-of-thought вместо ответа и обрывается по лимиту токенов (найдено на live-тесте владельца, гейт `smoke:agent-llm-provider` 18/18 стоит регресс-guard). **Owner live-verified 2026-07-06:** реальный ключ + реальный профиль → `llm_used:true`, чистый тёплый русский ответ от Nemotron через уже залогиненную сессию в браузере. **Слайс 2 `/explain sentence` SHIPPED v3.11.109 (2026-07-06; приватность решена владельцем ДО кода — §15.9):** consent = ОТДЕЛЬНЫЙ durable `agent_read_texts` (по духу класса C §5 «разрешить агенту видеть полный текст»; сервер требует ОБА согласия на КАЖДЫЙ вызов, fail-closed 403 с точными кодами) + разовый first-use confirm в модале объяснения; scope-контракт ЖЁСТКИЙ — `scope_level='sentence_only'` (только явный; соседи/абзац → 400 UNSUPPORTED_EXPLAIN_SCOPE; `db/agentSentenceRepo.js` ФИЗИЧЕСКИ извлекает одну строку по text_key+order_index — learnerArtifactsRepo остался opaque, новая способность в отдельном модуле); `agent/explainer.js` — детерминированное ядро (предложение + резолвер-морфология keyingService + пересечение с due/weak/production_gap), LLM только формулирует (R1), isCleanProse-фильтр, LLM-less fallback (перевод+морфология+слабые места); `facts_used`-провенанс на каждый факт (user_sentence: consented_artifact+scope+anchor · translation: studio_translation/derived · morphology: resolver/asserted · learner_state) → `agent_explanations` через инструмент create_explanation; **отзыв consent → purge контентных полей до tombstone** (facts_used='[]', purge_reason; провал purge НЕ молчит — виден в ответе и audit_log); `get_sentence_context_if_available` включён (снят OPAQUE_ARTIFACT_STORE); endpoint `/api/agent/explain` (401/CSRF/rate-limit/404-якоря); UI: чекбокс «🤖 Разрешить наставнику читать мои тексты» в ☁-модале + per-row 🤖-кнопка ТОЛЬКО на своих текстах (корпус не в artifact-store by-design; контекст-first — кнопка живёт В читалке, R17-гейт 8) + модал результата (textContent-only, честная деградация, подпись провайдера); i18n +15×3; PRIVACY.md-аддендум дополнен. Гейт **`smoke:agent-explain` 33/33** (consent-лестница · scope-гейты вкл. «соседи не в ответе/facts_used/stdout» · провенанс · MNAR · revoke→purge · export-sweep · kill-switch без ledger-burn · stdout-гигиена). **Owner live-verify /explain пройден 2026-07-06 («Тестирование прошло успешно»).** **P6.3 production-hardening + P6.4 construct-субстрат SHIPPED v3.11.110 (2026-07-06, owner brief):** (P6.3) client duplicate-tap guard (single-flight `_explainInFlight` — мобильный «тапнул-не-дождался-тапнул» не жжёт ledger повторно; сервер pre-call reserve = вторая линия) + гейт **`smoke:agent-explain-burst` 19/19** по РЕАЛЬНОМУ пути `generateGemini` (preload-шим `lib/agent-provider-shim.js` мокает @google/generative-ai в спавнутом сервере через NODE_OPTIONS): burst на последний кредит → ровно 1 provider-call/1 llm_used/1 честный USER_LIMIT · failed provider call освобождает бюджет (status='failed' не считается) · over-limit не ходит в провайдер · ledger без reserved-грязи · адверсариальная provider-ошибка с prompt+key ВНУТРИ message полностью санитизирована (ни prompt, ни предложение, ни ключ — ни в stdout, ни в ответе) · kill-switch = 0 provider-calls. (P6.4) `agent/constructs.js` — pure construct-id реестр (channel_gap.reading_to_dictation/reading_to_reverse/receptive_to_production + 7 binyan.recognition) + детерминированная детекция: **construct_id назначает ТОЛЬКО сервер** (filterKnown = структурная гарантия; LLM получает titles, не ids); channel-gap уточняется по РЕАЛЬНЫМ каналам review_log (get_word_lifecycle), без production-событий → честный receptive_to_production (R9/R11 — не утверждаем гранулярность тоньше данных); binyan — только когда резолвер утверждает (bare-surface серверный резолв биньян часто не даёт — замерено, это норма); имена биньянов матчатся ТОЧНОЙ формой с огласовками (стрип-коллизия פעל=paal/pual/piel). /explain: constructs в ядре+ответе+facts_used (source=construct_registry/derived, evidence_item_keys)+fallback-тексте; /plan: production_gap несёт construct_ids на секции + construct_id на пунктах. Гейты: agent-explain 41/41 (+5 pure, +3 e2e) · agent-plan 28/28 (+2). **v3.11.111 (подготовка live-verify P6.4, 2026-07-06):** статическая проверка рецепта поймала дефект — `productionImbalance` считал провал production только по again, а D1 НАМЕРЕННО пишет провал диктанта рецептивно-сильного слова как Hard(2) → честно проваленный диктант ВЫКИДЫВАЛ слово из gap-секции; фикс: prodFails = again+hard (гейт-ассерт added, explain 42/42). Плюс UI-серфейс констрактов для верифи без DevTools: строка «⚙ <титул>» в модале /explain (всегда, не только в fallback) + титулы конструкций в плане ☁-бокса (сервер шлёт title_ru/title_en на секции — клиент реестр не дублирует). **v3.11.112 (owner live-verify кейс טוב, 2026-07-06):** слово, проваленное сегодня в чтении И диктанте, не появлялось НИГДЕ в плане — production_gap честно требует рецептивной силы (провал везде ≠ канальный разрыв), а lapses-first top-5 due-среза на большом профиле хоронит свежие провалы под годами накопленных lapses у старых слов. Фикс: секция **fresh_struggles ПЕРВОЙ** в плане («Сегодня не далось — вернёмся прямо сейчас») — `learnerGraphRepo.getRecentStruggles` (≥2 провала grade≤2 за 24ч учебного времени, prod_fails отдельно → канал по факту: были production-провалы → диктант) + инструмент `get_recent_struggles` + дедуп с gap/due-срезами; UI-изменений не требует (план-бокс рендерит секции генерически). Гейт agent-plan 30/30 (+2: секция первая с fails_24h/prod_fails_24h; слово не дублируется в due). **Owner re-verify 2026-07-06: fresh_struggles ✓ live** (טוב→לטוב первой секцией, канал/дедуп верны). **UX-развилка владельца «план текстом — работать невозможно» → РЕШЕНИЕ: эволюция P6.5 → P9** (вариант 1; контракт /plan УЖЕ семантический — id/category-R17/recommended_channel/item_keys/construct_id — миграции не будет, меняются только рендереры; Mini App P8 = второй рендерер того же контракта). **P6.5 SHIPPED v3.11.113:** план ИСПОЛНЯЕМ — кнопка «▶ Начать» на секциях category='тренировать' → кросс-текстовая тренировка ПО item_keys секции (переиспользована машинерия startDueReview через вынесенный _buildDueSourcedItems; пул = getDueWithSource с горизонтом-в-будущее, т.к. gap-слова не обязательно просрочены; слова без source-якорей — честный skip + honest-empty подсказка; канал = recommended_channel через trainChannelSet, капсы тренера откатывают честно) + «▶ В Зал» на 'вернуть к чтению'; DOM-рендер строго textContent; i18n +3×3; 380px ✓. Следующий шаг эволюции = **P9 «дом наставника»**: 🤖-панель в шапке Зала (план+действия, лимиты, история объяснений, consent-галочка агента переезжает из ☁; ☁ возвращается к синку) — скелет Mini App P8. **Owner live-verify P6.5 ПРОЙДЕН 2026-07-06 («Тестирование прошло успешно») — CLG-P6 ЗАКРЫТ ЦЕЛИКОМ** (/plan + /explain + hardening + construct-субстрат + исполняемый план, всё live-verified). Следующий слайс программы = P9 «дом наставника» (🤖-панель в шапке Зала; контракт не меняется — только рендерер) |
| **CLG-P7** | Telegram Bot: pairing (hardening §8), команды, журнал действий во вкладке. **Решения владельца ПРИНЯТЫ 2026-07-06 — канон `TELEGRAM_P7_DECISION_2026_07_06.md`** (главный принцип: «бот — тонкий канал, не новый учебный мозг»; стадирование P7.0a annul-семантика → P7.0b серверный грейдер+gold → P7.0c активация record_review_answer web-smoke → P7.1 pairing+read-only команды /start /link /unlink /status /plan /explain /due /summary /help → P7.2 /review → P7.3 proactive отдельно; развилки: webhook = in-process route MVP · отдельный pairing-consent с утверждённой формулировкой · журнал = лёгкий bot_action_log + ОБЯЗАТЕЛЬНЫЙ dedup update_id · MVP только command-response) | Полный цикл «/review → ответ → review_log → **строка появляется в ЛОКАЛЬНОМ OPFS-логе** (down-sync, не только API) → Зал видит»; **grader-accuracy gate:** gold-набор ответов (ktiv-пары, огласовки, near-miss) ≥ порога ДО права записи grade; P7.0a acceptance = 10 критериев владельца (annul меняет ТОЛЬКО projection, лог append-only) | 🟡 **P7.0a annul-семантика SHIPPED v3.11.115 (2026-07-06):** спека → adversarial-критика (wf_1bf34023: 3 BLOCKER + 8 MAJOR — все отработаны, адъюдикация в TELEGRAM_P7_DECISION) → двухпроходный fold в fsrs-core + независимом реф-реплее (collectAnnulled по всему списку; ТОЛЬКО review/skip-цели; un-annul не поддерживается); клиентский `clearSrsState` (null-фолд ⇒ carrier удалён / у manual-строки чистятся только srs_*); grade-policy-вызыватели фильтруют withoutAnnulled; LemmaCanon.annulId; ENGINE_VERSION=fsrs6-core-v2 + one-shot client-heal + oracle rebuild на mismatch + oracle.annul_rows; SQL-агрегаты annul-aware (струггалы/last_review_at/lifecycle+constructs). Скоуп: sent:-цели ВНЕ annul (write-path P7.0c отклоняет); минтер-контракт: annul.item_key = item_key цели. Гейты: server-replay 65/65 (golden-v2 ×8, v1 байт-стабилен) · memory-canon 63/63 · agent-plan 32/32 · agent-explain 43/43 · fsrs/learner-graph/mentor-home/grade-policy/cloud-sync/api. **P7.0b грейдер+gold SHIPPED v3.11.116 (2026-07-06):** `agent/grader.js` (deterministic, LLM структурно недостижим; алгебра приёма = байт-паритет с Залом {N(form),strip(N(form))}×{N(ans),strip(N(ans))}; лемма НЕ принимается — lemma-echo эксплойт закрыт, замер: Зал её и не принимал; проклитик-пути = МАРКИРОВАННЫЙ accepted_variant — 20 400 измеренных пар унаследованной дыры; ktiv v1 = near_miss/'ktiv-candidate'; skip=Again(1) без смягчения; empty/unsupported → gradable=false MNAR) + META_ALLOW расширен идентификаторными провенанс-ключами + канал-словарь P7.2 '<семья>:<tg-режим>'. Критика wf_ccbad91d (3 BLOCKER+6 MAJOR, замер 226 834 клеток) — адъюдикация в TELEGRAM_P7_DECISION. Гейт **smoke:grader-gold 58/58** (gold 22/22 · свип позитивы 452/452 / негативы 0 false-accept · D1+P7.0a · провенанс 7 полей). **P7.0c активация record_review_answer SHIPPED v3.11.117 (2026-07-06):** `agent/reviewer.js` (роль reviewer; LLM недостижим) за флагом `AGENT_REVIEW_WRITE` (прод OFF → FEATURE_FLAG_OFF) + endpoint `POST /api/agent/review` (свой лимитер 60/мин) через closed router → ШТАТНЫЙ ingest (learnerLogRepo.ingestBatch, trustedAgentSource) + БЕЗУСЛОВНЫЙ recompute. Спека v1 → adversarial-критика (wf_28ac3c6e, 3 линзы R17-B/R11·R12/R14·R13/R16+харнесс: **4 BLOCKER + 8 MAJOR + 8 MINOR**) → v2 → код. Ключевые фиксы: ОБЯЗАТЕЛЬНЫЙ `attempt_id` (idempotency_key 'agentrev:'+attempt_id — id из серверного reviewed_at давал дубль на каждый ретрай, назвали все 3 линзы); **channel v1 = ТОЛЬКО рецептивные семьи** (production dictate/reverse → reject PRODUCTION_CHANNEL_LOCKED до P7.2 challenge-binding: лемма из /plan + 'dictate:agent' воскрешал lemma-echo→hasProductionSuccess навсегда, D1 отравлен); **ktiv write-gate** (recorded:false — хасер display-форма vs честный ktiv-male ввод = ложный lapse, Зал принимает male-поверхность вхождения, сервера её v1 нет); recompute безусловный (replay-ветка без new_item_keys); **reserved_source** (клиентский agent:-fake reject, echo=dup — провенанс аттестован); annul: reason обязателен + окно 24ч; **crash-window annul-to-null** (syncDown recompute по-странично ДО курсора — первый писатель делал теоретическую дыру P7.0a живой); анти-«тихий 0» + EXPECTED_UNRESOLVED reviewer сам детектит (displayForItemKey фолбэчит сырым ключом). Гейт **smoke:agent-review 66/66** (OFF/ON матрица · штатный ingest+провенанс · attempt_id идемпотентен · production-lock · ktiv-gate · reserved_source · annul byte-restore проекции движко-независимо + 24ч + double-replay + annul-to-null удаляет проекцию · oracle clean · браузер-нога OPFS down-sync «Зал видит» local==replay==server + clearSrsState). Регрессия зелёная (server-replay 65/65 · memory-canon 63/63 · cloud-sync 32/32 · agent-plan/explain/burst/mentor-home/grade-policy/learner-graph/ingest/auth/api). **Privacy РЕШЕНО владельцем 2026-07-07 = (A)** (сырой ответ не персистится). **P7.1a Telegram pairing + channel security SHIPPED v3.11.118 (2026-07-07):** мигр. 027 (channel_links/pairing_tokens/telegram_updates/bot_action_log) + `db/channelLinkRepo.js` (всё под withTxnLock) + `agent/telegram/{router,api}.js` + webhook `/api/telegram/webhook` (secret ДО парсинга — исключён из глоб. bodyParser, свой 256kb; from.id rate → drop-with-200; private-only; dedup+эффект в одной txn + rollback) + `/api/agent/telegram/{pair,status,unlink}` + consent telegram_delivery атомарный каскад + delete-completeness + mentor-home Telegram-блок + i18n ru/en/he. Строго read-only (бот не пишет память; запись = P7.2). Спека v2 = адъюдикация критики wf_a67874c5 (**2 BLOCKER + 11 MAJOR + 4 MINOR**): confused-deputy → ДВУСТОРОННИЙ confirm (owner flow-decision A: redeem→pending, telegram-side /confirm→active, жертва видит чужой аккаунт→cancel); consent-bypass → consent ДО токена + атомарный revoke; secret-before-parse; from.id rate не IP; private-only; dedup-rollback; delete-completeness; server-pinned consent-version; router read-only транзитивно. Гейт **smoke:telegram-pairing 33/33**. **P7.1a OWNER LIVE-VERIFIED на проде 2026-07-07** (двусторонний цикл /start→/confirm→active). **P7.1b read-only content-команды SHIPPED v3.11.119 (2026-07-07):** /plan /explain /due /summary из готовых контуров (agentRuntime.plan/listExplanations/constructsSummary + learnerGraphRepo.getDue); роутер ГЕЙТит (in-txn, транзитивно read-only), agent/telegram/content.js производит ВНЕ txn (импортит server.js, не роутер), format.js (plain-text, hard-wrap 4096, белый список полей, ru/en). Строго read-only w.r.t. review_log. Спека v2 = адъюдикация критики wf_72c44361 (2 BLOCKER + 5 MAJOR): /explain гейтит ЖИВЫМ agent_read_texts fail-closed (класс C, не по purge); delivery-point recheck ПОСЛЕ produce (окно LLM /plan); best-effort 200 + per-command cap; расширенный denylist. Гейт **smoke:telegram-content 15/15**. Webhook уже live → команды работают в боте владельца после деплоя. **P7.2 ЗАКРЫТ ЦЕЛИКОМ:** P7.2a reverse:tg v3.11.120 · P7.2b cloze:tg v3.11.121 · P7.2c dictate:tg v3.11.123 (owner bake 6912 слов, LIVE) · P7.2d premium selector v3.11.124 — все owner live-verified, **AGENT_REVIEW_WRITE=1 ON на проде** (production-запись активна). **P7.3 proactive: P7.3a foundation+due-нудж v3.11.125 + P7.3c adaptive backoff (/notoday /mute RETURN_AFTER_GAP) v3.11.126 SHIPPED — за флагом `AGENT_NUDGE_ENABLED` (деф. OFF, активация = owner-решение). ОСТАТОК: P7.3d premium reason-aware нуджи** (recon `TELEGRAM_P7_3d_REASON_AWARE_RECON_2026_07_09.md`) |
| **CLG-P8** | Telegram Mini App: тренировки + прогресс; **контекст-first** (§8) | Due review в Mini App пишет в тот же лог; **слово показано в исходном предложении** + deep-link в Зал | 🟡 **P8.1–P8.5 SHIPPED owner-live v3.11.130–140** (канон `TELEGRAM_MINI_APP_P8_RECON_2026_07_09.md`: auth initData→scoped cookie · home 4 счётчика · review-сессия 4 production-модальности + read-MC fallback + ahead · manual mode · handoff в Зал). Кросс-поверхностный континуитет и паритет счётчиков — программа R1–R4 `ROOM_DUE_CONTINUITY_2026_07_11.md` (v3.11.143–151, закрыта). **ОСТАТОК: P8.6 ops rollout** (§19 preconditions: токен-ротация, firewall :8000, rollback-drill с MA-событиями, purge-джобы, kill-switch) |
| **CLG-P9** | **Переопределена владельцем 2026-07-06 → «Дом наставника»** (канон-решения: `MENTOR_HOME_P9_DECISION_2026_07_06.md`): полноэкранный вид ВНУТРИ Зала (🤖 в шапке, hash `#mentor`, паттерн roomContent↔roomReader — НЕ страница/drawer, DB-teardown урок v3.11.101) · `public/js/mentor-home.js` — **API-only модуль** (данные ТОЛЬКО из `/api/agent/*`+`/api/auth/me`, никакого OPFS) + **host-adapter** действий (`runTrainer`=startPlanSectionTraining P6.5 · `openReading` · `openTextAt`(text_key,order_index)→scrollToOrderIdx; OPFS-резолв — сторона хоста) = скелет Mini App P8 по построению · MVP все 4 блока: план+действия (переезд из ☁) · история объяснений (`GET /api/agent/explanations` — purge-aware list, tombstone честный, rowid-курсор newest-first) · статус/лимиты + consent agent_read_texts (переезд из ☁; ☁ вернулся к синку/пушу/аккаунту) · зачаток misconception (`GET /api/agent/constructs/summary` — агрегат из facts_used kind='constructs' (purge-aware по построению) + plan-task construct_ids (класс A, добавлены в payload), ⊆ реестра, серверные титулы). Прежний скоуп «agent-кнопки в поверхностях» частично поглощён P6.2/P6.5 (per-row 🤖, ▶-кнопки плана); остаток — Студия-кнопки/token_index — за P10-горизонтом | 380px light/dark, RTL, parity-гейты зелёные; `smoke:mentor-home` (401-gating · user-scoped · purge-aware · ⊆ реестра · MNAR) | 🟢 **SHIPPED v3.11.114 (2026-07-06):** гейт **smoke:mentor-home 25/25** (вкл. user2-изоляцию через direct-DB сессию и «plan-вхождения переживают purge, explanation-вхождения гибнут») · скриншоты @380px light/dark/HE-RTL + Tier-1 честная заглушка «нужен облачный аккаунт» + deep-link `#mentor` с холодного бута ✓ (авто-открытий НЕТ — R17 §2.3; открытие только тап/URL) · регрессия agent-plan 30/30 · agent-explain 42/42 · burst 19/19 · auth 26/26 · learner-graph 14/14 · reader-parity · room-smoke 14/14 · i18n 226 · api-smoke · cloud-sync 32/32 (☁-модал перевёрстан). **Owner live-verify на проде ПРОЙДЕН 2026-07-06 — CLG-P9 ЗАКРЫТА.** Готовность CLG-P7 — `docs/planning/TELEGRAM_P7_READINESS_2026_07_06.md` |
