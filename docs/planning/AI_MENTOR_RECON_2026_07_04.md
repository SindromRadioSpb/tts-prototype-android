...
10. ▶ **СЛЕДУЮЩЕЕ: остаток CLG-P6** — (а) owner live-verify `/explain` на реальном
   профиле (свой текст → синк → 🤖 на строке → first-use confirm → объяснение;
   проверить и revoke-путь: снять галочку → purge-строка в ☁-статусе);
   (б) real-provider burst-verify на /explain (лимит резервируется до вызова,
   prompt не в stdout — §15.8-паттерн слайса 1); (в) misconception construct-id
   субстрат (§7; НЕ вся misconception-система — только стабильные id конструкций,
   чтобы объяснения и будущие ошибки ссылались на одно). Telegram (P7) — не раньше
   гейта G-5.
