...
### P2.1 — owner traceability (v3.11.229)

Production-разбор 2026-07-23 подтвердил не потерю membership, а UX-смешение двух действий:
использованная владельцем ссылка была `LOGIN` для существующего участника, поэтому новая строка
не должна была создаваться. Owner-панель обязана предотвращать такое прочтение:

- primary CTA называется «Пригласить нового участника» и явно обещает отдельный аккаунт;
- сервисное действие строки называется «Ссылка в этот аккаунт» и прямо сообщает, что нового
  участника не создаёт;
- GET `/access` — `private, no-store`, клиент добавляет cache-buster и имеет ручное «Обновить»;
- видны счётчики ACTIVE/REVOKED/pending, время обновления, даты добавления/последнего входа;
- USED/REVOKED/EXPIRED ссылки не исчезают: журнал различает `JOIN` и `LOGIN`, адресата и время.

### P2.2 — intermediate-width UI quality gate (v3.11.230)

Owner-live на окне 510 px выявил дефект, который не поймали проверки 380/1280: auto-колонка
сервисных кнопок сжимала имя участника до одной буквы на строку. Контракт исправлен:

- строка участника всегда одноколоночная: identity/dates занимают полную ширину, действия идут ниже;
- имена переносятся по словам (`break-word`), а не принудительно в любой позиции;
