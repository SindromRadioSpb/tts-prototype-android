...
| 3c | P0 honesty: устранить ложный MADLAD provider-status до feature exposure | ✅ DONE 2026-08-04 — commit `24cc2b54`; server MADLAD fail-closed, client capability не угадывается | Codex | absent/unpaired Companion никогда не показывается ready; red/green tests |
| 3d | Реализовать authenticated Browser→Companion MADLAD MT + model lifecycle + provenance | ✅ ENGINEERING DONE 2026-08-04 — Browser→versioned loopback, exact v2 lifecycle, scheduler, mapping/cancel/retry/provenance; real blank-row defect закрыт | Codex | `623b0e3b`, `cc785906`; текущий scoped commit; без implicit fallback |
| 3e | Engineering gates + invite beta | 🟡 PRODUCTION ACTIVATED / BROWSER PARTIAL — 66 Python, 29 Node, 233 i18n; final `ce811de7` beta.4 lifecycle PASS; served `3.11.308` exposes the explicit invite flow | Codex | real Chrome translate/save/filter/edit/cold-reopen and Companion-off no-fallback PASS; file-picker import + owner-live remain, public distribution closed |
| 3f | Production preflight/deploy + owner-live | 🟡 PREFLIGHT/ACTIVATION PASS / OWNER ACTION PENDING — fresh verified backup; root 92%→76%; `disk_warn=false`; one active `015c0a05` container with `LOCAL_MT_BETA_ENABLED=true`; app/SW `3.11.308` | Codex → owner | owner selects exported JSON for idempotent import, then real learning-text ceremony; do not claim production/owner-live PASS before both |
