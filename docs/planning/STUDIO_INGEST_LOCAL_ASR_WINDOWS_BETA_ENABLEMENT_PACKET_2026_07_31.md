...
The owner-reported UI holes are fixed in local web candidate `3.11.279`, latest code commit
`1046686694f76afdb4d53fa0766274d98607ca3a` (building on `0bdf4529`):
...
- its companion/model result is rendered immediately after the `127.0.0.1` privacy hint inside
  the Local pairing block, instead of the unrelated modal-wide import/job status footer;
- pairing failures use the same proximal result region, while the modal-wide footer remains
  available for file-import/job status;
...
surfaces. The earlier full-panel run measured `365 == 365`; the onboarding panel `348 == 348`; the
import dialog `365 == 365`. The `3.11.279` follow-up also passed RU/LTR and HE/RTL at 380×844 with
the proximal status visible, the global import status empty, and no horizontal overflow or Gemini
