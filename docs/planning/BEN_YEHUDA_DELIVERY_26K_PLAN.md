...
- **A3 — клиент:** ✅ **SHIPPED+PROD 2026-06-10** (commits `b992fba`+`4db4358`, SW `v3.10.22-corpus-search`).
  Вариант-2 IA (Период→Автор→Работа) + рейл «Готовы к чтению». **Slice 1** = drill период→автор→работа
  (сетка периодов с датой/аннотацией/счётчиками · lean-градуированный список авторов · секции
  «Готовы»/«перевод позже» + датчик объёма + провенанс · baked ▶ openable / unprocessed ⏳ disabled).
  **Slice 2** = глобальный поиск (niqqud-insensitive) + фасеты (Готовые/жанр/язык со счётчиками) +
  L2 сорт градуир.⇄алфавит + ивритский א–ת jump-bar. Данные: `corpus-index-v3.json` (author-index/
  ready/facets) + `corpus-search-v3.json` (плоский, ленивый). Мобайл-бюджет = root + сайдкар + 1 блок
  автора. Гейты `smoke:corpus-nav` 33/33 + `smoke:corpus-room` 18/18. **index.html не тронут.**
  Прод-верифи: Android (Pixel 7) drill+search открывают reader; iOS — тот же OPFS-путь, что у канона.
- **A5 — fill-queue/дашборд покрытия** + таргетированные прогоны. ⏳ **NEXT** (done 100 / pending 24 541).
