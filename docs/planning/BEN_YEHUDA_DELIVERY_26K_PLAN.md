...
- **A0 — ИССЛЕДОВАНИЕ+ПЛАН (next session, ДО кода):** разобрать (1) исходный git-репо
  `projectbenyehuda/public_domain_dump` (структура каталога/метаданных/путей, что ещё извлекаемо: жанры,
  теги, серии, годы) и (2) IA сайта-аналога **https://benyehuda.org/** — навигация автор/жанр/период,
  устройство списков ДО входа в карточку. **Цель:** предложить премиальные варианты IA/структуры Зала
  (возможно перенять структуру до входа в карточку benyehuda.org) — варианты по ролям R4/R6/R8 +
  рекомендация, ЧТОБЫ НЕ ПЕРЕДЕЛЫВАТЬ в конце. Решить granularity шардинга здесь. Выход: обновлённый
  раздел IA в этом плане + согласие владельца → потом A4/A2/A3. **A0 ПЕРВЫМ.**
...
- **A4 — миграция works → прод-том** (R5, предусловие 26K; owner-token push-эндпоинт — паттерн P0-010).
  Идёт ПЕРЕД A2, чтобы не плодить тысячи loose-файлов в git (рекоменд. порядок владельца).
