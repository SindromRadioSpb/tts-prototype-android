...
Effective Gemini API terms verified on 2026-07-16 state that unpaid-service content may be used to improve Google products and restrict API-client use. On 2026-07-16 the owner explicitly classified the current deployment as owner-only testing by one adult developer, confirmed that it has not been shared with other users, and consented to Google using the owner's personal-library inputs under the applicable unpaid-service terms. Within that boundary, the owner authorized managed production lesson composition with `gemini-3.1-flash-lite`, capped at 300 RPD and 9 RPM, followed by the deterministic safe plan. The detailed validator remains the publication authority; the offline shadow critic receives no learner-visible authority. This authorization must be revisited before access is offered to any other user or the data/terms boundary changes. No provider key is committed.
