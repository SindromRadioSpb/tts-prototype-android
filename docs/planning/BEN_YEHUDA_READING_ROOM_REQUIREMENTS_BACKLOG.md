...
### BRR-P1-007 — Coverage-aware каталог: фильтры/поиск + бейдж покрытия + «следующий для тебя» ✅ A1–A4 + A3-клиент (Slice 1+2) SHIPPED+PROD 2026-06-10 · A5 NEXT
...
- **Current:** ✅ **A3-клиент SHIPPED+PROD 2026-06-10** (SW `v3.10.22-corpus-search`): Корпус-вкладка листит ВЕСЬ перечень (26 455 coverage-aware, не только baked-100) через Период→Автор→Работа drill на каталоге v3; coverage-бейджи (⚙ машинный/🔇 без озвучки), честный «перевод позже» (unprocessed не openable); глобальный поиск (niqqud-insensitive) + фасеты Готовые/жанр/язык; L2 ивритский jump-bar. Данные `corpus-index-v3.json`+`corpus-search-v3.json`; гейты `smoke:corpus-nav` 33/33. Остаётся для v1.1: фасет «озвучка» (когда появится corpus-audio), within-era фасеты, BYOK «перевод на лету», «next»=i+1 рекомендация. **A5 (fill-queue/дашборд покрытия для владельца) — NEXT.**
...
### BRR-P1-008d — Word-karaoke в **Studio** (построчно), само-кеш ✅ SHIPPED+PROD (2026-06-14, SW v3.10.55, владелец подтвердил) — *surface: Studio, не Зал*
- **SHIPPED 2026-06-14:** перенос «бегущего слова» из Зала в Studio (`index.html`), ТОЛЬКО построчно (решение владельца).
  ...
  `docs/planning/BRR_P1_008D_STUDIO_WORD_KARAOKE_2026_06_14.md`. **✅ Прод-верифицирован владельцем 2026-06-14.**
