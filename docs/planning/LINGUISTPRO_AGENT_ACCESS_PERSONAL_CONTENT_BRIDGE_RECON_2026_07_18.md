...
Дата: 2026-07-18 · Статус: **PROPOSAL** (решение принимает владелец) · Сессия: research-only, кода нет. Адверсариальная критика проведена (§7), правки внесены.
...
- **S0 — замер (владелец):** снипеты §3; заодно проверить фактический статус consent `cloud_texts` и свежесть артефактов.
...
1. **Замер:** прогнать снипеты §3 (A — OPFS, B — реплика, C — ops) и вернуть цифры.
2. **Prerequisites:** подтвердить P1 (коррекция consent-copy `cloud_texts` + класс C §2.5) и P2 (delete-семантика артефактов: полный фикс или минимум-вариант «честный каталог»).
3. **Грануляция:** начать с per-text грантов (S2) и добавить standing `*` (S3) позже — ок? Или сразу standing?
4. **Single-parser доктрина:** разрешить PUT-time экстракцию bounded-метаданных (title/rows_count) в sidecar до агентских согласий — или lazy-индекс только при выданном scope?
5. **②-заметки:** ждать S4-пакета или включить metadata-уровень заметок раньше?
6. **Over-carry §2.3 (структурная часть):** заказать слим-бандл/`state_bundle` отдельным пакетом (consent-copy закрывается в P1 независимо)?
