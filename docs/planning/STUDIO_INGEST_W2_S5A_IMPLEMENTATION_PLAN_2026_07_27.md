...
⚠ **`merge:false` — режим сырого разбора, капы в нём НЕ применяются.** Причина: капы существуют
ради контракта `ingest/segTable.js` — они ограничивают то, что мы ОТПРАВЛЯЕМ на сервер. В режиме
`merge:false` ничего не отправляется, это диагностический путь для оракульного гейта, которому
нужно сверить все реплики файла (у ручной фикстуры их 411 — больше капа). Продуктовый путь всегда
идёт со слиянием и капы получает. Клиентский гейт `v3AudioSegmentsForRequest` и сервер проверяют
размер независимо, так что обхода капа этим не открывается.

То есть `finish()` принимает признак «применять ли капы»: при `merge:false` пропускать проверки
`MAX_SEGMENTS`/`MAX_SEG_TEXT`, всё остальное (плотный `i`, `CAPTIONS_EMPTY` на нуле реплик) —
...
Плюс три проверки поведения плеера, которые автоматический смоук поймать не может (перечислены
ревью задачи 6 — Step 3B задачи 10 меряет только конечное состояние через 2,5 с):

4. **Буферизация не гасит подсветку.** Дождаться реальной паузы на догрузке (или притормозить сеть
   в DevTools) — строка обязана остаться подсвеченной, а не «слететь». Это и есть смысл правки
   `paused`, которая теперь возвращает `false` во время буферизации.
5. **Задержка событий.** Нажать паузу и снова воспроизведение — подсветка обязана останавливаться и
   возобновляться без заметного глазу отставания. События YouTube приходят асинхронно, и это
   единственное место, где видно, насколько.
6. **Пауза родными кнопками YouTube.** Поставить ролик на паузу его собственным контролом, а не
   кнопкой Студии: подсветка обязана замереть. Это проверяет, что событие доходит до модуля и
   останавливает rAF-цикл, а не только наш собственный путь.
