...
### BRR-P2-004 Фильтр/сорт — ✅ SHIPPED (Stage 4, SW v3.10.59)
- **L3-работы сорт+фильтр** (был пробел: на L3 контролов не было): сегмент-контрол сорта **По порядку / По алфавиту /
  По длине** (`corpusWorkComparator`, паттерн L2-`.corpus-sort`) + **genre-`<select>`** из жанров текущего автора (скрыт при ≤1).
  Перекраска **на месте** над уже загруженными works (`paint()`, без ре-фетча); ready/later-сплит сохранён; genre-фильтр
  сбрасывается на смену автора, сорт сохраняется. i18n `room.corpus.sort.{graded,length}`. @380px + e2e (сорт меняет порядок).
- L1-результаты сорт (relevance/coverage) — отложено (L1 уже имеет богатый фильтр-бар; coverage-сорт требует анти-stampede
  снапшота). Не блокер: основной пробел (L3) закрыт.
- *(В этом же слайсе — fix доставки Stage 3: lemma-индекс шардирован по бюджету размера `lemma-<i>-v<N>.json` (один файл 19MB
  перерастал upload-cap и рос с корпусом); клиент `ensureLemma` грузит все шарды и мёржит; манифест `lemma_files[]`.)*
...
## ⏳ ОТКРЫТЫЕ ПУНКТЫ — вернуться ПОСЛЕ смоук-чека владельца

> Якорь для возобновления. Развилки/контекст — выше (§Жёсткие факты, §Стадии, §Риски). Сначала владелец гоняет смоук-чек
> на проде → баги/недоработки логируем в таблицу ниже → чиним (тест→fix→тест) → затем берём NEXT.

### A. Сознательно отложено / известные неполноты (НЕ баги, by design)
- **FTS-покрытие 10 228 / 26 455 (38.7%)** — *настоящий* поиск внутри проиндексированных; растёт итеративно:
  `npm run fetch:corpus-bodies -- --limit N` → `npm run build:corpus-fts` → бамп `FTS_DATA_REV`(library-ui)+`CACHE_VERSION`(sw) →
  `AUDIO_UPLOAD_TOKEN=… npm run push:corpus-fts` → commit манифест+sw+library-ui → prod-verify (Node fetch). ~16K тел осталось.
- **non-ready FTS-хиты** — честный «перевод позже» (открыть нельзя); открываемость растёт бейком ③.
- **Омографы** — офлайн form-first даёт разные pid (מלך сущ vs глаг); поле помечено «по форме слова» (R10). Истинная
  контекст-дизамбигуация — отдельный трек ④ (Dicta-silver).
- **L1-результаты-сорт** (relevance/coverage/era) — отложен (L1 уже имеет фильтр-бар; coverage-сорт требует анти-stampede
  снапшота, не per-card). Подключать в `renderResultsInto`/`appendPagedWorkRows`.
- **Поиск закладок** в общей results-поверхности — `searchBookmarks` готов в `local-db.js`, UI-группу подключить рядом с FTS-группой.
- **HE-i18n** `room.resume.*`/`room.bookmark.*`/`room.corpus.search.*` — best-effort, на native-review.

### B. Журнал смоук-чека (пополнять по ходу; «это часть багов», добавлять новые)
| # | Что не так / недоработка | Где | Серьёзн. | Статус |
|---|---|---|---|---|
| 1 | «Продолжить»: сыграл не-последнюю строку и вышел → карточка исчезла (close-flush писал top-row=0, перезатирая позицию) | ① `library-ui.js closeReader` | high | ✅ fixed BRR-P2-005 (furthest-max `_sessionMaxRow`/`mergeProgress`; gate +9) |
| 2 | Resume: при переходе к строке нет визуального выделения строки-цели | ①.2 `scrollToReaderRow` | med | ✅ fixed BRR-P2-005 (`.rm-row-jump` амбер заливка+полоса, держится до взаимодействия) |
| 3 | Закладка: открытие закладки — нет выделения строки | ②.1 `scrollToSentence` | med | ✅ fixed BRR-P2-005 (тот же `.rm-row-jump`) |
| 4 | FTS: проваливание в хит открывает работу СВЕРХУ, не на совпавшей строке, без выделения | ③.1 `appendFtsGroup`/`openCorpusWork` | med | ✅ fixed BRR-P2-005 (`firstMatchRow`→jump+highlight; gate +6) |
| 5 | Требование: премиум reading-UX (Studio+конкуренты) | треб.4 | req | ✅ док `BRR_READING_UX_REQUIREMENTS_2026_06_15.md` (P0 done, P1/P2 беклог) |
| 6 | После воспроизведения строки она не остаётся подсвеченной (нужен маркер «здесь остановился») | ① reader `onKaraokeRowChange` | med | ✅ fixed BRR-P2-005.2 (highlightReaderRow на старте play; маскируется синим .row-playing, амбер проявляется после; держится до взаимодействия) |
| 7 | Поиск по корпусу → проваливание в текст: нет подсветки строки (title-хит не передавал запрос; firstMatchRow был потокенный, не ловил подстроку «שחר»∈«שחרור») | ③ `renderResultsInto`/`firstMatchRow` | med | ✅ fixed BRR-P2-005.2 (ftsQuery протянут и в title-группу; firstMatchRow + подстрочный скан) |
| 8 | Искал ФРАЗУ «אתם כבר יודעים» → 2399 шумных «по форме слова», конкретная строка зарыта (FTS был ПОСЛОВНЫЙ AND, не фразовый; маркер «по форме слова» горел на все контентные) | ③ `appendFtsGroup`/`corpus-fts` | **high** | ✅ fixed **BRR-P2-006** (позиционный фразовый поиск; «מה הם עושים» — единственный phrase-хит сверху; маркер снят) |
| 9 | Вставил строку «לו אך ככוכב הנופל בנשפים» → «0» без признаков работы ~18с, потом внезапно результат (нашёл «לְמִרְיָם» — верно). Нет индикатора загрузки; холодная вставка тянула весь индекс разом; гонка запросов | ③ `appendFtsGroup`/`renderResultsInto`/`corpus-fts`/`build-corpus-fts` | **high** | ✅ fixed **BRR-P2-006a** (индикатор «Ищем в текстах…» + анти-гонка `corpusFtsSeq` + прогрев на загрузке + 2-уровневое под-шардирование тяжёлых букв) |

> **BRR-P2-006a** (SW v3.10.63-fts-fast) — поиск-по-фразе: обратная связь + скорость. **(A)** Индикатор `«🔎 Ищем в текстах…»`
> (спиннер, `role=status`) в `appendFtsGroup` сразу до await; empty-state не во время загрузки; **анти-гонка** `corpusFtsSeq`
> (устаревший запрос не дорисовывается; body переиспользуется in-place). **(B-прогрев)** `CorpusFTS.warm()` (манифест+lemma+
> lemmamap ~6.5MB) в `requestIdleCallback` при загрузке Зала; `warmQuery()` греет шарды по `input` (вставка → все сразу);
> `ensureShard` single-flight. **(B-структура)** ТЯЖЁЛЫЕ буквы (`sharded_letters`: 14 шт.) шардированы по ПЕРВЫМ ДВУМ буквам
> (`ex-<L1L2>`) → «הנופל» грузит крошечный `ex-הנ` (~0.6MB) вместо всех 11MB буквы ה. Клиент `shardKeyFor`/`ensureShard` по
> префиксу; манифест `bucket_files`(prefix)+`sharded_letters`; билд `--bucket-2level-threshold` (деф ~3MB raw). **Замер
> (вставка-6-слов):** exact-шарды 30MB→**1.34MB** (22×); первый холод ~37MB→~7.8MB; warm/повтор — **313–932мс** (был ~18с).
> 334 шарда <10MB на ТОМЕ (ре-пуш ДО деплоя — новые префикс-имена не конфликтуют с живым манифестом, окна 404 нет). Гейты
> `smoke:corpus-fts` 48/0 + `-parity` 30/0 (`shardKeyFor`/prefix-манифест/детерм). Прод-верифи (Node+браузер @380px, свежий
> код): индикатор виден, анти-гонка держит, «לְמִרְיָם» сверху, 0 console-errors. **Остаточный холод-пол = 6.5MB lemma**
> (P2-беклог: phrase-группа из одних префикс-шардов ДО lemma — прогрессивно).

> **BRR-P2-006** (SW v3.10.62-fts-phrase) — ПОЗИЦИОННЫЙ фразовый full-text «найти строку/фразу». **Двухслойный индекс под мобайл:**
> `lemma` = COUNT-only (pid, мал, грузится всегда ~6.5MB — питает word-AND и группу «слова в тексте», БЕЗ изменения footprint
> от v7) · `exact` = ПОЗИЦИОННЫЙ skeleton-индекс по ВСЕМ токенам, шардирован по первой букве + под-шардам ≤8MB (`ex-<L>[-<i>]`),
> грузится лениво только для букв запроса. Фраза = смежные offset'ы (`phraseHit`, slop) по exact-позициям → «точная фраза» =
> точный консонантный run. UI: две под-группы **«🔎 Точная фраза (N)»** (сверху) + **«Слова в тексте (M)»**; вводящий маркер
> «по форме слова» СНЯТ. Drill-in на строку фразы (`firstPhraseRow`→jump-highlight). `MAX_POS=16` (фраза-цель @слово 12 в покрытии).
> Замер: always-load 6.5MB не вырос; phrase-запрос ленив ~9–15MB типично (heavy-буквы ה/ו/ב кэшируются; sub-shard по 2-й букве —
> P2-беклог). Индекс 75.9MB gz @10.2K на ТОМЕ (37 шардов <10MB upload-cap). Гейты `smoke:corpus-fts` 39/0 + `-parity` 26/0
> (decodePositions/phraseHit/firstPhraseRow/MAX_POS-cap/детерминизм). Node+браузер(@380px, проба свежести кода): запрос
> «אתם כבר יודעים» → ровно **1** phrase-хит «מה הם עושים» (ready) сверху, 2398 scattered ниже, 0 console-errors. index.html не тронут.

> **BRR-P2-005** (SW v3.10.60-room-jumphighlight) — fix-блок находок 1–5 выше: furthest-max «Продолжить» + единый
> jump-highlight `.rm-row-jump` (resume/закладка/FTS) + FTS-открытие на совпавшей строке. Гейты `smoke:reader-resume` 31/0,
> `smoke:corpus-fts` 17/0. @380px light+dark + e2e (баг ① БД-подтверждён; 3 jump-пути; 0 console-errors).
> Премиум-планка + беклог P1/P2 → `docs/planning/BRR_READING_UX_REQUIREMENTS_2026_06_15.md`.

### C. NEXT — меню после стабилизации (выбор владельца)
- ★ **ПРОДУКТОВОЕ ЗАКРЫТИЕ БЛОКА ПОИСКА (S1–S19) — УТВЕРЖДЕНО владельцем 2026-06-16, реализация в НОВОЙ сессии одним
  заходом.** Канон требований + промт → **`docs/planning/BRR_SEARCH_DISCOVERY_STATE_2026_06_16.md`**. P0 сниппет/подсветка/
  прогрессивная-фраза/инпут/релевантность → P1 readability-поиск/KWIC/поиск-по-корню/поиск→Anki → P2 scoped/история/списки/related/in-reader-find/фильтры.
- ④ **R10-качество** тап-глосса vs Dicta-silver + бейджи провенанса + **47097 идиш** (R6/R7).
- ⑤ **Anki-sync** (real mastery → строгая i+1 80–95%).
- **Рост FTS-покрытия** к полным 26K (см. A — дофетч+ребилд+пуш, итеративно).
- ③ **publish бейкнутых → каталог v8** (зависит от ротации токена) — растит открываемость FTS-хитов.

🔑 **Предусловие публикации репо:** ротация `AUDIO_UPLOAD_TOKEN` (засвечен в чате) + Gemini + старый GCP.
