...
> **СУПЕРСЕДЕД этапом 1 двухрежимности (решение владельца 2026-07-19, v3.11.220 39ed250):** подавление и cancel-on-issue СНЯТЫ; личнотекстовые cloze выпускаются всегда; честность = провенанс `agent_exposed` (мигр. 052: mint-флаг + OR-проверка activeGrant на грейде → meta.agent_exposed в review_log). Грейд/расписание не тронуты — D1-взвешивание (успех мягче / провал полновесен) решается позже ПО ДАННЫМ. Аргументация — диалог с владельцем: цена запрета постоянна, загрязнение вероятностно-редко, FSRS самокорректируется, подсказанным бывает только успех.
