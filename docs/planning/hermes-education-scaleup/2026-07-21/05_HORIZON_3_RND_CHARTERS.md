...
Каждый чартер — отдельное исследование с owner go (Д6), собственными порогами успеха/провала и
stop-условиями. Выход чартера — evidence-отчёт + рекомендация, не код в проде. Провал — валидный
...
| Prerequisites | H2 closed; get_text_coverage жив (выбор целевых слов детерминирован) |
...
| Prerequisites | H2 closed; накопленные случаи «агент объяснил хуже, чем моя заметка» (собирать в H1.8/H2.7 evidence) |
...
идентификатором чартера и owner go; никакой чартер не наследует go другого.
