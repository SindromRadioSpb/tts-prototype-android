...
## Rollback
Capability можно отключить (убрать запись/скоуп) без влияния на остальные 16; деплой отката —
revert-коммит. Owner-approved correction 2026-07-23: migration 054 расширяет закрытый SQL CHECK
`agent_connection_grants.scope` с 15 до 16 значений, иначе гранулярный `morphology.read` невозможно
выдать. Расширенный CHECK остаётся rollback-совместимым надмножеством для старого сервера.
