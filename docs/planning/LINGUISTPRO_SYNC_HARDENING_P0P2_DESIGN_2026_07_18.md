...
### 6.10b Live-инцидент воскрешения (2026-07-18, найден live-verify владельца; ФИКС v3.11.215 e3ef955)

Сценарий владельца: удалил текст на ПК (82/82) → айфон 83/82 → ПК 83/83 (текст воскрес). Audit-диагноз: consent общий на аккаунт → на втором устройстве галочка уже стояла; тапы по ней дали три пары revoke→grant с интервалом 0,5–0,9 с (19:33/19:36/19:40), каждый revoke = purge всех 84 артефактов; drain delete-интента (19:41:01) попал в окно ПОСЛЕ purge → артефакта нет → правило «tombstone только при changes>0» (анти-мусор F2-8) дало no-op → волна перезаливки второго устройства (19:42, полluted-штампы 15:45:xx — след старого кода до P0) легально воскресила текст. LWW/tombstone-механика не ломалась — сломалась комбинация «purge по toggle» × «tombstone требует существования».

Фикс: (1) `deleteArtifact` ставит tombstone ВСЕГДА (кроме DELETED_OLDER); анти-мусор — кап 2000/пользователя с прюнингом старейших; (2) снятие галочки в ☁-модале требует подтверждения (confirm: «облачные копии будут удалены немедленно», ru/en/he); (3) `smoke:sync-slim` 50/50 (+сцена: удаление в purge-окне → tombstone существует, второе устройство применяет удаление, не воскрешает). Урок в копилку: **иерархию «согласие ↔ данные» нельзя вешать на один и тот же чекбокс без фрикции, если снятие деструктивно**; и tombstone — маркер ИНТЕНТА пользователя, а не производная от наличия строки.
