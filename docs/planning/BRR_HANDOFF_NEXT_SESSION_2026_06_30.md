...
Все три = **v3.11.55** сейчас; следующий шип = `3.11.56` / `v3.11.56`.
...
**GAP / Step-0:** ✅ СДЕЛАНО 2026-06-30 — recon-док создан: `docs/planning/BRR_EPIC6_CURATED_LIBRARY_2026_06_30.md` (current-state + Wave-split + инварианты, заземлён на живой код). **Wave-1 W1-a SHIPPED v3.11.56** (reader-byline + source-attribution, закрывает per-work-attribution P1). **Остаётся Wave-1:** W1-b card-attribution (⚠ clutter-риск vs PC-2 de-noise — НЕ авто-шипать, owner-judgment) · W1-c author-row QID-link (era-chip на author-row не тривиален: era per-work) · W1-d roadmap/moat микрокопи (owner-eyes на формулировки). **Wave-2 (defer, sign-off):** author bios/life-dates, editorial-entry-points/literary-reading-order.
