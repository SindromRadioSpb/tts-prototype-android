...
- **Прод:** `https://linguistpro.kolosei.com` · живёт **v3.11.55** · git **HEAD `d811c83`** на `main` · рабочее дерево чистое (кроме служебного `.remember/remember.md`).
...
**GAP / Step-0:** **отдельного Epic-6 recon-дока НЕТ** — только §ЭПИК 6 в аудите. **Создать `docs/planning/BRR_EPIC6_*.md`** (measure-before-code recon) ДО кода: findings аудита предшествуют этой сессии (PC-9 уже добавил tappable author-drill на rail-карточках) → перепроверь, какие findings ещё открыты против живого `library-ui.js`.
