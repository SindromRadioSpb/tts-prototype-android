...
- **Прод:** `https://linguistpro.kolosei.com` · живёт **v3.11.65** · git **HEAD `4e53c72`** на `main` · рабочее дерево чистое (кроме служебного `.remember/remember.md`).
  - **2026-07-01 (🔬 PROCLITIC Phase-2 — gold R1-ВЫВЕРЕН + детектор ЗАМЕРЕН; ЧЕКПОЙНТ, NEXT=Phase-3 bake-Dicta; КОДА В ПРОДЕ НЕТ). 🟢🟢 ОТКРОЙ `BRR_PROCLITIC_PHASE2_RECON_2026_07_01.md §9` — самодостаточная точка входа.** Состояние: Phase-0 (3b store) ✅ · **Phase-1 gold ЗАМОРОЖЕН+R1-выверен владельцем** (`docs/research/epic-proclitic-phase2/2026-07-01/gold-frozen.json` 332 строки 251проклит/81«-»; manual/ + R1-notes) · **офлайн FSA-детектор замерен** (`detector-proto-v3.js`+`DETECTOR_FINDINGS.md`): existence-prec **94.9%**/labeled-seg **94.6%**, do-no-harm ПРОХОДИТ (0 ложных на именах/фоссилиях; בית/משה/מורה воздерживаются), слитн.артикль из огласовки работает; **3 FP + low-recall = покрытие Pealim** (не алгоритм). **NEXT (Phase-3, владелец авторизовал): bake-time Dicta per-work overlay** (прогон Dicta по baked-работам→карта имён+сегментации→авторитетный overlay→≥99) → затем pure-модуль `proclitic-segment.js` + гейт `smoke:reader-proclitic` + рендер вес-обратно-частоте. Решения зафиксированы (gold-first·R12→R10·вес-обратно-частоте·bake-Dicta). Владелец выбрал чекпойнт (Вариант B) → новая сессия. Полный план+алгоритм+файлы+next в recon §9. владелец → Phase-2 проклитик (тап בַּבַּיִת → показать ב БЕЗ ломки בַּיִת). **Это задача СЕГМЕНТАЦИИ.** Measure-before-code: замер vs Dicta-silver (наивный Pealim 90.7%/70.1%, слоёный guard ~97%/70%; FP-класс = имена משה→מ+שה, нарратив-вав ויאמר, фоссилии) → **5-линзовая адверсариальная роль-критика** (R11/R10/R1/R4/R2-R5, все FIX-FIRST). **Дизайн:** двухуровневый — офлайн FSA-грамматика приставок (`ו?→{ש|כש|לכש|מש}?→ה?→один из ב/ל/כ/מ`, воздержание ничья/глубина≥3) + abstain-стоплист (фоссилии/вав/архаика) + **bake-time Dicta per-work** (убивает имена) + рендер вес-обратно-частоте; метрика РАЗДЕЛЬНАЯ existence≥99/labeled-seg≥95/recall-негейтящий vs **НЕЗАВИСИМЫЙ gold**. **Решения владельца (AskUserQuestion):** gold-first · **R12 свёрнут в R10** (+R11 tripwire, канон обновлён) · вес-обратно-частоте · **bake-time Dicta авторизован**. **Phase-1 ЗАШИПЛЕН (`6e27c19`):** `build:proclitic-gold` → `gold-worksheet.tsv` (332 строки, стратифиц. имена/вав/фоссилии/контент + засеяны хард-негативы בית/משה/מורה→«-» и каноничный וַיֹּאמֶר) + `gold-LEGEND.md` + `gold-frozen.json`. **🔵 СЛЕДУЮЩЕЕ: владелец/R1 выверяет R1_VERDICT в worksheet → frozen gold → строю Phase-2 офлайн-детектор + гейт `smoke:reader-proclitic`.** Recon `BRR_PROCLITIC_PHASE2_RECON_2026_07_01.md`; evidence+критики `docs/research/epic-proclitic-phase2/2026-07-01/`.
  - **2026-06-30 (🐞 3b fix + склонение, v3.11.65 `4e53c72`): корректная Pealim-ссылка + таблица склонения служебных слов.** БАГ (нашёл владелец): тап אֶל «к» → ссылка вела на 5261 `אַל` «не» (гомограф). Причина: функц-линк-карта выбирала id по niqqud-stripped поверхности, без учёта смысла. Анализ (офлайн-датасет 9279 парадигм): у אל 3 смысла — **2682 אֶל «к» (с 10-клеточной местоим. парадигмой)**, 5261 אַל «не», 7560 אֵל «бог»; билдер брал 5261. Из 25 самостоятельных **10 несут полное местоим. склонение** в датасете (את על של אל עם אין עוד מן יש בין). **Фикс (объединяет баг+фичу):** в каждую usage-запись запечён ВЕРИФИЦИРОВАННЫЙ sense-id (`build:fn-usage-pealim`, идемпотентно) → reader-morph берёт его для (а) правильной dict-ссылки И (б) для склоняемых частиц — полной таблицы Pealim-склонения + tap-to-speak (та же машинерия, что глаголы/сущ.). זו/כל (смысла нет в датасете) → честный Pealim-поиск, без гомограф-deeplink. Верификация ДВУМЯ независимыми источниками: детерм. ре-деривация из датасета + кросс-чек против 3b-curated suffix_series (17 совпали, 3 — благ. суплетивы/длина). Гейт `smoke:function-usage` **679** (+ регрессия אל→2682 + per-id no-homograph oracle + declension-cells). reader-morph smoke обновлён (אֵין теперь несёт СВОЁ curated-склонение, не чужую таблицу). Все reader-гейты+parity. 380px light/dark премиум (ссылка→dict/2682, полные таблицы שֶׁלִּי…/אֵלַי…). Store rev→2 (cache-bust). Producer `scripts/premium/build-function-usage-pealim.js`.
  - **2026-06-30 (🎉 ЭПИК 3b SHIPPED, v3.11.64 `9ef819c`): тап по служебному слову → курируемая секция «Употребление».** Sign-off получен (владелец одобрил DRAFT 32 записей; местоимения отложены отдельной фичей). Пайплайн: corpus-frequency producer (`build:fn-freq`, 796 работ/556K токенов → порядок батча) → full-depth авторинг (workflow author→adversarial-R1-verify: 27 ok, 2 авто-фикс אך/או) → independent R1-обзор (16/16 суффикс-серий стандартны, 0 «падежей») → live-store `public/data/usage/function-usage.v1.json` (32 записи curator=owner) + лоадер `function-usage.js` → `usageHtml` секция в reader-morph (role·governs·suffix-chips·position·collocations·pitfalls·register·examples) + **honesty-гейт** (functionWord && !Dicta-disputed → тап עד как «свидетель» прячет usage) → i18n `room.morph.usage.*`×3 + `.rm-usage` CSS (theme-var) → SW precache + триада v3.11.64. Гейт `smoke:function-usage` **585** (схема+R1/R9+**независимый reachability-оракул из FUNCTION_GLOSS**). 380px light/dark премиум, prod-verified. **Reachable: 25 самостоятельных; PHASE-2: 7 проклитик ה ב ל ו מ כ ש** (запись в store готова; нужен `card.proclitic`-экспорт резолвера, чтобы показывать ב при тапе по בַּבַּיִת). Артефакты: `docs/research/epic3b-function-usage/2026-06-30/` (frequency + DRAFT + README). Recon `BRR_EPIC3B_FUNCTION_USAGE_RECON_2026_06_30.md` (SHIPPED-шапка).
  - **2026-06-30 (3b recon + W1-d, v3.11.63 `785b62c`):** **(а) 3b «Употребление служебных слов» — RECON доставлен, ждёт sign-off владельца** (`docs/planning/BRR_EPIC3B_FUNCTION_USAGE_RECON_2026_06_30.md` + DRAFT-сэмпл של/את/ב в `docs/research/epic3b-function-usage/2026-06-30/draft-function-usage.json`, commit `370c622`). Gap подтверждён (служебные слова не имеют usage-карточки). **R1-инвариант: 3b-КОД (loader+render+гейт+`function-usage.v1.json`) НЕ может лечь до утверждения контента владельцем** — §8 гейтинг-вопросы (глубина схемы · авторинг · частотный порядок батча · флаг регистра · число примеров) на решении владельца. **(б) W1-d дешёвый roadmap/moat-копи ЗАШИПЛЕН:** под секцией «В каталоге · перевод позже» — note `room.corpus.laterRoadmap` («Перевод и огласовка добавляются партиями…»); About-копи `room.about.offline` усилена (назван offline-моат); `.corpus-section-note` CSS. i18n 226/0, corpus-room 18/18, 380px light/dark, prod-verified live.
  - **2026-06-30 (D7.1 discoverability, v3.11.61→62 `4b667b8`/`7d61a88`): heatmap findable.** Владелец не нашёл календарь (был спрятан в итогах тренировки). Вынесен: всегда-видимая 📅 в шапке «📚 Учить» (`.room-study-cal`) + тапабельный streak-group на due-бейдже (home+study-sheet) + train-summary streak — ВСЕ открывают единый bottom-sheet `openStudyHeatmap` (mirror openFinishedAllSheet; reusable `buildHeatmapGrid`; honest empty-state на activeDays===0; re-entrancy guard в try/catch). i18n `heatTitle`/`heatEmpty`×3. Adversarial-review 0 Crit/High; v3.11.62 фикс MEDIUM (Escape над heatmap не закрывает study-sheet под ним — guard на `.list-picker-ov`) + dead-CSS. 380px light/dark, prod-verified.
  - **2026-06-30 (T6 applied, v3.11.60 `6a43f5f`): 11 одобренных биографий канон-авторов ЖИВЫ.** Перенесены из draft в `corpus-editorial-v1.json` (curator=owner), `CORPUS_AUTHORS_DATA_REV`→2, prod-verified рендер на author-landing (Bialik: «Национальный поэт ивритского возрождения» + bio). **Build-once payoff доказан: data-only правка → премиум author-page, zero UI.** Гейт `smoke:corpus-editorial` 20/20 (store non-empty: curators present R9 + every editorial-QID→node «no orphan bio»). **Следующие биографии = тот же цикл: правка `corpus-editorial-v1.json` + бамп DATA_REV + гейты + deploy.** Исключённые (политические Штерн/Файнберг + нишевые) — на авторинг владельца.
