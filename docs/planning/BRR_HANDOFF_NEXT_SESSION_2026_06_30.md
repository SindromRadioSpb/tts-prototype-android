...
- **Прод:** `https://linguistpro.kolosei.com` · живёт **v3.11.57** · git **HEAD `881dfdd`** на `main` · рабочее дерево чистое (кроме служебного `.remember/remember.md`).
...
Все три = **v3.11.57** сейчас; следующий шип = `3.11.58` / `v3.11.58`. (Новые гейты Эпика 6: `smoke:corpus-authority`, `smoke:corpus-editorial`; producer `npm run build:author-nodes` после правок `authorNodes.js` или индекса.)
...
**GAP / Step-0:** ✅ ЗАВЕРШЕНО. Recon + **build-once АРХИТЕКТУРА (квартет) ЗАШИПЛЕНА** (Increments 1–3, v3.11.55→57; см. план §6 blueprint + §7 shipped, и §0-врезку выше). QID author-authority sidecar + curated editorial namespace + precedence-guard + author-landing surface + authority build-gate — всё live+prod-verified. **W1-a + W1-c (author-authority в ридере + L2/landing) поглощены квартетом.** **Остаётся (с причиной, план §7.2):** (1) **full-L2-list collapse-by-QID** — блок W4 (search-rows без qid → сломает scoped-search; нужен producer-инкремент: qid в `corpus-search`); (2) **shared-byline-renderer консолидация** (5 поверхностей) — maintainability-рефактор над свежей PC/FB-полировкой, без user-visible-выигрыша; (3) **W1-b** card-source-link (clutter-hold) · **W1-d** roadmap-copy; (4) **Wave-2 контент** (bios/why-read/collections/reading-orders) — perpetual, HOME+slots готовы → дроп как данные под R1/R7-sign-off.
