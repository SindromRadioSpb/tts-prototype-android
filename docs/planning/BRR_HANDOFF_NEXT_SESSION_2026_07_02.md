...
> ## 🔄 ОБНОВЛЕНИЕ 2026-07-04 — retention-программа (§2 п.3) ОТГРУЖЕНА ЦЕЛИКОМ
> Прод сейчас **v3.11.87** (был v3.11.80 на момент написания ниже). «Следующее» из §2 —
> **retention-программа — SHIPPED+PROD** (P0–P5 + P3 + P4, версии v3.11.81→87):
> - **P0** канон-лог `review_log` + `lemma-canon.js` · **P1** `fsrs-core.js` (FSRS-6, байт-парити
>   vs ts-fsrs@5.4.1) · **P2** Зал→FSRS · **P5/P5.6/P5.7** reading-native моат (due-кольцо +
>   reveal-then-grade + ручная-пометка-сидирует-расписание + замыкание цикла) · **P3** Студия→FSRS
>   (единый движок+журнал, память Зал+Студия едина) · **P4** Anki-слияние (инжест→`review_log` +
>   `updateSrsState` живая кросс-экранная сверка).
> - **Живой канон-статус фаз → `docs/planning/RETENTION_PROGRAM_RECON_2026_07_02.md`** (шапка + §8).
>   Доктрина-предок `docs/SRS_STRATEGY_v3_2.md` частично superseded (баннер сверху).
> - **СЛЕДУЮЩЕЕ:** **P4.1** (фикс демоции 'new' в покрытии + развилки A/B/C/D Anki-round-trip/
>   кросс-корпус — разбор в памяти `[[project_retention_program]]`) → **P6** метрика (retention-report;
>   осмысленно через ~3 мес накопления). Затем — отложенные §2 п.4–6 (пилот-инструментовка / E2E-sync /
>   AI-graded).
> - **Ops-урок:** прод CX23 дважды ронял деплой из-за **диска 100%** (накопление docker-образов) →
>   SSH `docker image/builder prune -af` (без `--volumes`) + рестарт `coolify`; профилактика (cron
>   prune / Coolify Docker Cleanup) пока НЕ поставлена — деталь `[[feedback_coolify_deploy_ops]]`.
> - Разделы §1/§3/§4 ниже — исторические (на момент 2026-07-02); §4 grounding УЖЕ ИСПОЛЬЗОВАН
>   (recon написан и реализован). Проверь актуальный статус Dicta-конвейера (§3) отдельно.
>
> ---
...
3. ✅ **retention-программа — SHIPPED+PROD 2026-07-04** (#2 FSRS + #5 канон-модель памяти +
   #6 reading-native retrieval; v3.11.81→87; канон-статус — recon-док) ·
4. **← СЛЕДУЮЩЕЕ: P4.1 (Anki-round-trip/кросс-корпус развилки) → P6 метрика**, затем
   #3 инструментовка пилота (триггер: новая дата, лид ~3 нед) · 5. #4 E2E-sync recon ·
