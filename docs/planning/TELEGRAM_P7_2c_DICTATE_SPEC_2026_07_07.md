...
## SHIPPED v3.11.123 + OWNER LIVE-VERIFIED НА ПРОДЕ 2026-07-08 (dictate LIVE)
...
фильтр; D-4 контраст на живом пути). Полная регрессия зелёная (9 наборов).

**Owner-запуск на проде 2026-07-08:** bake полного безопасного набора (BYOK офлайн, 6912 dictate-
безопасных лемм из 9279, ~61К символов = $0 в free-tier WaveNet) → push на том (6912 mp3, keyless-
раздача) → Coolify `PUBLIC_BASE_URL=https://linguistpro.kolosei.com` + `AGENT_REVIEW_WRITE=1` → restart.
**Live-verify в @LinguistProMentorBot ПРОЙДЕН:** /review → 🎧 аудио → `לטוב` (correct) → «✅ Верно —
слово записано правильно на слух»; второй раунд `פן` → ответ `פאל` → «Не засчитано. Ожидалось: «פן»».
Реальные review_log-строки записаны (флаг ON активировал ВЕСЬ P7.2 write-path: reverse+cloze+dictate).

**Наблюдение из live-verify (вход в P7.2d, R5/R2 quality):** второе слово `פן` — 2-буквенная
литературная частица; **85 dictate-безопасных items имеют 2-буквенную письменную форму** (פן, לא, של,
פה, בת, אב, לב…) — слишком короткие/функциональные для честного аудио-диктанта (сигнал слишком мал,
«Ожидалось: «2 буквы»» ощущается несправедливо). НЕ баг (поток корректен), а **уточнение выборки для
P7.2d**: поднять мин. длину written ≥3 в `dictateFormForItemKey` (замер: −85 items = 1.2% набора)
и/или исключить function-POS из dictate. Прод-ассеты 2-буквенных слов остаются, просто перестанут
выбираться (безвредно). Решение — в дизайн-фазе P7.2d (docs/planning/TELEGRAM_P7_2d_SELECTOR_PREP).
