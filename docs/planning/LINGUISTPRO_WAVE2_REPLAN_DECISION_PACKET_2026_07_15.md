...
> **Execution delta — 2026-07-16, current routing:** this packet remains the historical dependency/authority decision. G0, C3a, N1 and the LB0–LB2 implementation track are complete; lesson-quality evidence remains intentionally paused at `OPERATIONALLY_COMPLETE / EVIDENCE_DEFERRED`. S0 Scale Envelope, S1 Role/Authority Registry and S2 Typed Artifact/Context contracts are owner-approved. S3 CP0 is `DESIGN_APPROVED`, not `OPERATIONALLY_COMPLETE`: no observer/runtime/migration/deployment exists yet. The owner-directed execution amendment makes the canonical single-implementer path **S3 execution authorization → S3-T/CI → S3-L 10,000-run synthetic with zero real external API calls → healthy default-off deploy → `ENGINEERING_COMPLETE / LIVE_EVIDENCE_DEFERRED` → F1 correctable continuity**. The unavailable seven-day S3-O owner-live window moves to a parallel deferred evidence track and no longer blocks F1; later findings require tracked remediation and relevant gate reruns. Full S3 `OPERATIONALLY_COMPLETE`, CP1, AA2 runtime access and external cohorts remain blocked until that live evidence is completed. AA0 personal no-secret evidence and AA1 OAuth/tool-schema/threat-model documentation remain parallel and separately gated; AA3 follows bounded evidence and public AA4 waits for S7. This routing update authorizes documentation only — not S3 implementation, AA0 execution, Hermes/MCP/OAuth connection, F1, CP1, durable lesson retention, background jobs, M1 or S4–S7 work.
