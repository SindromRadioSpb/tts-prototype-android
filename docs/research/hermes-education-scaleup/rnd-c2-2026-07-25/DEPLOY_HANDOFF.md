...
This is the operator handoff for the owner-enabled experimental product surface.

## Completed deployment

- Final product deployment via `deploy-c2-webui.ps1` has receipt status `PASS` at
  `2026-07-25T11:33:07.5825465+03:00`.
...
- Eight predefined topics and free conversation are available on desktop and iPhone. Topics are
  soft starting points and may change naturally during a session.
- Product tokens use `purpose=practice`. Product sessions do not call `/result`, write
  localStorage or modify retained RT1/RT2 research evidence.
- Final checks: product token HTTP 200; desktop and 390 x 844 mobile UI PASS; no horizontal
  overflow; pre-start transcript/controls hidden; console errors 0; microphone not activated.
- See `PRODUCT_CONTRACT.md` for the privacy, advisory-only and research-boundary contract.
...
- The browser now uses the matching constrained `v1alpha` WebSocket endpoint and decodes binary
  ...
  `audioSent: false`.
- The former device/run selector is preserved only in research history. It is not part of the
  product surface.
