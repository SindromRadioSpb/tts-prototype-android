...
### 3b. End-to-end runtime verdict (where timing actually comes from)

`MediaHost.restoreForRows()` run over the real `buildExactBindingPassport(...)` for both cards:

| | `TEXT-A` | `TEXT-B` |
|---|---|---|
| rows / segments | 432 / 432 | 561 / **432 (another card's)** |
| `timingSource` | `studio-exact-binding` | `null` |
| timing entries | 432 | **0** |
| `timingDropReason` | — | `NO_EXACT_SEGMENT_MAPPING` |

Timing is never stored per row. It exists only as `start_ms`/`end_ms` inside
`studio_caption_revisions.segments_json`; a row reaches it through a row→segment link produced by
one of three `media-host.js` paths, in priority order: `studio-exact-binding` (stored
`caption_segment_id`), `composite-positional` (index mapping, admitted only when row count equals
segment count **and** ≥95% of rows match their segment word-for-word), and `aligned-offline`
(K3 text alignment).

All three refuse for `TEXT-B`: no stored link (0/561), positional blocked by 561 ≠ 432, alignment
returns `ROW_NOT_IN_SEGMENT` because the segments belong to different media. The honesty gates
therefore worked exactly as designed — the only unguarded write in the chain was the binding itself.
`TEXT-B` still plays `PKG-1`'s blob, without karaoke and without per-row replay.
...
- `README.md` — this evidence record. Measurements are reproduced inline rather than as dumps,
  because the raw query output contains the owner's private library content.
- `screenshots/import-center-380-ru-not-prepared.png` — the new honest catalog state at 380 px RU,
  generated by `npm run smoke:import-center:browser` on a synthetic fixture (no owner content).
  The same gate writes its live output to `docs/research/studio-p4-import-center/2026-08-02/screenshots/`;
  this copy is the durable artifact for the 2026-08-06 change.

## Implementation status

F1/F2/F3 of the packet are implemented at `APP_VERSION=3.11.315` (`CACHE_VERSION=v3.11.315`,
locale `?v=117`). Full suite: 820 tests, 816 pass, the same four pre-existing failures that fail on
the parent commit (three `classic mode …` layout assertions and one `recovery UX …` assertion whose
expected strings moved into `media-host.js` before this change). No new failure was introduced.
