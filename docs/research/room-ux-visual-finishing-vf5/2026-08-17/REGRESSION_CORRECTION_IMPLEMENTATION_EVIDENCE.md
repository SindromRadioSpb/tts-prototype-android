...
F7=3.11.400_CACHE_BUST_AND_STATIC_ROLLBACK
...
- VF0–VF4/release-lock targeted suite — PASS `72/72`.
...
- Old HTML/new SW: the new worker precaches `/js/media-host.js?v=400`; an old
  unversioned request still resolves through the current worker/network fallback.
- New HTML/old SW: the `?v=400` URL cannot match the old precache entry and therefore
...
- Static rollback: revert this bounded commit, restore release `3.11.399`, redeploy,
