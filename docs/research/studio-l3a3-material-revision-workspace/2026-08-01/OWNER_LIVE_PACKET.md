...
Client version: `3.11.283`

Browser migrations: `MIGRATIONS.length=46`; v46 is `MIGRATIONS[45]`

## Status boundary

Automated implementation and local gates are complete. Owner-live is **not** claimed.
No push, deploy, production/server schema or data mutation, new server API, cloud sync,
Hermes, L2/L4/L5/L6, or full-media ZIP was performed.
...
- material core/repository: 9/9 pass;
- 380 px material browser gate: RU + HE, no horizontal overflow, no page errors,
...
- full `npm test`: 695 total, 686 pass, 9 fail — the same known baseline class:
...
1. Before opening the app, record `git log -1 --format=%H`, verify version
   `3.11.283`, and verify actual `MIGRATIONS.length=46`.
...
5. Change one manual RU value and select **Сохранить без модели**. Confirm the network
...
6. Make timing-only and speaker-only transcript edits. Confirm affected-row count is
   zero and a zero-call revision can bind the exact new caption revision.
7. Change text in one mapped caption. Confirm only mapped rows are listed, source-owned
...
8. Select each configured provider contract (`gcp`, `madlad`, `google-free`, `gemini`)
...
9. Exercise split and merge. Confirm Workspace shows `MAPPING_REVIEW_REQUIRED` and does
   not enable targeted regeneration until mapping is resolved. Verify honest 0/1/N
   badges; do not accept guessed mappings.
10. Open the same promoted material in two tabs. Commit in tab A, then attempt a commit
    from the old base in tab B. Tab B must show stale-base recovery and must not alter
    the head or compatibility projection.
11. Use **Полная новая версия** only from the advanced disclosure. Confirm explicit cost
    preview, no fallback, one new immutable revision, and that all prior revisions remain
    selectable/readable.
12. Reopen Library/Reader/notes/SRS/Anki/audio surfaces for the material. Confirm no
