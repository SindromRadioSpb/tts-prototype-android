...
## Shipped (v3.11.11)
- **Single source of truth:** `--ws-{new,l1,l2,l3,l4,known,ignore}-{bg,bd,tx}` CSS variables (light + both dark selectors). Table words, card chips, family chips + the new legend all reference them.
- **Premium temperature palette:** new=blue · 1=amber · 2=orange · 3=lime · 4=teal · **known=emerald (NOT transparent)** · ignore=slate. Card status buttons are per-state soft chips (bg+border+text); active = 2px border + bold + inset ring; focus-visible outline (a11y).
