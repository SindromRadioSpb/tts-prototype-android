...
## Реализация 3.11.471: карточка слова, повторное заполнение и мультивыбор

Owner-проверка `יִשְׂרָאֵל` выявила три разрыва одного жизненного цикла:
append-only решение корректно уменьшало очередь, но не попадало в карточку
слова; при повторном открытии форма забывала уже подтверждённый разбор; выбрать
ровно два из трёх оставшихся примеров было невозможно.

Исправление сохраняет единственный источник истины:

- карточка читателя строит read-only проекцию только по точному
  `text_key + sentence_id + order_index + word_offset + source sentence`;
- в карточку допускается только действующая `manual_correction`; подтверждение
  кандидата без доступного в читателе свежего candidate fingerprint остаётся
  fail-closed;
- последний подтверждённый разбор безопасной группы предзаполняет форму как
  черновик и явно подписан; без просмотра и подтверждения новых событий нет;
- контексты стали независимыми checkbox: можно выбрать произвольное подмножество,
  видеть `N из M` и проверить точный список до записи;
- для неоднородной группы UI разрешает только один выбранный контекст;
- `propernoun`, `participle` и `other` локализуются и в карточке слова, поэтому
  внутренние POS-коды больше не просачиваются на пользовательскую поверхность.

Проверки до production: scoped unit/release-lock 36/36, полный lexical audit
40/40 (включая full/slim backup roundtrip и Obsidian receipt), i18n 233/233,
локальный browser smoke RU 380 px / HE RTL 380 px / EN desktop, отдельный
reader-card smoke с `Израиль` и provenance `проверено вами`.

Полный `npm test`: 1300/1306 PASS. Те же шесть несвязанных legacy-падений
воспроизведены на чистом `HEAD` до этого релиза: три контракта старого Classic
mode, один Import Center shortcut и два старых Room IA disclosure-контракта.
Изменения 3.11.471 этих поверхностей не касаются; release-lock падения после
смены версии устранены и входят в scoped green gate. Итоговый production-статус
и commit фиксируются после deploy.
