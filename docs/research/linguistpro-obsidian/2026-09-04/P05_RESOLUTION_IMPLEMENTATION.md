...
## Совместимость проекции карточки 3.11.472

Живая owner-проверка после 3.11.471 доказала, что само событие для
`lpro:...:200a12cf-...:5` корректно сохранено и экспортируется как `resolved`, но
долгоживущая вкладка могла держать LocalDb namespace без нового точечного
метода чтения. Окно проверки при этом работало через установленное чтение по
тексту, а карточка молча оставалась без проекции.

3.11.472 сохраняет быстрый indexed lookup как основной путь и добавляет
совместимый read-only путь по тексту. Он обязательно фильтрует события по тому
же exact `occurrence_id`, после чего повторно проверяет криптографический
`source_anchor`; расширения решения на похожие слова или предложения нет.
Regression smoke теперь использует слово во второй позиции предложения и
эмулирует именно старый LocalDb namespace. До deploy: lexical/release tests
green, i18n 233/233, browser reader показывает `Израиль · проверено вами`.

## Канонически эквивалентные огласовки 3.11.473

Owner-профиль выявил второй, независимый fail-closed разрыв: resolver сохранил
токен `יִשְׂרָאֵל` с порядком combining marks `sin dot -> sheva`, а reader
передал визуально и канонически эквивалентное `sheva -> sin dot`. Буквы,
sentence UUID, offset, текст и контекст совпадали, но сырой SHA-256 по правилам
не совпал и карточка отклонила проекцию.

3.11.473 не нормализует и не переписывает исторические события. Если прямой
`source_anchor` не совпал, read-only projector перебирает ограниченный набор
перестановок только тех же Unicode combining marks внутри нажатого токена.
Проекция допускается лишь тогда, когда одна перестановка воспроизводит
сохранённый SHA-256 байт-в-байт. Буквы, предложение, text key, sentence UUID,
order и offset при этом не меняются; изменение контекста или букв остаётся
`null`/fail-closed. Для изменённого файла также поднят самостоятельный cache
key `lexical-resolution-service.js?v=4`.

## Надёжный checkpoint Obsidian receipt 3.11.474

После успешного жизненного цикла owner-профиль показал `QuotaExceededError`:
ZIP уже был сформирован и передан браузеру, но последующая запись полного
receipt в почти заполненный `localStorage` ошибочно показывала весь экспорт как
неудачный. В 3.11.474 полный receipt по-прежнему является частью ZIP, а его
локальный operational checkpoint хранится в отдельной IndexedDB
`linguistpro-operational-receipts-v1`. Старый localStorage-ключ читается только
для одноразовой совместимости и удаляется после успешной миграции. Ошибка
необязательного checkpoint больше не перечёркивает уже сохранённый ZIP.

## Ручной мультивыбор для всех групп 3.11.475

Owner-проверка показала, что прежнее понятие `batch_review_eligible` было
ошибочно использовано сразу для двух разных прав: безопасного выбора всей
группы и ручного выбора нескольких конкретных контекстов. Поэтому большинство
групп с `unknown_pos`, `collision` или `skipped_token` выглядели как набор
checkbox, но фактически вели себя как radio: новая отметка снимала предыдущую.

В 3.11.475 права разделены:

- любой кластер допускает ручной выбор произвольного подмножества occurrence;
- «Выбрать все примеры» остаётся только у однородного кластера с
  `batch_review_eligible=true`;
- у неоднородного кластера вместо заблокированного checkbox показано
  локализованное пояснение «Выберите примеры вручную» с tooltip;
- exact-impact перечисляет каждый выбранный ID и контекст перед append-only
  записью; невыбранные occurrence не меняются;
- Obsidian-проекция больше не утверждает, что такой кластер можно обрабатывать
  только по одному: она различает ручной поднабор и автовыбор всей группы.

Проверки до production: lexical audit 42/42, i18n 233/233, release/UI locks
38/38, отдельные manual-multiselect контракты 8/8. Browser smoke на 380 px
прошёл в RU/EN/HE: для группы `unknown_pos` отсутствует массовый «Выбрать
все», два независимых checkbox дают exact preview ровно из двух решений,
append-методы в smoke запрещены. Визуальный снимок RU сохранён в
`screenshots/morphology-review-manual-multiselect-380-ru.png`.

Полный `node --test` после обновления всех release locks: 1302/1308 PASS.
Остались ровно шесть ранее известных несвязанных legacy-падений: три Classic
mode, один Import Center shortcut и два Room IA disclosure-контракта. Новых
падений в морфологии, Obsidian, локализации или release-lock нет.

Production preflight выявил, что сам изменённый генератор Obsidian ещё был
подключён по прежнему URL `obsidian-lexical-preview.js?v=3`. Поэтому финальный
release marker поднят до 3.11.476, а URL генератора — до `v=4`; это исключает
старую подпись про выбор примеров в долговременном browser/service-worker
кэше. Функциональная модель ручного мультивыбора 3.11.475 не менялась.

## Единое значение в карточках Зала и Студии 3.11.477

Owner-проверка выявила межповерхностный разрыв: Reading Room уже читал
append-only морфологическое решение по точному occurrence, а адаптер Studio
подключал только отдельный пользовательский перевод из `word_study`. Из-за
этого пользователь был вынужден повторно вводить то же значение, после чего
карточка показывала provenance «ваш» вместо «проверено вами».

В 3.11.477 Studio загружает тот же `LexicalResolutionService` и передаёт в
общую карточку `lookupLexicalResolution`. Проекция остаётся fail-closed и
occurrence-bound: она сверяет text/sentence/order/offset/source anchor и не
распространяет значение на непроверенные омографы. Для старых сохранённых строк,
в которых есть стабильные text/sentence ID, но отсутствует `order_index`, Studio
восстанавливает его из канонической таблицы предложений, а не из текущей
визуальной позиции строки.

Browser-регресс создаёт реальный append-only разбор `יַאלְלָה` со значением
`Давай! Вперёд!`, открывает его в Studio и проверяет значение, локализованную
часть речи «междометие», provenance «проверено вами» и отсутствие редактора
повторного перевода. Отдельный contract-test фиксирует порядок загрузки общего
projector до студийного адаптера.
