...
3. **HE consent template native review** — ⏸ **review brief ready, awaiting reviewer.** RU complete + EN translated; HE skeleton machine-grade per `docs/RESEARCH_ETHICS_CONSENT_TEMPLATE.md` §HE. Resolution path: partner with ulpan teacher for native review (Default option b). Tracked in `RESEARCHER_GUIDE.md` §8 pre-deployment checklist as a blocker for actual ulpan deployment, not for v3.2.0 tag (RU/EN sufficient for development + non-Hebrew-speaking pilots). **2026-05-14 update:** structured review brief authored at [`docs/HE_CONSENT_REVIEW_BRIEF.md`](HE_CONSENT_REVIEW_BRIEF.md) — section-by-section parallel RU/EN/HE layout, 5 placeholder fill-ins (purpose, retention, risks, benefits, contacts) + 2 missing sections (who-conducts, what-asked) + review checklist for already-translated sections (grammar, gender forms, terminology, loan-words, tone, cultural appropriateness, RTL/LTR mixing) + 4 return-format options (inline edit / GitHub PR / comments / Word track-changes). Ready to dispatch; closure now bottlenecked by reviewer response time, not authoring work.
