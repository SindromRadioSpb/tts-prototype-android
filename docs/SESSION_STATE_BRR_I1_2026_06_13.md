...
«Следующий для тебя» ЗАВЕРШЁН и в проде (HEAD e231420, SW v3.10.43).
READ FIRST: docs/SESSION_STATE_BRR_I1_2026_06_13.md + docs/planning/BRR_P1_007_I_PLUS_1_DESIGN_2026_06_12.md
+ .remember/remember.md + docs/PROJECT_ROLES.md (роли R1–R10 применяй авто; R10 = вычислит. морфолог:
дизамбигуация + ИЗМЕРИМОСТЬ обязательна). Owner-инвариант: бескомпромиссное качество, без заглушек.

НОРМЫ: index.html НЕ трогать (Зал = public/library.html, шарят OPFS-движок); MEASURE до кода (.tmp-харнесс;
для профиль-зависимого тестируй НЕПУСТОЙ профиль!); большие фичи → recon-first дизайн в docs/planning/<TICKET>.md
НА УТВЕРЖДЕНИЕ; развилка → варианты по ролям + рекомендация (AskUserQuestion), решает владелец; гейты зелёные
до push; commit+push автономно (Coolify авто-деплой); prod-verify после; SW CACHE_VERSION бамп при смене
shell-ассета (+ CORPUS_VOCAB_DATA_REV при смене формата corpus-vocab-сайдкара); @380px RTL скрин перед UI-коммитом.
SW-апдейт на устройстве = тост «Обновить» (не нужно закрывать вкладки). ⚠ НЕ Windows-curl для UTF-8-иврита.

СОСТОЯНИЕ (всё в проде): i+1-полки Корпуса живут, калибровка §7 на реальном профиле владельца —
CV.CFG зона 70–90%, «знакомо»=сохранённое слово (KNOWN_STATES все состояния), AUTHOR_CAP=0 (показываем все
in-zone). Движок public/js/corpus-vocab.js (pure, smoke:corpus-vocab-engine 37/37). Сайдкар corpus-vocab-v7.json.
Валидация-инструмент ?validate=1 (overlay) + scripts/premium/realprofile-validate-snippet.js — для будущих профилей.

СЕГОДНЯ я хочу: <ВЫБОР>
  (a) Фильтрация/сортировка полок Корпуса (author-diversity и пр. — когда текстов больше; типично для блоков).
  (b) Бейк современных текстов → поднять наполнение i+1-зоны (литкорпус выше словаря; покрытие ~54%).
  (c) Anki-sync (R-3) → реальная mastery в known/learning → вернуть строгие 80–95%.
  (d) другое / полиш («Подробнее→Студия» deep-link; GCP-WaveNet form-audio; ranked-candidate senses).

ОТКРЫТО (owner): 🔑 СРОЧНО ротация AUDIO_UPLOAD_TOKEN (был в чате) + Gemini + GCP TTS key; 47097 = идиш в
ивр.корпусе. Если фича — recon-first дизайн НА УТВЕРЖДЕНИЕ; если профиль-зависимое — измеряй на непустом профиле.
```
