...
- [~] **Phase 9.1** — Foundation (polymorphic schema + versioning + smart-collections) — **in progress (A + B done; C/D/E pending)**
  - [x] **9.1.A** — Schema migrations 021–025 *(commit `8da394e`, 2026-05-10)*. notes_v2 polymorphic table, note_versions, note_links, roots, sentence_notes data migration + read-only VIEW shim. 18/18 Playwright tests pass.
  - [x] **9.1.B** — local-db.js polymorphic API *(commit `3a45833`, 2026-05-10)*. Backwards-compat preserved (upsertNote / listNotes / deleteNote / searchNotes / resolveNote work via VIEW + new schema). New polymorphic helpers: createNote, updateNote (with versioning + 50-FIFO), deleteNoteById, listNotesByTarget, listAllNotesForText, getNoteById, searchAllNotes, listNoteVersions, restoreNoteVersion, setNoteLinks, listOutgoingLinks, listBacklinks, seedRoots, searchRootsAutocomplete, getNotesSmartCollectionsSummary, getTextIdsForNotesSmartChip. **38/38** Playwright tests pass; events-emission-test (Phase 11.0+11.1) regression-clean 23/23; main app load 0 JS errors.
  - [ ] **9.1.C** — Notes modal UI revamp *(invasive — handed off to separate session per user direction; worktree-isolated)*. Target picker (sentence/word/root/binyan/text/note/free), note type switcher (5 kinds), M5 versioning sidebar (history list + diff view + restore button), M7 cross-text smart-chips (6 filters in Library).
  - [ ] **9.1.D** — Bundle compat (`library/notes_advanced.json` in ZIP).
  - [ ] **9.1.E** — i18n keys (ru/en/he) + final premium audit + plan-doc live-status flip to fully [x].
