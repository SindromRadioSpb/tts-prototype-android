...
- [~] **Phase 9.1** — Foundation (polymorphic schema + versioning + smart-collections) — **in progress; live on branch `worktree-agent-ad33453576637a27d`** — NOT yet merged into main; merge waits for C+D+E completion.
  ...
  - [x] **9.1.B** — local-db.js polymorphic API *(commit `3a45833`, 2026-05-10, merged to main)*. Backwards-compat preserved (upsertNote / listNotes / deleteNote / searchNotes / resolveNote work via VIEW + new schema). New polymorphic helpers: createNote, updateNote (with versioning + 50-FIFO), deleteNoteById, listNotesByTarget, listAllNotesForText, getNoteById, searchAllNotes, listNoteVersions, restoreNoteVersion, setNoteLinks, listOutgoingLinks, listBacklinks, seedRoots, searchRootsAutocomplete, getNotesSmartCollectionsSummary, getTextIdsForNotesSmartChip. **38/38** Playwright tests pass; events-emission-test (Phase 11.0+11.1) regression-clean 23/23; main app load 0 JS errors.
  - [~] **9.1.C** — Notes modal UI revamp + **premium hardening pass in progress** *(branch-only, NOT in main)*. Initial implementation by autonomous agent across 5 stages (commits `949a932..e1af2d2`). Interactive premium polish pass added i18n stubs + a11y + dynamic title + dark-mode contrast (`d785aac` + review doc `a2fa06b`). **2026-05-11 hardening pass started** — re-audit + decisive hardening before proceeding to 9.1.D bundle compat. See `docs/research/9_1_C_PREMIUM_HARDENING_PLAN.md` for scope and DoD.
  - [ ] **9.1.D** — Bundle compat (`library/notes_advanced.json` in ZIP) — **pending; blocks on 9.1.C hardening completion.**
  - [ ] **9.1.E** — i18n keys (ru/en/he) + final premium audit + plan-doc live-status flip to fully [x] — **pending; blocks on 9.1.D.**
