...
- [~] **Phase 9.3.5** — Foundation Reinforcement *(branch `phase-9-3-5-foundation-reinforcement`, started 2026-05-11, ~5–7 days)*. End-of-Phase-9.3 strategic retrospective surfaced 6 product gaps that must close before Phase 9.4 morphology — see `docs/research/9_3_5_STRATEGIC_REVIEW.md`. Order **B → D → A → C → E → F** (broken-promise closure before guidance):
  - [ ] **B** — Multi-note row-index panel. Row click → list of all notes for the row + "+ New note" quick-pick → premium "layered learning" UX. Count-badge replaces presence-only badge.
  - [ ] **D** — SRS trainer renders `card_kind='note'` cards. Closes the broken promise from 9.3.C ("🎴 Сделать карточкой" creates card → trainer renders front/back from `body_json` via template schema).
  - [ ] **A** — Target × Type guidance matrix. Per-target canonical type + reasonable/muted/hidden combinations. Smart defaults. Schema CHECKs stay open.
  - [ ] **C** — Event emission completeness. 7 new privacy-safe events: `note_anchor_set/clear`, `note_link_added/removed`, `note_srs_convert`, `note_open`, `template_field_filled`. Metadata only — no body, no Hebrew text. Closes research-mode analytics gap.
  - [ ] **E** — Cross-text "Где встречается" hub (light MVP). Root/binyan reference views grouped by text. NOT force-directed graph (M8 stays in v3.3).
  - [ ] **F** — Word token disambiguation. `target_id = '<sentence_id>:<word_offset>'` for word notes. Legacy bare-lemma = "any occurrence". Phase 9.4 prereq.
  - Plus blind-spot fix: `body_json` schema-versioning (`"v": 1`) added in Workstream A to future-proof template evolution.
...
**Last updated:** 2026-05-11 (post-smoke 9.1.F closure)
