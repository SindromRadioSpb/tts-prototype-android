...
- [x] **Phase 9.1** — Foundation (polymorphic schema + versioning + smart-collections) — **complete on branch `worktree-agent-ad33453576637a27d`**; ready to merge into main. See `docs/research/9_1_FOUNDATION_FINAL_REPORT.md`.
  ...
  - [x] **9.1.E** — i18n keys (ru/en/he) + final premium audit *(branch-only — commits `783e0e8` + `57d13c1`)*. ~90 new keys across `notes.*` / `library.*` / `confirms.*` / `toast.*` namespaces. Hebrew translations machine-grade (native review scheduled before Direction 11 ulpan deployment). Final regression: 23/0 events + **42/0** notes-v2 + main app 0 new JS errors. Initial commit had duplicate-namespace bug (last-key-wins shadowing); `57d13c1` merged keys into existing namespaces; 13/13 RU + 4/4 EN + 3/3 HE critical keys verified.
  - [x] **9.1.F** — Post-smoke hardening *(branch-only, this commit)*. Three bugs caught during user manual smoke-check on 2026-05-11. (1) History versioning UX rewritten — `v_N` now snapshots body AFTER N-th save (was: previous body before save); `createNote` + `upsertNote` seed v1; `v3NotesRestoreNoteIdIfMissing()` back-fills noteId on modal reopen so History sidebar shows existing versions immediately. (2) Notes Preview / textarea / sentence-ctx switched from hardcoded `#fff` to `var(--theme-bg-card)` + `var(--theme-text-primary)` for dark-mode readability. (3) Row TTS adds HEAD pre-flight against `/api/audio/<key>` and falls through to fresh TTS on cache miss; server upload returns 500 on write failure (was silently ok:true). 43/43 notes-v2 (+1 new test).
- [x] **Phase 9.2** — Audio anchoring (M2) *(branch `phase-9-2-audio-anchoring`, 2026-05-11)*. Premium chip in modal: "📍 Привязать к {time}" with live ticking current time when row audio is playing; flips to "📍 0:04.5 ▶︎" + clear button when anchored. Alt+A hotkey, RTL-safe digit cluster, dark-mode contrast, focus-visible rings. Anchor persists on note row (`notes_v2.audio_anchor_ms`), survives modal close/reopen, propagates through bundle export/import roundtrip. Row badge gets a 📍 sub-overlay when the sentence's free note has an anchor. Smart-chip "📍 Audio-noted" already wired in 9.1.C now lights up. Phase also closed a pre-existing Shape A import bug where `item.rows → sentences` reshape silently dropped `row_id`, causing sentence-targeted polymorphic notes from `notes_advanced.json` to be dropped on import. **47/47 + 23/23** tests + 0 new JS errors.
- [x] **Phase 9.3.5** — Foundation Reinforcement (Notes Premium redesign) *(branch `phase-9-3-5-foundation-reinforcement`, 2026-05-11..12)*. Original Workstream A–F replaced mid-flight with a **strategic redesign** after 2026-05-12 dogfood surfaced 6 bugs in 9.3 plus user feedback that the 35-cell target×type matrix + 3-pane Markdown editor felt like an "engineered tool" not a premium learning surface. Strategic review (see `docs/research/9_3_5_TARGET_TYPE_REDESIGN.md` + `9_3_5_STRATEGIC_REVIEW.md`) surveyed Anki/Pleco/LingQ/Obsidian/Notion/RemNote and approved **Option C** (lock-at-creation + explicit Convert) + WYSIWYG editor scope add. New plan executed as workstreams **R1–R6** + ad-hoc bug-closure pass **R2.1 / R2.2** + scope-decision commit.
  - [x] **R1** *(commit `b053ed0`)* — Lock-at-creation UX + intent quick-pick + word token picker (`'<sid>:<offset>'` target_id format locked, ready for 9.4 morphology) + anchor reactivity (closes Bug 1.1 from dogfood). Target × Type segmented controls removed from UI — schema CHECKs remain open at DB level.
  - [x] **R2** *(commit `b2ceeba`)* — WYSIWYG free-note editor (native `contenteditable` + compact icon-toolbar + selection bubble + 8 markdown shortcuts; no preview, no legend) + history sidebar lifted out of editor split container (closes Bug 1.2). Decision rationale: `docs/research/9_3_5_WYSIWYG_DECISION.md`.
  - [x] **R2.1** *(commit `5087386`)* — Closed 3 dogfood bugs: bubble theme (hardcoded dark surface), row-index sort (created_at ASC), count-badge persistence (new `getRowNoteCounts` API + `v3NotesRowCountsPrime` bulk-prime + save/delete flow ordering).
  - [x] **R3** *(commit `63af80e`)* — Convert flow: dropdown menu → confirm dialog → API `convertNoteType(id, newType)` snapshots old body to versions + writes blank body for new type + bumps updated_at. New `note_type_convert` event (metadata only). +15 i18n keys × 3 locales.
  - [x] **R4** *(commit `83371a2`)* — 8 DB-level bug-closure tests in `notes-v2-test.html`: sort order, count UNION query + word-target prefix, convert snapshot/blank/preserve/idempotence/SRS-link semantics.
  - [x] **R2.2** *(commit `dc5a759`)* — Closed 8 second-dogfood bugs: row-click always opens row-index (empty-state has intent buttons); multi-Free notes per row (fresh-intent skip pre-fill + retire legacy upsertNote in save); badge survives non-hover (row-note-active driven by count>0); close→row-index navigation (`v3NotesModalReturnContext`); 🎴 ✓ В SRS opens Trainer directly; convert drops linked SRS card + `srs.cleanupOrphanedNoteCards` sweep on Trainer open; toast z-index 100001; history preview height `min(60vh, 520px)`.
  - [x] **SRS scope decision** *(commit `f6213c8`)* — Approved 2026-05-12. LinguistPro = creation+linkage layer; Anki = recommended review layer. In-app Trainer = functional stub. Premium SRS (FSRS + Anki Connect sync + premium Trainer UX) deferred to v3.4 epic. Research retention metric replaced with `cards_exported_to_anki / cards_added_to_srs` mastery-engagement proxy. See `docs/SRS_STRATEGY_v3_2.md`.
  - [x] **R5** *(commit `106fd33`)* — Onboarding refresh (help-card body simplified, help-popover SRS section rewrites to "карточки здесь, повторение в Anki", new Convert section in popover) + Anki CTA banner on SRS Trainer home (dismissible) + new `srs_card_exported_to_anki` event emitted at both Anki export entry points (registered in `CONTRACTS_ANALYTICS.md`).
  - [x] **R6** *(this commit)* — CHANGELOG, `9_3_5_WYSIWYG_DECISION.md`, this PREMIUM_NOTES_PLAN final-state update, R6 smoke checklist for user.
  - **Net coverage:** Bugs 1.1 / 1.2 / 1.3 / 2.x from 2026-05-12 dogfood — closed. Bugs from 2026-05-12 second dogfood (R2.2) — closed. Original Workstreams B / C / F / A — fully absorbed into R1 + R2.1 + R3 + R2.2. Original Workstream D (trainer-side note-card rendering) — split: card creation + drop-on-convert + orphan sweep landed; **premium Trainer UX → v3.4** per SRS_STRATEGY. Original Workstream E (Cross-text «Где встречается» hub) — light MVP still pending; absorbs into 9.4 morphology work or v3.3 polish epic.
- [x] **Phase 9.3** — Linking + Templates + SRS micro-cards (M3 + M4 + M6) *(branch `phase-9-3-templates-links-srs`, 2026-05-11)*. Three milestones in one branch: **M3 Templates** (4 structured forms — word_study / grammar_rule / translation_discrepancy / pronunciation_note, required-field validation, switching-stash, hydrate from existing body_json); **M4 Links** (collapsible panel with outgoing + backlinks, atomic `addNoteLink`/`removeNoteLink`, click-to-navigate backlink chips); **M6 SRS** (migration 026 seeds 4 note-card templates; `srs.createCardFromNote` writes srs_cards row + sets `notes_v2.srs_card_id` back-pointer; confirm dialog + idempotent re-conversion). Trainer-side rendering of `card_kind='note'` deferred to v3.3. Tests: **57/57** notes-v2 (+10 cases) + 23/23 events + 0 new JS errors. ~50 i18n keys × RU/EN/HE. See `docs/research/9_3_TEMPLATES_LINKS_SRS_REPORT.md`.
- [~] **Phase 9.4** — Morphology — **Local-first pre-computed dictionary** *(branch `phase-9-4-morphology`, starts 2026-05-12)*. 2026-05-12 scope-decision overrides original Phase 9.0 § 7 "HebMorph sidecar" recommendation: user opted for fully-local offline-first in-browser layer (no Railway cost, no JVM on user device, no privacy surface). Same hspell-data-files source (~250K word forms), but generated as static asset at build time and shipped via Service Worker cache. 17-point requirement contract locked in `docs/MORPHOLOGY_REQUIREMENTS_v3_2.md`. Key invariants: multi-analysis storage (NOT flattened single-best), word normalization shared build↔runtime, prefix segmentation captured at build, provider abstraction for future v3.4 Tier 3 (DictaBERT in-browser) and Tier 4 (optional cloud fallback). Effort: ~5.5-6.5d.
  - [ ] **9.4.A** — Roots seed JSON (~100 entries) + migration 027 populating `roots` table. 0.5d.
  - [ ] **9.4.B** — Word-study UI: root autocomplete (seed + user-added UNION) + binyan select polish (7 patterns + "other"). 0.5d.
  - [ ] **9.4.C-local** — Build pipeline `scripts/morph/build-morphology.js`: hspell-data-files → multi-analysis JSON + meta.json with provenance/checksum. Idempotent, deterministic. Includes normalization, prefix segmentation. 2.5-3d.
  - [ ] **9.4.D** — Runtime provider chain (`IMorphologyProvider` interface) + `LocalDictionaryMorphologyProvider` + `SeedAutocompleteMorphologyProvider` + lazy fetch on first word_study open + SW caching + word-study auto-fill + manual override semantics + Settings status panel. 1.5-2d.
  - [ ] **9.4.E** — Privacy guard (#17 — morphology lookups never emit events) + smoke test suite ≥30 cases + i18n × 3 + NOTICE.md license update + CHANGELOG + deploy + user smoke checklist. 0.5-1d.
- [~] **Phase 9.5** — Knowledge graph view (M8) — *deferred → v3.3*

---

**Last updated:** 2026-05-12 (Phase 9.3.5 closure + SRS scope decision)
