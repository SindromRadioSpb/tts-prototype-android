...
test("current release lock cache-busts changed Room, media host and locale assets exactly", () => {
  assert.match(studio, /window\.APP_VERSION\s*=\s*"3\.11\.559"/);
  assert.match(roomHtml, /id="roomFooterVersion"[^>]*>v3\.11\.559</);
  assert.match(sw, /const CACHE_VERSION = "v3\.11\.559"/);
  assert.match(studio, /\/js\/media-host\.js\?v=553/);
  assert.match(roomHtml, /\/js\/media-host\.js\?v=553/);
  assert.match(roomHtml, /\/css\/reader-core\.css\?v=399/);
  assert.match(roomHtml, /\/js\/library-ui\.js\?v=545/);
  for (const url of [
    "/js/library-ui.js?v=545",
  ...
  for (const locale of ["ru", "en", "he"]) {
    assert.match(studio, new RegExp(`/i18n/locales/${locale}\\.js\\?v=227`));
    assert.match(roomHtml, new RegExp(`/i18n/locales/${locale}\\.js\\?v=227`));
  }
});
