...
test("shared Reader builder and playback expose localized, current row-TTS actions", () => {
  ...
  assert.match(roomJs, /\/js\/reader-core\.js\?v=399/);
});
...
test("3.11.401 release lock cache-busts changed Room, media host and locale assets exactly", () => {
  assert.match(studio, /window\.APP_VERSION\s*=\s*"3\.11\.401"/);
  assert.match(roomHtml, /id="roomFooterVersion"[^>]*>v3\.11\.401</);
  assert.match(sw, /const CACHE_VERSION = "v3\.11\.401"/);
  assert.match(studio, /\/js\/media-host\.js\?v=401/);
  assert.match(roomHtml, /\/js\/media-host\.js\?v=401/);
  assert.match(roomHtml, /\/css\/reader-core\.css\?v=399/);
  assert.match(roomHtml, /\/js\/library-ui\.js\?v=401/);
  for (const url of [
    "/js/library-ui.js?v=401",
    "/js/reader-core.js?v=399",
    "/css/reader-core.css?v=399",
    "/js/media-host.js?v=401",
  ...
  for (const locale of ["ru", "en", "he"]) {
    assert.match(studio, new RegExp(`/i18n/locales/${locale}\\.js\\?v=169`));
    assert.match(roomHtml, new RegExp(`/i18n/locales/${locale}\\.js\\?v=169`));
  }
});
