...
test("current release lock cache-busts changed Room, media host and locale assets exactly", () => {
  assert.match(studio, /window\.APP_VERSION\s*=\s*"3\.11\.608"/);
  assert.match(roomHtml, /id="roomFooterVersion"[^>]*>v3\.11\.608</);
  assert.match(sw, /const CACHE_VERSION = "v3\.11\.608"/);
  ...
  assert.match(roomHtml, /\/js\/library-ui\.js\?v=608/);
  for (const url of [
