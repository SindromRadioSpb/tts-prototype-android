...
test("current release lock cache-busts changed Room, media host and locale assets exactly", () => {
  assert.match(studio, /window\.APP_VERSION\s*=\s*"3\.11\.580"/);
  assert.match(roomHtml, /id="roomFooterVersion"[^>]*>v3\.11\.580</);
  assert.match(sw, /const CACHE_VERSION = "v3\.11\.580"/);
  ...
  for (const locale of ["ru", "en", "he"]) {
    assert.match(studio, new RegExp(`/i18n/locales/${locale}\\.js\\?v=233`));
    assert.match(roomHtml, new RegExp(`/i18n/locales/${locale}\\.js\\?v=233`));
  }
});
