...
test('metadata projection reads actual YouTube ASR passport, omits transcripts and respects detach',async t=>{
 ...
 const actual=await project(source);assert.equal(actual.videoId,'iG9CE55wbtY');assert.equal(actual.durationSeconds,480.5);assert.equal(actual.language,'he');assert.equal(actual.hasCaptions,true);assert.equal(actual.source,'YouTube');
 source.source.captions.video.author='Test channel';assert.equal((await project(source)).source,'Test channel');
