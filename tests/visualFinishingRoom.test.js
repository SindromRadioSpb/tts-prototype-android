...
test("shared Room identity text remains emoji-free at the current locale cache key", () => {
  ...
  for (const locale of ["ru", "en", "he"]) {
    assert.match(html, new RegExp(`/i18n/locales/${locale}\\.js\\?v=187`));
    assert.match(indexHtml, new RegExp(`/i18n/locales/${locale}\\.js\\?v=187`));
  }
});
...
test("current release surfaces lock together and retain the corrected Room module URL", () => {
  ...
  assert.match(html, /<script type="module" src="\/js\/library-ui\.js\?v=426"><\/script>/,
