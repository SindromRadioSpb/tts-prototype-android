...
test('cold reload reopens a just-saved card instead of exposing a false second save', () => {
  const start = html.indexOf('// Restore last opened Library/Dashboard text after the DOM is ready (F5).');
  const end = html.indexOf('// ──────────────────────────────────────────────', start);
  const boot = html.slice(start, end);
  assert.match(boot, /document\.addEventListener\("DOMContentLoaded"/);
  assert.doesNotMatch(boot, /window\.addEventListener\("load"/);
