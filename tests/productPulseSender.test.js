...
async function fixture() {
  ...
  vm.runInNewContext(fs.readFileSync("public/js/product-telemetry.js","utf8"),{window:root,document:doc,location:{hostname:"app.test",pathname:"/library.html"},navigator:nav,sessionStorage:{setItem(){}},crypto:webcrypto,Date:Clock,fetch:async(url,opts)=>{if(opts&&opts.body)events.push(JSON.parse(opts.body));return{ok:true,json:async()=>url.includes("client-config")?{version:"3.11.606"}:{collect:true}};},setInterval:fn=>timers.push(fn)});
