...
test("B5 exposes one release version across Studio, Room and service worker", () => {
  ...
  const studioMorph = indexHtml.match(/\/js\/morph-host\.js\?v=(\d+)/);
  const roomMorph = libraryHtml.match(/\/js\/morph-host\.js\?v=(\d+)/);
  const roomModule = libraryHtml.match(/\/js\/library-ui\.js\?v=(\d+)/);
  assert.ok(studioMorph && roomMorph && roomModule, "versioned shared shell assets must exist");
  assert.equal(roomMorph[1], studioMorph[1], "Studio and Room must load the same MorphHost revision");
  assert.match(serviceWorker, new RegExp(`"/js/morph-host\\.js\\?v=${studioMorph[1]}"`),
    "the service worker must precache the exact shared MorphHost URL used by both shells");
  assert.match(serviceWorker, new RegExp(`"/js/library-ui\\.js\\?v=${roomModule[1]}"`),
    "the service worker must precache the exact Room module URL");
